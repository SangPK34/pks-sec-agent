...
def test_force_compact_threshold_scales_with_model_max():
    model_threshold = _empty_completion_force_compact_threshold(150_000)
    assert model_threshold == int(150_000 * EMPTY_COMPLETION_FORCE_COMPACT_FRACTION)
    assert model_threshold >= EMPTY_COMPLETION_FORCE_COMPACT_FLOOR
...
def test_empty_completion_max_failures_clamped():
    ...
    old = os.environ.get("PKS_EMPTY_COMPLETION_MAX_FAILURES")
    try:
        os.environ["PKS_EMPTY_COMPLETION_MAX_FAILURES"] = "99"
        assert _empty_completion_max_failures() == 12
        os.environ["PKS_EMPTY_COMPLETION_MAX_FAILURES"] = "0"
        assert _empty_completion_max_failures() == 1
    finally:
        if old is None:
            os.environ.pop("PKS_EMPTY_COMPLETION_MAX_FAILURES", None)
        else:
            os.environ["PKS_EMPTY_COMPLETION_MAX_FAILURES"] = old
