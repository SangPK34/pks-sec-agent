...
from pks.sdk.agents import orchestration_mas_hint as omh
...
def test_maybe_inject_respects_config(monkeypatch: pytest.MonkeyPatch, enabled: bool) -> None:
    monkeypatch.setenv("PKS_ORCHESTRATION_MAS_HINT", "true" if enabled else "false")
    reset_config()
    ...
    if enabled:
        ...
        assert rc.trace_metadata.get("_pks_mas_hint_injected") is True
    else:
        assert calls == []


def test_maybe_inject_skips_non_orchestration_agent(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PKS_ORCHESTRATION_MAS_HINT", "true")
    reset_config()
...
def test_maybe_inject_once_per_run_config() -> None:
    ...
    tool = SimpleNamespace(name="run_specialist")
    fr = SimpleNamespace(tool=tool)
    rc = SimpleNamespace(trace_metadata={"_pks_mas_hint_injected": True})
