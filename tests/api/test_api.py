...
from pks.api.app import create_pks_api_app
from pks.api.commands import CommandExecutionResult, CommandMetadata
from pks.api.sessions import SessionManager
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    monkeypatch.setattr("pks.api.sessions.Runner.run", fake_runner)
    monkeypatch.setenv("PKS_API_KEY", "test-key")
    manager = SessionManager(agent_factory=_fake_agent_factory)
    app = create_pks_api_app(session_manager=manager, command_executor=DummyCommandExecutor())
    return TestClient(app)


@pytest.fixture()
def auth_headers() -> dict:
    return {"X-PKS-API-Key": "test-key"}
