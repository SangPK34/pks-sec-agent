...
from pks import parallel_worker
...
def test_simple_parallel_cancelled_error_uses_interrupt_flow(monkeypatch):
    ...
    monkeypatch.setattr(cli_headless.Runner, "run", cancelled_run)
    monkeypatch.setattr("pks.agents.get_available_agents", lambda: {})
    monkeypatch.setattr("pks.agents.get_agent_by_name", lambda *_args, **_kwargs: dummy_agent)
