...
from pks.sdk.agents import Agent, ModelResponse, OpenAIChatCompletionsModel
...
class BaseCLITest:
    ...
    def create_mock_model_response(
        ...
        from pks.sdk.agents.usage import Usage
    ...
    def run_cli_simulation(
        ...
        """
        # Set streaming mode
        os.environ["PKS_STREAM"] = "true" if stream_mode else "false"
        ...
        mock_patches = [
            ...
            patch("pks.repl.ui.prompt.get_user_input", side_effect=input_simulator),
            patch("pks.repl.ui.logging.setup_session_logging", return_value="test_history.txt"),
            # Session recording
            patch("pks.sdk.agents.run_to_jsonl.get_session_recorder"),
            # CLI UI components
            patch("pks.repl.commands.FuzzyCommandCompleter"),
            patch("pks.repl.ui.keybindings.create_key_bindings"),
            patch("pks.repl.ui.banner.display_banner"),
            patch("pks.repl.ui.banner.display_quick_guide"),
            # LLM calls
            patch("litellm.completion", side_effect=litellm_simulator),
            patch("litellm.acompletion", side_effect=litellm_simulator),
            # Timing functions
            patch("pks.util.start_idle_timer"),
            patch("pks.util.stop_idle_timer"),
            patch("pks.util.start_active_timer"),
            patch("pks.util.stop_active_timer"),
            patch("pks.util.get_active_time_seconds", return_value=1.0),
            patch("pks.util.get_idle_time_seconds", return_value=2.0),
            # Rich console output
        ...
        def apply_patches_and_run():
            with (
                patch.multiple("pks.repl.ui.prompt", get_user_input=input_simulator),
                patch.multiple(
                ...
                patch.multiple("pks.repl.commands", FuzzyCommandCompleter=Mock()),
                patch.multiple("pks.repl.ui.keybindings", create_key_bindings=Mock()),
                patch.multiple(
                    "pks.repl.ui.banner", display_banner=Mock(), display_quick_guide=Mock()
                ),
                patch.multiple(
                    "pks.util",
