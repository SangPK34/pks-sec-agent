...
import pks.repl.commands._virtualization_monolith as virt_monolith
from pks.repl.commands._virtualization_monolith import VirtualizationCommand
...
def test_handle_run_subcommand_unique_prefix_activates(virt_cmd: VirtualizationCommand) -> None:
    ...
    with patch.object(virt_cmd, "handle_activate_image", return_value=True) as act:
        with patch("pks.repl.commands._virtualization_monolith.DockerManager") as DM:
            dm = DM.return_value
...
def test_set_active_container_missing_pks_workspace_no_workspace_setup_error() -> None:
    """PKS_WORKSPACE unset must not trigger NoneType when validating workspace name."""
    recorded: list[str] = []
    ...
    prior_ws = os.environ.pop("PKS_WORKSPACE", None)
    prior_ac = os.environ.pop("PKS_ACTIVE_CONTAINER", None)
    try:
        ...
        assert os.environ.get("PKS_ACTIVE_CONTAINER") == "abc123deadbeef"
        ...
        if prior_ws is not None:
            os.environ["PKS_WORKSPACE"] = prior_ws
        else:
            os.environ.pop("PKS_WORKSPACE", None)
        if prior_ac is not None:
            os.environ["PKS_ACTIVE_CONTAINER"] = prior_ac
        else:
