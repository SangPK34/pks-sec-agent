...
from pks.repl.commands.help import HelpCommand
from pks.repl.ui.banner import _QUICK_GUIDE_COMMANDS_DOC_URL, display_quick_guide
...
class TestDisplayQuickGuide:
    ...
    def test_no_pks_prompt_prefix_and_docs_url_in_output(self):
        buf = StringIO()
...
class TestHelpCommand:
    ...
    def mock_console(self):
        ...
        with patch("pks.repl.commands.help.console") as mock_console:
            yield mock_console
    ...
    def mock_commands_registry(self):
        ...
        with patch("pks.repl.commands.help.COMMANDS", mock_registry):
            yield mock_registry
    ...
    def mock_aliases_registry(self):
        ...
        with patch("pks.repl.commands.help.COMMAND_ALIASES", mock_aliases):
            yield mock_aliases
    ...
    def test_handle_var_usage_panel_includes_help_var_syntax(self, help_command, mock_console):
        ...
        assert "PKS_MODEL" in body or "Examples" in body

    def test_handle_var_known_config_variable(self, help_command, mock_console):
        """/help var PKS_MODEL renders a success Panel with catalog content."""
        result = help_command.handle_var(["PKS_MODEL"])
        assert result is True
        mock_console.print.assert_called_once()
        panel = mock_console.print.call_args[0][0]
        assert isinstance(panel, Panel)
        body = str(panel.renderable)
        assert "PKS_MODEL" in body
        assert "/env" in body

    def test_handle_var_known_extra_only_variable(self, help_command, mock_console):
        """Former “additional” vars (e.g. PKS_YOLO) are in the catalog and get long-form help."""
        result = help_command.handle_var(["PKS_YOLO"])
        assert result is True
        panel = mock_console.print.call_args[0][0]
        assert isinstance(panel, Panel)
        body = str(panel.renderable)
        assert "PKS_YOLO" in body
    ...
    def test_handle_var_unknown_variable_returns_false(self, help_command, mock_console):
        ...
        result = help_command.handle_var(["NOT_A_REAL_PKS_VAR_XYZ123"])
    ...
    def test_handle_var_multiple_names_mixed_ok(self, help_command, mock_console):
        ...
        result = help_command.handle_var(["PKS_DEBUG", "NOT_REAL_VAR_ABC"])
        assert result is False
    ...
    def test_handle_help_calls_print_environment_reference(self, help_command, mock_console):
        ...
        with patch(
            "pks.repl.commands.environment_reference.print_environment_reference"
        ) as mock_ref:
    ...
    def test_handle_env_subcommand(self, help_command, mock_console):
        ...
        assert "PKS_MODEL" in panel_content
    ...
    def test_create_styled_table_function(self):
        ...
        from pks.repl.commands.help import create_styled_table
    ...
    def test_create_notes_panel_function(self):
        ...
        from pks.repl.commands.help import create_notes_panel
    ...
    def test_handle_aliases_with_empty_registry(self, help_command, mock_console):
        ...
        with patch("pks.repl.commands.help.COMMAND_ALIASES", {}):
            with patch("pks.repl.commands.help.COMMANDS", {}):
                result = help_command.handle_help_aliases()
    ...
    def test_handle_help_topics_subcommand(self, help_command):
        ...
        with patch("pks.repl.commands.help.console", console):
            with patch(
                "pks.repl.commands.command_reference_index.help_topic_rows_by_category",
