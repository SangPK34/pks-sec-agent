...
from pks.repl.commands.agent import AgentCommand
...
class TestAgentCommand:
    ...
    def test_get_model_display_with_env_var(self, agent_command):
        ...
        os.environ["PKS_TEST_MODEL"] = "custom-model"
    ...
    @patch("pks.repl.commands.agent.get_available_agents")
    @patch("pks.repl.commands.agent.get_agent_module")
    ...
    @patch("pks.repl.commands.agent.get_available_agents")
    @patch("pks.repl.commands.agent.visualize_agent_graph")
    def test_handle_select_by_name(
        ...
        assert result is True
        assert os.environ.get("PKS_AGENT_TYPE") == "blueteam_agent"
        assert os.environ.get("PKS_AGENT_ROUTE_MODE") == "pinned"
    ...
    @patch("pks.repl.commands.agent.get_available_agents")
    @patch("pks.repl.commands.agent.visualize_agent_graph")
    def test_handle_select_root_agent_sets_auto_route_mode(
        self, mock_visualize, mock_get_agents, agent_command, mock_agents
    ):
        """Selecting root_agent enables auto handoff routing."""
        mock_get_agents.return_value = mock_agents

        result = agent_command.handle_select(["root_agent"])
        assert result is True
        assert os.environ.get("PKS_AGENT_TYPE") == "root_agent"
        assert os.environ.get("PKS_AGENT_ROUTE_MODE") == "auto"
        mock_visualize.assert_called_once_with(mock_agents["root_agent"])

    @patch("pks.repl.commands.agent.get_available_agents")
    @patch("pks.repl.commands.agent.visualize_agent_graph")
    def test_selection_agent_key_is_a_root_alias(
        self, mock_visualize, mock_get_agents, agent_command, mock_agents
    ):
        mock_get_agents.return_value = mock_agents

        assert agent_command.handle_select(["selection_agent"]) is True
        assert os.environ.get("PKS_AGENT_TYPE") == "root_agent"
        assert os.environ.get("PKS_AGENT_ROUTE_MODE") == "auto"
        mock_visualize.assert_called_once_with(mock_agents["root_agent"])
    ...
    def test_handle_select_orchestration_agent_sets_auto_route_mode(
        ...
        assert result is True
        assert os.environ.get("PKS_AGENT_TYPE") == "orchestration_agent"
        assert os.environ.get("PKS_AGENT_ROUTE_MODE") == "auto"
    ...
    def test_handle_select_orchestration_agent_shows_beta_notice(
        ...
        with patch("pks.repl.commands.agent.console.print") as mock_print:
            result = agent_command.handle_select(["orchestration_agent"])
    ...
    def test_handle_select_by_number(
        ...
        # Clear PKS_AGENT_TYPE to ensure clean test
        if "PKS_AGENT_TYPE" in os.environ:
            del os.environ["PKS_AGENT_TYPE"]
        ...
        if result is False:
            ...
            assert "PKS_AGENT_TYPE" not in os.environ
        else:
            ...
            assert os.environ.get("PKS_AGENT_TYPE") == expected_key
    ...
    def test_handle_current_single_agent(self, mock_get_agents, agent_command, mock_agents):
        ...
        mock_get_agents.return_value = mock_agents
        os.environ["PKS_AGENT_TYPE"] = "blueteam_agent"
        os.environ["PKS_PARALLEL"] = "1"  # Ensure single agent mode

        result = agent_command.handle_current([])
        assert result is True

    @patch("pks.repl.commands.agent.get_available_agents")
    def test_handle_current_defaults_to_root_agent(
        self, mock_get_agents, agent_command, mock_agents
    ):
        """Test handle_current defaults to root_agent when PKS_AGENT_TYPE is unset."""
        mock_get_agents.return_value = mock_agents
        os.environ.pop("PKS_AGENT_TYPE", None)
        ...
        assert "root_agent" in current_panel_content
        assert "Agent Key:[/bold] root_agent" in current_panel_content
    ...
    def test_handle_current_agent_not_found(self, mock_get_agents, agent_command, mock_agents):
        ...
        mock_get_agents.return_value = mock_agents
        os.environ["PKS_AGENT_TYPE"] = "nonexistent_agent"
        os.environ["PKS_PARALLEL"] = "1"
    ...
    def test_handle_current_parallel_mode(self, mock_get_agents, agent_command):
        ...
        try:
            ...
            if "PKS_PARALLEL" in os.environ:
                del os.environ["PKS_PARALLEL"]
...
class TestAgentCommandIntegration:
    ...
    @patch("pks.repl.commands.agent.get_agent_module")
    @patch("pks.repl.commands.agent.visualize_agent_graph")
    @patch("pks.agents.get_agent_by_name")
    def test_full_workflow(
        ...
        assert result2 is True
        assert os.environ.get("PKS_AGENT_TYPE") == "agent1"
        ...
        assert os.environ.get("PKS_AGENT_TYPE") == "agent3"
    ...
    def test_environment_variable_handling(self, mock_get_agents):
        ...
        os.environ["PKS_TEST_MODEL"] = "env-specific-model"
