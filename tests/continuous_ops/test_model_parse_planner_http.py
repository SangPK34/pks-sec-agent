...
from pks.continuous_ops.model_parse import parse_mission_with_planner


def test_planner_retries_504_then_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test-planner")
    monkeypatch.setenv("PKS_MISSION_PLANNER_MAX_ATTEMPTS", "3")
...
def test_planner_all_504_sets_http_status(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test-planner")
    monkeypatch.setenv("PKS_MISSION_PLANNER_MAX_ATTEMPTS", "2")
    monkeypatch.setattr("time.sleep", lambda *_a, **_k: None)
