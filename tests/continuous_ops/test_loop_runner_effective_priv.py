"""effective_privileged_worker respects PKS_AVOID_SUDO."""
...
from pks.continuous_ops.loop_runner import LoopConfig, effective_privileged_worker
...
def test_avoid_sudo_disables_effective_priv(monkeypatch, tmp_path: Path, truthy: str) -> None:
    monkeypatch.setenv("PKS_AVOID_SUDO", truthy)
...
def test_no_avoid_sudo_keeps_priv(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.delenv("PKS_AVOID_SUDO", raising=False)
    cfg = _cfg(tmp_path, privileged=True)
