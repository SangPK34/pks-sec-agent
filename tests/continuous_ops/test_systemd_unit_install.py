...
from pks.continuous_ops.systemd_unit import enable_linger_for_session_user, install_user_unit


def test_install_user_unit_success(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    unit = tmp_path / "pks-cops-test.service"
    ...
    monkeypatch.setattr("pks.continuous_ops.systemd_unit.subprocess.run", fake_run)
...
def test_install_user_unit_systemctl_failure(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    unit = tmp_path / "pks-cops-bad.service"
    unit.write_text("[Unit]\nDescription=x\n[Service]\nExecStart=/bin/true\n", encoding="utf-8")
