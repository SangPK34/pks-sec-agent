...
from pks.continuous_ops.scriptgen import render_loop_script
from pks.continuous_ops.systemd_unit import _systemd_extra_env_lines, write_systemd_user_unit
...
def test_write_systemd_user_unit_contains_execstart(tmp_path: Path) -> None:
    ...
    p = write_systemd_user_unit(
        run_dir=tmp_path, service_stem="pks-cops-run_test", python_bin=sys.executable
    )
    assert p.is_file()
    ...
    assert "PKS_AVOID_SUDO" not in txt
    assert "Environment=PKS_CONTINUOUS_OPS_SYSTEMD_PLAIN_LOG=1" in txt

    p2 = write_systemd_user_unit(
        run_dir=tmp_path,
        service_stem="pks-cops-run_avoid",
        python_bin=sys.executable,
        avoid_sudo=True,
    )
    p2_txt = p2.read_text(encoding="utf-8")
    assert "Environment=PKS_AVOID_SUDO=1" in p2_txt
    assert "Environment=PKS_CONTINUOUS_OPS_SYSTEMD_PLAIN_LOG=1" in p2_txt
...
def test_write_systemd_user_unit_embeds_pythonpath(tmp_path: Path) -> None:
    ...
    p = write_systemd_user_unit(
        run_dir=tmp_path,
        service_stem="pks-cops-run_pp",
        python_bin=sys.executable,
        avoid_sudo=True,
        venv_root=None,
        pythonpath_extra="/home/dev/pks/src",
    )
    body = p.read_text(encoding="utf-8")
    assert "Environment=PYTHONPATH=" in body
    assert "/home/dev/pks/src" in body
