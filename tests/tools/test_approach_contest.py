...
from pks.config import reset_config
...
def _reset_cfg(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("PKS_ORCHESTRATION_WORKER_MAX_TURNS", "2")
    reset_config()
    yield
    reset_config()
...
async def test_contest_auto_continues_when_one_branch_runner_raises() -> None:
    from pks.sdk.agents.items import ItemHelpers
    from pks.tools.misc import approach_contest
    ...
    with patch("pks.agents.get_agent_by_name", return_value=mock_agent):
        with patch("pks.sdk.agents.run.Runner.run", side_effect=fake_run):
...
async def test_contest_both_branches_invalid_agent() -> None:
    ...
    with patch("pks.agents.get_agent_by_name", side_effect=boom):
        ctx = MagicMock()
        payload = json.dumps(
            {
...
async def test_run_specialist_filters_to_selected_tool() -> None:
    ...
    with patch("pks.agents.get_agent_by_name", return_value=_mock_agent()):
        with patch("pks.sdk.agents.run.Runner.run", side_effect=fake_run):
...
def test_worker_result_failed_and_status_helpers() -> None:
    ...
    from pks.tools.misc.approach_contest import WorkerResult
...
def test_build_worker_input_contains_required_headings() -> None:
    ...
    from pks.tools.misc.approach_contest import WorkerSpec, _build_worker_input
...
def test_new_group_id_is_unique_and_prefixed() -> None:
    ...
    from pks.tools.misc.approach_contest import _new_group_id
...
async def test_invalid_agent_type_error_lists_available(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("pks.agents.get_agent_by_name", side_effect=boom):
        with patch("pks.agents.get_available_agents", return_value=fake_catalogue):
            tool = approach_contest.run_specialist
            ctx = MagicMock()
            payload = json.dumps(
                {
...
def test_per_worker_output_cap_scales_down_for_multi_branch() -> None:
    from pks.tools.misc import approach_contest as ac
