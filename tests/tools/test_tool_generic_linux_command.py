...
from pks.tools.evidence.capture_notice import packet_capture_failure_notice
...
_CMD_MODULE = "pks.tools.reconnaissance.generic_linux_command"
...
async def test_generic_linux_command_ignores_legacy_pretruncate_flag(mock_run, monkeypatch):
    raw_output = "HEAD\n" + ("middle-data\n" * 5000) + "TAIL"
    mock_run.return_value = raw_output
    monkeypatch.setenv("PKS_CTX_TRUNC", "true")

    result = await generic_linux_command.on_invoke_tool(
        RunContextWrapper(None),
        json.dumps({"command": "cat large.txt"}),
    )
...
@pytest.mark.asyncio
@patch("pks.util.user_prompts.prompt_sudo_elevation")
@patch(f"{_CMD_MODULE}.run_command_async", new_callable=AsyncMock)
async def test_generic_linux_command_does_not_infer_sudo_from_file_content(
    mock_run, mock_sudo_prompt,
):
    report = (
        "The operation returned permission denied during the original test.\n"
        "This sentence is report content, not a shell error."
    )
    mock_run.return_value = report

    result = await generic_linux_command.on_invoke_tool(
        RunContextWrapper(None),
        json.dumps({"command": "cat report.md"}),
    )

    assert result == report
    mock_run.assert_awaited_once()
    mock_sudo_prompt.assert_not_called()
...
@pytest.mark.asyncio
@patch("pks.util.user_prompts.prompt_sudo_elevation", return_value=None)
@patch(f"{_CMD_MODULE}._get_workspace_dir", return_value="/tmp/pks-test-workspace")
@patch(f"{_CMD_MODULE}.run_command_async", new_callable=AsyncMock)
