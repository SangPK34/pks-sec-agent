"""Tests for ``pks.tools.web.fetch_url``."""
...
from pks.tools.web import fetch_url as fetch_url_module
from pks.tools.web.fetch_url import (
    _check_ssrf,
    _clamp_max_chars,
    _detect_kind,
    _extract,
...
class TestSSRFGuard:
    ...
    def test_blocks_when_fqdn_resolves_to_metadata_ip(
        ...
        when PKS_FETCH_ALLOW_INTERNAL=true."""
...
def _reset_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Ensure tests start from a clean PKS_FETCH_* env."""
    for key in (
        "PKS_FETCH_ALLOW_INTERNAL",
        "PKS_FETCH_USER_AGENT",
        "PKS_FETCH_MAX_BYTES",
        "PKS_FETCH_TIMEOUT",
    ):
...
async def test_fetch_blocks_metadata_even_with_internal_allowed(
    ...
    monkeypatch.setenv("PKS_FETCH_ALLOW_INTERNAL", "true")
...
async def test_fetch_json_pretty(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            ...
            content=b'{"name":"pks","stars":42}',
        )

    _install_mock_transport(monkeypatch, handler)
    out = await _invoke(fetch_url, url="https://api.example.com/repo")
    assert '"name": "pks"' in out  # pretty-printed has a space
...
async def test_antibot_fallback_recovers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "pks.tools.web.fetch_url._fetch_curl_cffi_sync", fake_curl
    )
    out = await _invoke(fetch_url, url="https://example.com/")
...
async def test_user_agent_override(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_mock_transport(monkeypatch, handler)
    monkeypatch.setenv("PKS_FETCH_USER_AGENT", "MyAgent/1.0")
...
def test_int_env_parsing(monkeypatch: pytest.MonkeyPatch) -> None:
    from pks.tools.web.fetch_url import _bool_env, _int_env
...
def test_html_fallback_when_trafilatura_returns_none(
    ...
    from pks.tools.web.fetch_url import _extract_html
...
def test_decode_body_handles_latin1_when_misdeclared() -> None:
    from pks.tools.web.fetch_url import _decode_body
...
class TestDetectJSRequired:
    ...
    def test_returns_indicator_when_marker_present(self, marker: str) -> None:
        from pks.tools.web.fetch_url import _detect_js_required
...
class TestBuildJSRequiredMessage:
    ...
    def test_nvd_url_carries_json_api_hint(self) -> None:
        from pks.tools.web.fetch_url import _build_js_required_message
