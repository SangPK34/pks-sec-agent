# PKS TUI Streaming Integration Tests

This directory contains comprehensive integration tests for the PKS TUI streaming functionality. These tests ensure that the streaming implementation works correctly across all parts of the system.

## Test Files

### 1. `test_tui_streaming_integration.py`
Tests the core streaming functionality with different PKS components:
...
- **Non-streaming Mode**: Verifies PKS_STREAM=false still works
...
1. **Environment Variables**
   ```bash
   export PKS_TELEMETRY=false
   export PKS_TRACING=false
   export PKS_STREAM=true  # or false for non-streaming tests
   ```
...
2. **Test Real Scenarios**
   - Base tests on actual PKS usage patterns
