...
from pks.sdk.agents.models.fake_id import FAKE_RESPONSES_ID
...
def clear_shared_model_history():
    from pks.sdk.agents.models.chatcompletions.model import clear_all_histories
...
def test_get_token_info_includes_agent_metadata() -> None:
    ...
    model = OpenAIChatCompletionsModel(model=pks_model, openai_client=dummy_client)  # type: ignore[arg-type]
...
async def test_fetch_response_non_stream(monkeypatch) -> None:
    ...
    model = OpenAIChatCompletionsModel(model=pks_model, openai_client=dummy_client)  # type: ignore
    ...
    assert kwargs["model"] == api_model_id(pks_model)
    assert kwargs["messages"][0]["role"] == "system"
    assert kwargs["messages"][0]["content"] == "sys"
    assert kwargs["messages"][1]["role"] == "user"
    # Unsupported unset fields are omitted from the direct JSON request.
...
async def test_fetch_response_stream(monkeypatch) -> None:
    ...
    assert response.model == pks_model
