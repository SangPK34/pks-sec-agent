...
from pks.sdk.agents.tracing import set_trace_processors
from pks.sdk.agents.tracing.setup import GLOBAL_TRACE_PROVIDER
...
_PKS_ENV_RESTORE_KEYS = ("PKS_TRACING", "PKS_TELEMETRY")


@pytest.fixture(autouse=True)
def restore_pks_tracing_env_after_each_test():
    snapshot = {k: os.environ[k] for k in _PKS_ENV_RESTORE_KEYS if k in os.environ}
    missing = [k for k in _PKS_ENV_RESTORE_KEYS if k not in os.environ]
    yield
    for k in missing:
