...
def load_dotenv_and_defaults():
    """Load the cwd .env as the explicit PKS runtime configuration."""
...
def configure_warnings():
    """Suppress Python warnings except when PKS_DEBUG=2."""
...
def configure_loggers():
    ...
    _LOGGERS = [
        ...
        "pks.sdk.agents", "aiohttp", "LiteLLM", "litellm",
    ]
...
def initialize_ctf_if_needed():
    ...
    from pks.util import setup_ctf
    ...
    if is_pentestperf_available() and os.getenv("CTF_NAME", None):
        ...
        try:
            ...
            if type(exc).__name__ == "CTFSetupError" or any(
                ...
                print(
                    "Pista: define PKSBENCH_IMG_REGISTRY_TOKEN (token GitLab con read_registry para "
                    "registry.gitlab.com) en .env o export; prueba: "
                    'echo "$PKSBENCH_IMG_REGISTRY_TOKEN" | docker login registry.gitlab.com '
...
def create_last_log_symlink(log_filename):
    """Create a symbolic link ``last`` in ``~/.pks/logs`` pointing to the current log file."""
    try:
...
def _ensure_pks_dirs():
    """Create the standard ~/.pks/ directory tree on first run."""
    import os
    base = os.path.join(os.path.expanduser("~"), ".pks")
