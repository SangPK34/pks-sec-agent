...
def _load_component(name: str) -> Any:
    ...
    try:
        ...
        if sys.modules.get(f"pks.internal.components.{name}"):
            return sys.modules[f"pks.internal.components.{name}"]
        return importlib.import_module(f"pks.internal.components.{name}")
    except:
        return None
