...
def _print_deferred_update_notice(console: Console, update_info: dict) -> None:
    ...
    console.print(
        f"[#9aa0a6][PKS] Update available:[/] "
        f"[bold white]{current_version}[/bold white][#9aa0a6] -> [/]"
        f"[bold #ff3355]{latest_version}[/bold #ff3355]"
    )
    console.print(
        "[#9aa0a6][PKS] Run [/][bold #ff3355]pks --update[/bold #ff3355]"
...
def _ensure_headless_bound() -> None:
    ...
    ``__getattr__`` only runs for ``import pks.cli; pks.cli.run_cai_cli`` style
...
def main():
    ...
    # Run them in parallel so cold start is ~max(a,b) instead of a+b. Opt out: PKS_SKIP_UPDATE_CHECK=1
    try:
        ...
        skip_updates = os.getenv("PKS_SKIP_UPDATE_CHECK", "").lower() in ("1", "true", "yes")
    ...
    if parsed_args.unrestricted:
        ...
        # PKS: unrestricted = the steering flag only (extra_body steering_enabled=true), sent to
        # the SAME local gateway you already use. It does NOT redirect traffic to any third party.
    ...
    if parsed_args.tui:
        ...
        from pks.tui.cai_terminal import run_cai_tui
        run_cai_tui()
        return
...
def _handle_update_command():
    ...
    if not oss_mode and not api_key:
        console.print(Panel(
            ...
            "Please set a valid ALIAS_API_KEY in your .env file or environment, "
            "or set [bold]PKS_LICENSE_OFF=1[/bold] to update from public PyPI.",
            title="[red]Authentication Error[/red]",
            border_style="red",
        ))
        sys.exit(1)
