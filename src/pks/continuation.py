...
Uses PKSConfig singleton for model/key configuration instead of os.getenv() calls [S].
"""
...
async def generate_continuation_advice(
    ...
    """
    # Get the model from PKSConfig singleton [S]
    cfg = get_config()
    model_name = cfg.model
    
    # Check if we should use a fallback model for local testing [S]
    # This allows the continuation feature to work even without gpt-5.6-terra credentials
    fallback = cfg.continuation_fallback_model
    if model_name == "gpt-5.6-terra" and fallback:
        model_name = fallback
...
IMPORTANT: Respond with ONLY the continuation prompt. No explanations, no "Here's a prompt:", just the direct instruction."""
    ...
    try:
        ...
        kwargs["api_base"] = resolve_openai_base_url()
