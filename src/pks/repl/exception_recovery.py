...
Environment:
  PKS_REPL_EXCEPTION_RECOVERY          — default on; set 0/false/off to disable.
  PKS_REPL_EXCEPTION_RECOVERY_ATTEMPTS  — max completion attempts (1–5, default 3).
  PKS_REPL_EXCEPTION_RECOVERY_AGENT     — after hints, offer to run the main agent to fix
...
_SYSTEM_PROMPT = """You are helping debug a PKS (cybersecurity agent CLI) runtime error.
...
def is_recovery_enabled() -> bool:
    if os.getenv("PKS_TUI_MODE", "").strip().lower() in ("true", "1", "yes", "on"):
        return False
    v = os.getenv("PKS_REPL_EXCEPTION_RECOVERY", "1").strip().lower()
    return v not in ("0", "false", "no", "off")


def is_agent_recovery_offer_enabled() -> bool:
    v = os.getenv("PKS_REPL_EXCEPTION_RECOVERY_AGENT", "1").strip().lower()
    return v not in ("0", "false", "no", "off")


def recovery_max_attempts() -> int:
    try:
        return max(1, min(5, int(os.getenv("PKS_REPL_EXCEPTION_RECOVERY_ATTEMPTS", "3"))))
    except ValueError:
        return 3
...
def should_skip_model_for_exception(e: BaseException) -> bool:
    ...
    from pks.errors import LLMContextOverflow, LLMProviderUnavailable, LLMRateLimited, LLMTimeout
...
def _litellm_kwargs_for_model(model_name: str, cfg: Any) -> dict[str, Any]:
    ...
    if mn == "alias2-mini" or ("alias" in mn and "alias0.5" not in mn):
        kwargs["api_base"] = (os.getenv("OPENAI_API_BASE") or os.getenv("OPENAI_BASE_URL") or "http://localhost:20128/v1")
...
async def _acompletion_once(model_name: str, user_content: str, cfg: Any) -> str | None:
    ...
    timeout_s = float(os.getenv("PKS_REPL_EXCEPTION_RECOVERY_TIMEOUT", "50"))
...
def build_recovery_agent_user_message(hint: str, brief: str) -> str:
    ...
    return (
        "The PKS REPL hit an error. I authorized you to try to fix the local environment.\n\n"
...
def try_recover_with_model(
    ...
    *,
    ...
    from pks.util.streaming import PKS_GREEN, _PKS_MD_THEME
    ...
    user_blob = (
        "The following exception was raised in the PKS headless REPL main loop.\n\n"
        f"```\n{brief}\n```\n\n"
            ...
            if hint:
                ...
                try:
                    console.print(
                        Panel(
                            Markdown(hint),
                            title=f"[bold {PKS_GREEN}]Model suggestions[/bold {PKS_GREEN}]",
                ...
                if (
                    ...
                    try:
                        ...
                        if Confirm.ask(
                            f"\n{_warn_open}Let the PKS agent try to fix this now?{_warn_close} "
