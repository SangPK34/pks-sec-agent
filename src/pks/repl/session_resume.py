...
def sessions_table_from_metadatas(
    ...
    *,
    ...
    table.add_column("Cost", justify="right", style=f"bold {_PKS_ACCENT}")
...
def find_jsonl_by_token_in_logs(token: str) -> Optional[str]:
    """Newest ``cai_*.jsonl`` in session dirs (``~/.pks/logs``, legacy ``./logs``).
...
def _get_log_files_sorted(logpath: Optional[str] = None) -> list[Path]:
    ...
    if logpath:
        ...
        # Default: cai_*.jsonl in ~/.pks/logs (and legacy ./logs when distinct)
