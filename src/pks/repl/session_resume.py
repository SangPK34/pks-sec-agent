"""
Session resume functionality for PKS.

This module provides functions to resume a PKS session from a JSONL log file,
...
Uses the improved load_history_from_jsonl from run_to_jsonl.py which preserves
token, cache, and agent metadata for accurate session display.
"""
...
from pks.util.config_utils import get_session_logs_dir
...
# REPL tables: match quick-guide / banner accent (see ``pks.repl.ui.banner``).
_PKS_ACCENT = "#ff3355"
_PKS_MUTED = "#9aa0a6"


def _default_session_logs_directories() -> list[Path]:
    """Primary ``~/.pks/logs`` plus legacy ``./logs`` when it differs (not a symlink to same)."""
...
def _sorted_pks_jsonl_session_files(directories: list[Path]) -> list[Path]:
    """Unique ``pks_*.jsonl`` paths under each directory root, newest mtime first."""
    seen: set[str] = set()
    collected: list[Path] = []
    for d in directories:
        if not d.is_dir():
            continue
        for f in d.glob("pks_*.jsonl"):
            key = str(f.resolve())
            if key in seen:
                continue
            seen.add(key)
...
def find_newest_pks_jsonl_by_filename_prefix(session_arg: str) -> Optional[str]:
    """Newest file matching ``pks_{session_arg}*.jsonl`` in default session log dirs."""
    prefix = f"pks_{session_arg}"
    for f in _sorted_pks_jsonl_session_files(_default_session_logs_directories()):
...
def get_session_stats(log_path: str) -> Tuple[str | None, int, int, float, float]:
    """
...
def find_last_session_log() -> Optional[str]:
    ...
    jsonl_files = _sorted_pks_jsonl_session_files(dirs)
    if not jsonl_files:
        return None
...
def display_resumed_session(
    ...
    """
    Display the resumed session using the SAME format as pks-replay.

    Uses cli_print_agent_messages and cli_print_tool_output for consistent
    display with live sessions and replay.
    """
    from pks.util import cli_print_agent_messages, cli_print_tool_output, color
    ...
    if log_path:
        ...
        model, total_input_tokens, total_output_tokens, active_time, idle_time = stats
    ...
    for msg in messages:
        ...
        if role == "user":
            ...
            if content:
                print(color("PKS> ", fg="cyan") + content)
...
def restore_session_stats(log_path: str) -> bool:
    """Read and display token statistics for a resumed session."""
    try:
        _model, total_input, total_output, _active, _idle = get_session_stats(log_path)
        if total_input or total_output:
            console.print(
                f"[dim]Restored session context: "
...
def prompt_parallel_agent_setup(log_agents: list[str]) -> bool:
    ...
    try:
        import questionary
        from pks.repl.commands.settings.general import custom_style
    except ImportError:
...
def setup_parallel_agents_from_log(log_agents: list[str], model: Optional[str] = None) -> bool:
    ...
    """
    try:
        from pks.repl.commands.parallel import PARALLEL_CONFIGS, ParallelConfig
        ...
        os.environ["PKS_PARALLEL_AGENTS"] = ",".join(log_agents)
...
def list_recent_sessions(limit: int = 10) -> list[dict]:
    ...
    for log_file in jsonl_files:
        ...
        model, total_input, total_output, active_time, idle_time = stats
...
def sessions_table_from_metadatas(
    ...
    *,
    ...
    """Rich table for /sessions and /resume (PKS accent palette, numbered rows)."""
    table_kw: dict = {
        "title": f"[bold {_PKS_ACCENT}]{title}[/bold {_PKS_ACCENT}]",
        "show_header": True,
        "header_style": f"bold {_PKS_ACCENT}",
        "border_style": _PKS_ACCENT,
    }
    if include_pick_hint_caption:
        table_kw["caption"] = (
            f"[dim {_PKS_MUTED}]# = row number for bare /resume; "
            f"IDs match /sessions[/dim {_PKS_MUTED}]"
        )
        table_kw["caption_justify"] = "left"
    table = Table(**table_kw)
    table.add_column("#", justify="right", style=f"bold {_PKS_ACCENT}", width=4)
    table.add_column("ID", style="magenta", width=12)
    table.add_column("Date/Time", style="green")
    table.add_column("Agent", style="yellow")
    table.add_column("Msgs", justify="right", style=f"dim {_PKS_MUTED}")
    table.add_column("Tokens", justify="right", style=f"bold {_PKS_ACCENT}")
    table.add_column("Duration", justify="right", style=f"dim {_PKS_MUTED}")
    ...
    for idx, session in enumerate(sessions):
        ...
        token_count = int(session.get("total_input_tokens", 0) or 0)
        token_str = f"{token_count:,}" if token_count > 0 else "-"
        ...
        table.add_row(row_n, short_id, formatted_time, agent_name, msg_count, token_str, duration)
...
def prompt_pick_session_path(sessions: list[dict]) -> Optional[str]:
    ...
    console.print()
    console.print(
        f"[dim {_PKS_MUTED}]Enter #[/dim {_PKS_MUTED}][bold {_PKS_ACCENT}]1[/bold {_PKS_ACCENT}]"
        f"[dim {_PKS_MUTED}]–[/dim {_PKS_MUTED}][bold {_PKS_ACCENT}]{n}[/bold {_PKS_ACCENT}]"
        f"[dim {_PKS_MUTED}] from the table, or [/dim {_PKS_MUTED}][bold {_PKS_ACCENT}]q[/bold {_PKS_ACCENT}]"
        f"[dim {_PKS_MUTED}] to cancel.[/dim {_PKS_MUTED}]"
    )
    try:
        raw = console.input(f"[bold {_PKS_ACCENT}]❯[/bold {_PKS_ACCENT}] [dim {_PKS_MUTED}]Session:[/dim {_PKS_MUTED}] ").strip().lower()
    except (KeyboardInterrupt, EOFError):
        return None
...
def find_jsonl_by_token_in_logs(token: str) -> Optional[str]:
    """Newest ``pks_*.jsonl`` in session dirs (``~/.pks/logs``, legacy ``./logs``).
...
def _format_session_choice_ansi(session: dict, idx: int) -> str:
    ...
    token_count = int(session.get("total_input_tokens", 0) or 0)
    token_str = f"{token_count:,} tok" if token_count else "-"
...
def _format_session_choice(session: dict, idx: int, use_ansi: bool = False) -> str:
    ...
    line1 = f"{short_id} │ {time_str} │ {model_short:14} │ {msg_count:3} msgs │ {token_str:>9}{latest_badge}"
...
def _get_log_files_sorted(logpath: Optional[str] = None) -> list[Path]:
    ...
    Args:
        logpath: Optional custom logs directory. If provided, recursively
                 ...
                 If None, uses ``~/.pks/logs`` (and legacy ``./logs`` when distinct).
    ...
    if logpath:
        ...
        # Default: pks_*.jsonl in ~/.pks/logs (and legacy ./logs when distinct)
        jsonl_files = _sorted_pks_jsonl_session_files(_default_session_logs_directories())
...
def _fast_get_session_metadata(log_file: Path) -> Optional[dict]:
    ...
    to extract model and message count without full parsing.
    ...
    try:
        ...
        with open(log_file, "rb") as f:
            ...
            # Read last 32KB for the last assistant message
...
def _format_session_line(session: dict, idx: int, selected: bool = False) -> str:
    ...
    if selected:
        return (
            ...
            f"{msg_count:3} msgs │ {token_str:>9}{latest} {RESET}"
        )
    else:
        return (
            ...
            f"{DIM}{token_str:>9}{RESET}{latest}"
        )
...
def interactive_session_selector(limit: int = 10, logpath: Optional[str] = None) -> Optional[str]:
    ...
    def render_screen(page_sessions, selected_idx, current_page, has_prev, has_next):
        ...
        # Column header
        print(f"   {DIM}ID       │ Date       │ Model        │ Msgs    │ Tokens{RESET}")
        print(f"   {DIM}─────────┼────────────┼──────────────┼─────────┼────────{RESET}")
...
def _fallback_session_selector(limit: int = 15, logpath: Optional[str] = None) -> Optional[str]:
    ...
    while True:
        ...
        # Column headers
        console.print(
            "[dim]   #  │ ID       │ Date       │ Model          │ Msgs    │ Tokens[/dim]"
        )
