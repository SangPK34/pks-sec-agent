...
* The dialog is shown via :func:`prompt_toolkit.application.run_in_terminal`
  from inside the main REPL key binding (see :mod:`pks.repl.ui.keybindings`).
...
from pks.output import TASK_REGISTRY, TaskRecord
from pks.repl.ui.compact_renderer import format_secs
...
def _print_task_detail(record: TaskRecord, console: Console | None = None) -> None:
    ...
    title = Text()
    title.append("● ", style=f"bold {PKS_GREEN}")
    title.append(record.agent_name or "Agent", style=f"bold {PKS_GREEN}")
...
def _open_fallback_menu(values: list[tuple[str, str]], console: Console) -> None:
    ...
    console.print(Text("Expand task — select a number, then press Enter:", style=f"bold {PKS_GREEN}"))
