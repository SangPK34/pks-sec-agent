...
_QUICK_GUIDE_COMMANDS_DOC_URL = (
    "https://aliasrobotics.github.io/pks/pks/getting-started/commands/"
)
...
def display_banner(
    ...
    *model* and *agent_type* default to ``PKS_MODEL`` / ``PKS_AGENT_TYPE`` when omitted.
    ...
    version = get_version()
    model_name = model if model is not None else os.getenv("PKS_MODEL", "alias1")
...
def _quick_guide_alias_panel_title() -> Text:
    t = Text()
    t.append(" Alias1 ", style="bold #0d1117 on #58F9FF")
    t.append(" — best model for cybersecurity ", style="bold white on #00363f")
    return t
...
def display_quick_guide(console: Console):
    ...
    context_body = Text.assemble(
        ...
        (alias_url, f"{_PKS_GREEN} underline"),
    )
    context_tip = Panel(
        context_body,
