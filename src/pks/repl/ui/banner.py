"""
Module for displaying the PKS banner and welcome message.
"""
...
from pks.repl.ui.agent_notices import is_orchestration_agent, orchestration_beta_text
from pks.repl.ui.repl_input_shortcuts import quick_shortcuts_text

# Match ``pks.util.streaming.PKS_GREEN`` (avoid importing the whole streaming module).
_PKS_GREEN = "#58F9FF"
PKS_GREEN = _PKS_GREEN
_GREY_MID = "#888888"
_GREY = "dim white"
...
_BANNER_SPLIT_GAP = 2
# Minimum terminal width for the two-column help guide.
_MIN_QUICK_GUIDE_SIDE_BY_SIDE = 96

_QUICK_GUIDE_COMMANDS_DOC_URL = "https://github.com/SangPK34/pks-sec-agent"
...
_MODEL_HINT_FULL = "(Use a frontier reasoning model for best Cybersecurity performance)"
_MODEL_HINT_MED = "(Frontier reasoning model)"
...
def get_version():
    """Version for banner/UI: installed ``pks-framework`` (matches pip / ``pks --version``), else cwd pyproject."""
    try:
        import importlib.metadata

        return importlib.metadata.version("pks-framework")
    except importlib.metadata.PackageNotFoundError:
        pass
    except Exception as e:  # pylint: disable=broad-except
        logging.warning("Could not read installed pks-framework version: %s", e)
...
def _session_banner_title_bar(console: Console, version: str, *, panel_outer_width: int) -> Text:
    """Full *Cybersecurity AI* row, or compact *PKS v…* when the panel is narrow."""
...
def _banner_model_cell(model: str, hint: str) -> Text:
    t = Text()
    t.append(model, style=f"bold {_PKS_GREEN}")
    t.append(" ", style="")
...
def _build_session_banner_panel(
    ...
    *,
    ...
    cmds.add_column("Command", style=f"bold {_PKS_GREEN}", no_wrap=True)
    ...
    if not _yolo:
        _promo_rows.append(
            Text.assemble(
                ("Try ", "bold yellow"),
                ("pks --yolo", "bold black on bright_yellow"),
...
def _banner_logo_markup() -> Text:
    raw = [
        "                PPPPPPPPPPPPP         SSSSSSSSSSSSSSS",
        "                P::::::::::::P      SS:::::::::::::::S",
        "                P:::::::::::::P    S:::::SSSSSS::::::S",
        "                P::::::PPPP::::P   S:::::S     SSSSSSS",
        "                P:::::P    P::::P  S:::::S",
        "                P:::::P     P::::P S:::::S",
        "                P:::::P     P::::P  S::::SSSS",
        "                P:::::P    P::::P    SS::::::SSSSS",
    ...
    body = "\n".join(f"[bold {_PKS_GREEN}]{line}[/bold {_PKS_GREEN}]" for line in padded)
...
def count_tools():
    """Count the number of tools in the PKS framework."""
    try:
        # Count Python files in the tools directory
        tool_files = glob.glob("pks/tools/**/*.py", recursive=True)
...
def count_agents():
    """Count the number of agents in the PKS framework."""
    try:
        # Count Python files in the agents directory
        agent_files = glob.glob("pks/agents/**/*.py", recursive=True)
...
def count_ctf_memories():
    """Count the number of CTF memories in the PKS framework."""
...
def display_banner(
    ...
    *,
    ...
    """
    Display the PKS session header: green ASCII logo and session panel.

    Wide terminals: logo left, panel right. Narrow terminals: logo above, panel below.

    *model* and *agent_type* default to ``PKS_MODEL`` / ``PKS_AGENT_TYPE`` when omitted.
    Prefer passing :class:`PKSConfig` values from startup so the banner matches the active session.

    Title and model hint shorten automatically on narrow terminals.
    """
    # Blank line between the shell line (e.g. ``$ pks``) and the banner.
    console.print()

    version = get_version()
    model_name = model if model is not None else os.getenv("PKS_MODEL", "gpt-5.6-terra")
    agent_name = agent_type if agent_type is not None else os.getenv(
        "PKS_AGENT_TYPE", DEFAULT_AGENT_TYPE
    )
...
def display_framework_capabilities(console: Console):
    """
    Display a table showcasing PKS framework capabilities in Metasploit style.
    ...
    capabilities_panel = Panel(
        table,
        title="[bold blue]PKS Features[/bold blue]",
        border_style="blue",
        padding=(1, 2)
    )
...
def display_agent_overview(console: Console):
    ...
    # Add agent rows
    agents = [
        ("ctf_agent", "Basic CTF solver", "CTF challenges, Linux operations"),
...
def session_summary_panel_title() -> Text:
    ...
    t = Text()
    t.append(" PKS ", style="bold #0d1117 on #58F9FF")
    t.append(" Session Summary ", style="bold white on #00363f")
    return t
...
def _quick_guide_outer_title() -> Text:
    ...
    t = Text()
    t.append(" PKS ", style="bold #0d1117 on #58F9FF")
    t.append(" defacto scaffolding for cybersecurity agents ", style="bold white on #00363f")
    t.append("— /help <topic> for detailed docs ", style=f"bold {_PKS_GREEN} on #00363f")
    return t
...
def environment_reference_outer_title() -> Text:
    ...
    t = Text()
    t.append(" PKS ", style="bold #0d1117 on #58F9FF")
    t.append(" environment reference ", style="bold white on #00363f")
    t.append("— /env list + runtime hints ", style=f"bold {_PKS_GREEN} on #00363f")
    return t


def help_topics_outer_title() -> Text:
    """Outer title for ``/help topics`` (same chrome as other PKS help panels)."""
    t = Text()
    t.append(" PKS ", style="bold #0d1117 on #58F9FF")
    t.append(" topics ", style="bold white on #00363f")
    t.append("— commands by category + /help <topic> ", style=f"bold {_PKS_GREEN} on #00363f")
    return t


def _quick_guide_model_panel_title() -> Text:
    t = Text()
    t.append(" Model ", style="bold #0d1117 on #58F9FF")
    t.append(" — endpoint and reasoning ", style="bold white on #00363f")
    return t


def _quick_guide_subpanel_title(label: str) -> Text:
    """Title strip matching the model panel style for subpanels."""
    t = Text()
    t.append(f" {label} ", style="bold #0d1117 on #58F9FF")
    return t


def _quick_guide_top_row(console: Console, help_ref: Text, right_column) -> Group | Table:
    """Two columns when wide enough; stacked (reference, then GPT-4o mini) when narrow."""
    if _safe_console_width(console) >= _MIN_QUICK_GUIDE_SIDE_BY_SIDE:
        ...
        # Wider right column for GPT-4o mini (no fixed width cap — was leaving a large empty gap).
...
def display_help_topics_index(console: Console):
    ...
    from pks.repl.commands.command_reference_index import help_topic_rows_by_category
    ...
    intro_block = Text.assemble(
        (
            "PKS (Cybersecurity AI): penetration testing, bug bounty hunting, and security research.\n\n",
            "white",
        ),
        (
    ...
    def _topic_rows_table(rows: list[tuple[str, str]]) -> Table:
        ...
        tbl.add_column(style=f"bold {_PKS_GREEN}", ratio=1, min_width=18)
    ...
    def _category_block(title: str, rows: list[tuple[str, str]]) -> Group:
        heading = Text(title + "\n", style=f"bold underline {_PKS_GREEN}")
...
def display_quick_guide(console: Console):
    """Quick guide: banner palette; wide = command ref | GPT-4o mini, then full-width tail; else stacked."""
    g = f"bold {_PKS_GREEN}"
    rule = f"{'━' * 55}\n"

    help_ref = Text.assemble(
        ("PKS Command Reference", f"bold underline {_PKS_GREEN}"),
        "\n\n",
        (rule, _GREY_MID),
        "\n",
        ("AGENT MANAGEMENT", g),
    ...
    def _quick_guide_env_value_style(var_name: str, raw: str) -> str:
        """Booleans: on/true in PKS green; off/false dim. Other vars: neutral white."""
        v = (raw or "").strip().lower()
        if var_name in ("PKS_STREAM", "PKS_TOOL_STREAM"):
            if v in ("true", "1", "yes", "on"):
                return f"bold {_PKS_GREEN}"
            return "dim white"
        return "white"
    ...
    workflow_text = Text.assemble(
        ...
        ("Quick Start Workflows", f"bold {_PKS_GREEN}"),
        "\n\n",
        ("CTF Challenge", g),
        "\n",
        ("  1. ", _GREY),
    ...
    context_body = Text.assemble(
        ...
        ("Endpoint: ", _GREY),
        (os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1"), g),
        ("\nModel: ", _GREY),
        (current_model, g),
        ("\nThinking: ", _GREY),
        (os.getenv("PKS_REASONING_EFFORT", "provider default"), g),
        ("\n\nUse /model and /effort to change them.", _GREY),
    )
    context_tip = Panel(
        context_body,
        title=_quick_guide_model_panel_title(),
        title_align="left",
        border_style=_PKS_GREEN,
        padding=(1, 1),
    )
    _stream = os.getenv("PKS_STREAM", "false")
    _tool_stream = os.getenv("PKS_TOOL_STREAM", "true")
    env_body = Text.assemble(
        ("  PKS_MODEL ", _GREY),
        ("= ", _GREY),
        (f"{current_model}\n", _quick_guide_env_value_style("PKS_MODEL", current_model)),
        ("  PKS_AGENT_TYPE ", _GREY),
        ("= ", _GREY),
        (f"{current_agent_type}\n", _quick_guide_env_value_style("PKS_AGENT_TYPE", current_agent_type)),
        ("  PKS_PARALLEL ", _GREY),
        ("= ", _GREY),
        (f"{os.getenv('PKS_PARALLEL', '1')}\n", "white"),
        ("  PKS_STREAM ", _GREY),
        ("= ", _GREY),
        (f"{_stream} (LLM)\n", _quick_guide_env_value_style("PKS_STREAM", _stream)),
        ("  PKS_TOOL_STREAM ", _GREY),
        ("= ", _GREY),
        (f"{_tool_stream} (Tools)\n", _quick_guide_env_value_style("PKS_TOOL_STREAM", _tool_stream)),
        ("  PKS_WORKSPACE ", _GREY),
        ("= ", _GREY),
        (f"{os.getenv('PKS_WORKSPACE', 'default')}\n", "white"),
        ("  PKS_TEMPERATURE ", _GREY),
        ("= ", _GREY),
        (f"{os.getenv('PKS_TEMPERATURE', '0.7')}\n", "white"),
        ("  PKS_TOP_P ", _GREY),
        ("= ", _GREY),
        (f"{os.getenv('PKS_TOP_P', '1.0')}", "white"),
    )
    env_panel = Panel(
        env_body,
    ...
    tips_body = Text.assemble(
        ...
        (
            "  • Default entry agent is root_agent; /help agent compares routing vs orchestration_agent (BETA)\n",
            _GREY,
        ),
    ...
    _privacy_text = Text.assemble(
        (
            "PKS collects pseudonymized data to improve our research.Your privacy is protected in compliance with GDPR.",
            _GREY,
        ),
    )
