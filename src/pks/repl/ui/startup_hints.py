...
Disable with PKS_NO_STARTUP_HINTS=1 for CI or scripts.

Minimum time each message stays visible (so fast phases are readable):
``PKS_STARTUP_HINT_MIN_SEC`` (default ``0.55``). Set ``0`` to disable pacing.
"""
...
from pks.util.hint_renderables import build_startup_hint_renderable


def startup_hints_disabled() -> bool:
    v = os.environ.get("PKS_NO_STARTUP_HINTS", "").strip().lower()
...
class StartupHints:
    """Idempotent Rich status line: ``[PKS] | message`` with dim spinner."""
    ...
    def _min_visible_phase(self) -> float:
        if startup_hints_disabled():
            return 0.0
        try:
            return float(os.environ.get("PKS_STARTUP_HINT_MIN_SEC", "0.55"))
        except ValueError:
            return 0.55
