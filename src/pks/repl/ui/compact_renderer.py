...
Subscribes to :class:`pks.output.OutputManager` and renders a transient Rich
...
The ``[Pn]`` agent-id pill is only shown when ``PKS_PARALLEL > 1``; in
...
Design notes
------------
* Reuses :data:`PKS_GREEN`, :data:`COMPLETED_PILL`, :data:`ERROR_PILL` from
  :mod:`pks.util.cli_palette` — single brand-color policy (q29=b). Active
...
* TUI mode is excluded: the handler short-circuits when ``PKS_TUI_MODE=true``.
* ``pause()`` / ``resume()`` allow interactive prompts (sudo / sensitive
  guard) to take over the terminal cleanly. Coordination with
  :mod:`pks.util.wait_hints` is handled in :meth:`_start_live`.
"""
...
def _is_tui_mode() -> bool:
    return os.getenv("PKS_TUI_MODE", "").strip().lower() in ("1", "true", "yes")
...
def _row_for_record(
    ...
    *,
    ...
    than one agent is running in parallel (``PKS_PARALLEL > 1``); the static
    ...
    prefix.append(record.agent_name or "Agent", style=f"bold {PKS_GREEN}")
    if (
        record.agent_id
...
def _wait_hint_row(*, tick: int = 0) -> Text | None:
    ...
    Uses the same braille ``dots`` spinner frames and grey ``PKS`` pill as
    :class:`pks.repl.ui.startup_hints.StartupHints`, but drawn inside the compact
    ...
    """
    try:
        from pks.util.wait_hints import get_current_wait_hint_body
        from pks.util.hint_renderables import (
...
class CompactCLIHandler:
    ...
    def _start_live(self) -> None:
        with self._lock:
            ...
            try:
                from pks.util.wait_hints import set_compact_live_owner
    ...
    def _stop_live(self, *, release_ownership: bool = True) -> bool:
        ...
        try:
            ...
            pass
        if release_ownership and self._owns_wait_hints:
            try:
                from pks.util.wait_hints import set_compact_live_owner
