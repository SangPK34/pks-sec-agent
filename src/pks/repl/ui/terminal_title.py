...
Captures the previous title once before applying PKS branding and restores it on exit
...
PKS_DEFAULT_TERMINAL_WINDOW_TITLE = (
    "Tool Nhà Nghèo"
)
...
def _title_disabled() -> bool:
    return os.environ.get("PKS_DISABLE_TERMINAL_TITLE", "").lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def set_terminal_window_title(title: str | None = None) -> None:
    ...
    raw = PKS_DEFAULT_TERMINAL_WINDOW_TITLE if title is None else title
...
def _write_osc_raw(seq: str) -> None:
...
def set_terminal_activity(running: bool, label: str | None = None) -> None:
    """Reflect agent activity on the terminal tab (best effort), like Claude Code does.

    - Windows Terminal / ConEmu: OSC 9;4 progress — state 3 = indeterminate (an animated
      tab indicator) while running, state 0 = cleared when idle/done.
    - Every terminal: prefix the tab title with an emoji (🔎 running / 💥 idle) so the
      state is visible even without OSC 9;4 support.
    """
    if _title_disabled():
        return
    base = label or PKS_DEFAULT_TERMINAL_WINDOW_TITLE
    if running:
        _write_osc_raw("\033]9;4;3;0\007")     # indeterminate -> animated tab progress
        set_terminal_window_title(f"🔎 {base} — running…")
    else:
        _write_osc_raw("\033]9;4;0;0\007")     # clear progress
        set_terminal_window_title(f"💥 {base}")


def restore_terminal_window_title() -> None:
    """Restore the title captured before PKS branding (safe to call multiple times)."""
    if _title_disabled():
        return
    # Never leave Windows Terminal's indeterminate tab animation stuck after
    # Ctrl+C, an exception, or normal process exit.
    _write_osc_raw("\033]9;4;0;0\007")
