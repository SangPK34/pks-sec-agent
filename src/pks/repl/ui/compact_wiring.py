...
* :func:`is_compact_enabled` — single source of truth for the compact toggle
  (``PKS_COMPACT_REPL`` env var, default ``true`` per q3=b).
...
Excluded: TUI mode (``PKS_TUI_MODE=1``) is detected by
...
# ``/env PKS_COMPACT_REPL=...``) are intentionally ignored — the wiring of
# ``OUTPUT`` subscribers is fixed at startup, so the gate must match. Matches
# the documented behavior: "Restart PKS for the change to take effect."
...
def is_compact_enabled() -> bool:
    ...
    Honours ``PKS_COMPACT_REPL`` (``1``/``true``/``yes``/``on`` to enable,
    anything else to disable) read **once** at first invocation. Defaults to
    ``True``. Always ``False`` when ``PKS_TUI_MODE`` is set — the TUI owns its
    ...
    with _compact_cache_lock:
        if _compact_enabled_cached is not None:
            return _compact_enabled_cached
        if os.getenv("PKS_TUI_MODE", "").strip().lower() in ("1", "true", "yes"):
            _compact_enabled_cached = False
        else:
            raw = os.getenv("PKS_COMPACT_REPL", "1").strip().lower()
...
def install_compact_ui() -> None:
    ...
    with _install_lock:
        ...
        from pks.repl.ui.compact_renderer import install_compact_handler
        from pks.repl.ui.error_sink import install_error_sink
