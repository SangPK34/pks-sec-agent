...
def update_toolbar_in_background():
    ...
    try:
        ...
        # Try to get context usage from COST_TRACKER
        context_usage = 0.0
        try:
            from pks.util.pricing import COST_TRACKER, get_model_input_tokens
            ...
            if max_tokens > 0:
                context_usage = inp / max_tokens
        except Exception:
            try:
                context_usage = float(os.getenv('PKS_CONTEXT_USAGE', '0.0'))
            except Exception:
                pass
        ...
        toolbar_cache['html'] = HTML(
            ...
            f"<ansiyellow>$Limit:</ansiyellow> <ansiblue>${os.getenv('PKS_PRICE_LIMIT', 'inf')}</ansiblue>"
        )
