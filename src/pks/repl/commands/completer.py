"""
Command completer for PKS REPL.
...
from pks.repl.commands.base import (
    COMMANDS,
    COMMAND_ALIASES
)
...
class FuzzyCommandCompleter(Completer):
    ...
    def get_command_descriptions(self):
        ...
        if COMMAND_DESCRIPTIONS_CACHE is None:
            from pks.repl.commands import _ensure_all_commands_loaded
    ...
    def _resume_arg_completions(self, current_word: str):
        ...
        try:
            from pks.repl.session_resume import (
        ...
        for label, desc in (
            ...
            if not cw or label.startswith(cw):
                yield Completion(
                    ...
                    display=HTML(
                        f"<span style='color:#ff3355'><b>{html_escape(label)}</b></span> "
    ...
    def get_flush_agent_nonempty_suggestions(
        ...
        *,
        ...
        try:
            from pks.repl.commands.flush import ordered_nonempty_flush_agent_labels_repl
    ...
    def _resolved_env_set_target_is_ctf_name(words: List[str]) -> bool:
        ...
        try:
            from pks.repl.commands.env_catalog import ENV_VARS
            from pks.repl.commands.env_catalog_validate import resolve_catalog_spec
        except (ImportError, AttributeError):
            return False
        spec = words[2].strip()
        if not spec:
    ...
    def get_ctf_name_suggestions(self, current_word: str) -> List[Completion]:
        """Suggest PKSBench CTF ids for ``/env set CTF_NAME …`` (same pool as validation)."""
        from pks.repl.commands.env_catalog_validate import get_pksbench_ctf_names_for_completion_cached

        suggestions: List[Completion] = []
        names = list(get_pksbench_ctf_names_for_completion_cached())
        if not names:
            return suggestions
    ...
    def get_parallel_config_suggestions(self, current_word: str) -> list[Completion]:
        ...
        try:
            from pks.repl.commands._parallel_monolith import PARALLEL_CONFIGS
    ...
    def get_mcp_server_suggestions(self, current_word: str) -> List[Completion]:
        ...
        try:
            ...
            from pks.repl.commands.mcp import _GLOBAL_MCP_SERVERS
    ...
    def _refresh_virtualization_completion_cache(self) -> None:
        ...
        from pks.repl.commands._virtualization_monolith import DEFAULT_IMAGES, DockerManager
            ...
            for cid in self._virt_comp_containers:
                if cid.lower().startswith(cw):
                    yield Completion(
                        ...
                        start_position=-len(current_word),
                        display=HTML(
                            f'<span style="color:#ff3355"><b>{html_escape(cid)}</b></span> '
                            "<span style='color:#b8d4c9'>container</span>"
                        ),
                        style="fg:#ff3355 bold",
                    )
        elif subcommand == "run":
            for name in self._virt_comp_images:
                if name.lower().startswith(cw):
                    yield Completion(
                        ...
                        start_position=-len(current_word),
                        display=HTML(
                            f'<span style="color:#ff3355"><b>{html_escape(name)}</b></span> '
