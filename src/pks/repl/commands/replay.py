...
class ReplayCommand(Command):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="/replay",
            description="Replay a JSONL conversation (like pks-replay)",
            aliases=[],
        )
    ...
    def handle(self, args: Optional[List[str]] = None) -> bool:  # noqa: D401
        ...
        if args and len(args) == 1 and args[0].lower() in {"stop", "cancel"}:
            if os.getenv("PKS_TUI_MODE") == "true":
                try:
                    from pks.tui.routing.output_router import get_current_terminal_context, get_terminal_output
        ...
        try:
            ...
            if os.getenv("PKS_TUI_MODE") == "true":
                ...
                try:
                    from pks.tui.routing.output_router import (
                ...
                try:
                    ...
                    try:
                        from pks.tui.core.terminal_widget_registry import get_terminal_widget
...
async def _async_live_replay(file_path: str, delay: float, term_id: Optional[str], term_num: Optional[int]) -> None:
    ...
    from tools import replay as replay_tool
    from pks.sdk.agents.run_to_jsonl import get_token_stats, load_history_from_jsonl
    from pks.tui.routing.output_router import set_terminal_context, get_terminal_output
    from pks.tui.display.integration import display_agent_messages, display_tool_output
    ...
    for i, message in enumerate(messages):
        try:
            ...
            if role == "user":
                ...
                if out and content:
                    out.write(f"{replay_tool.color('PKS> ', fg='cyan')}{content}")
            ...
            if os.getenv("PKS_DEBUG") == "1" and out:
