"""REPL /api: read or write ``OPENAI_API_KEY`` in .env."""
...
from pks.repl.ui.startup_hints import mask_key_for_hint
...
class ApiCommand(Command):
    def __init__(self):
        super().__init__(
            name="/api",
            description="Show or set OPENAI_API_KEY in .env",
            aliases=["/apikey"],
        )

        self.add_subcommand("show", "Masked OPENAI_API_KEY (.env, else env)", self.handle_show)
        self.add_subcommand("set", "Write OPENAI_API_KEY to .env + os.environ", self.handle_set)
    ...
    def handle_show(self, args: Optional[List[str]] = None) -> bool:
        try:
            ...
            if current_key:
                ...
                console.print(
                    Panel(
                        f"OPENAI_API_KEY (masked): [bold {_PKS_GREEN}]{masked_key}[/bold {_PKS_GREEN}]{source_note}",
                ...
                console.print(
                    Panel(
                        f"[yellow]OPENAI_API_KEY is not set in .env or the environment.[/yellow]\n"
                        f"[{_GREY_SECONDARY}]Use [bold {_PKS_GREEN}]/api set <key>[/bold {_PKS_GREEN}] "
                        f"or [bold {_PKS_GREEN}]/api <key>[/bold {_PKS_GREEN}].[/]",
                        border_style="yellow",
                        title="OPENAI_API_KEY",
                    )
                )
    ...
    def handle_set(self, args: Optional[List[str]] = None) -> bool:
        if not args or not args[0]:
            ...
            console.print(f"Usage: [bold {_PKS_GREEN}]/api set <key>[/bold {_PKS_GREEN}]   or   [bold {_PKS_GREEN}]/api <key>[/bold {_PKS_GREEN}]")
            return False
        ...
        try:
            ...
            if success:
                ...
                console.print(
                    Panel(
                        f"OPENAI_API_KEY updated (masked): [bold {_PKS_GREEN}]{masked_key}[/bold {_PKS_GREEN}]\n"
    ...
    def _update_env_file(self, env_file_path: str, new_api_key: str) -> bool:
        try:
            ...
            key_pattern = r'^(OPENAI_API_KEY\s*=\s*)["\']?[^"\']*["\']?'
            new_line = f'OPENAI_API_KEY="{new_api_key}"'
    ...
    def _update_sidebar_keys(self) -> None:
        def _refresh(app) -> bool:
            ...
            if not sb:
                return False
            from pks.tui.components.sidebar import RefreshKeysMessage
        ...
        try:
            ...
            if _refresh(PKSTerminal._instance):
                return
        except Exception:
            pass
        try:
