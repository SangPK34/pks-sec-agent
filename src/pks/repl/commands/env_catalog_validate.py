...
def is_model_catalog_var(var_name: str) -> bool:
    ...
    if var_name in ("PKS_MODEL", "CTF_MODEL"):
        return True
...
def get_pksbench_ctf_names_for_completion_cached() -> tuple[str, ...]:
    """Unique canonical CTF names from PKSBench, sorted case-insensitively (REPL completer + validation)."""
    try:
        import pks.pksbench as cb_mod
...
def _load_pksbench_ctf_names_lowercase() -> Optional[Set[str]]:
    """Return lowercase CTF ids from PKSBench config, or None if unavailable."""
    names = get_pksbench_ctf_names_for_completion_cached()
    if not names:
        return None
    return {n.lower() for n in names}


def _first_pksbench_ctf_name_canonical() -> Optional[str]:
    """First CTF ``name`` string from PKSBench config (for tests / samples)."""
    names = get_pksbench_ctf_names_for_completion_cached()
...
def _validate_ctf_name(value: str) -> Optional[str]:
    ...
    try:
        ...
        return None
    known = _load_pksbench_ctf_names_lowercase()
    if not known:
        return None
    if v.lower() not in known:
        return (
            f"No PKSBench CTF named '{v}'. Use an id from the PKSBench catalog "
            "(same names as in pks.pksbench ctf_configs.jsonl)."
        )
    return None


def _validate_model_value(value: str) -> Optional[str]:
    """Accept any non-empty exact model ID served by the configured endpoint."""
    v = (value or "").strip()
    if not v:
...
def validate_catalog_value(var_name: str, value: str, var_info: EnvVarEntry) -> Optional[str]:
    ...
    # Ports
    if var_name.endswith("_PORT") or var_name in ("PKS_AUTH_DEVICE_PORT",):
        return _validate_int_range(value, 1, 65535, "Port")

    if var_name == "PKS_PARALLEL":
        return _validate_int_range(value, 1, 20, "PKS_PARALLEL")

    if var_name == "PKS_DEBUG":
        return _validate_int_range(value, 0, 2, "PKS_DEBUG")

    if var_name == "PKS_TOOL_OUTPUT_MAX":
        return _validate_int_range(value, 800, 10_000_000, "PKS_TOOL_OUTPUT_MAX")

    if var_name == "PKS_MODEL_MAX_INPUT_TOKENS":
        return _validate_int_range(
            value, 8_000, 10_000_000, "PKS_MODEL_MAX_INPUT_TOKENS"
        )

    if var_name == "PKS_REASONING_EFFORT":
        if value.strip().lower() not in {
            ...
            return "PKS_REASONING_EFFORT must be off, low, medium, high, xhigh, or max"
        return None

    if var_name == "PKS_COMPACT_REPL":
        return _validate_bool_string(value)

    if var_name in ("PKS_TEMPERATURE",):
        return _validate_float_range(value, 0.0, 2.0, "PKS_TEMPERATURE")

    if var_name in ("PKS_TOP_P",):
        return _validate_float_range(value, 0.0, 1.0, "PKS_TOP_P")

    if var_name in ("PKS_MAX_TURNS", "PKS_MAX_INTERACTIONS"):
        return _validate_inf_or_float(value, var_name)

    if var_name in CATALOG_BOOL_VAR_NAMES:
        return _validate_bool_string(value)

    if var_name == "PKS_PARALLEL_EXTERNAL_TIMEOUT":
        s = (value or "").strip()
        try:
            x = float(s)
        except ValueError:
            return f"PKS_PARALLEL_EXTERNAL_TIMEOUT must be a number, got '{value}'."
        if x <= 0:
            return "PKS_PARALLEL_EXTERNAL_TIMEOUT must be positive."
        return None

    if var_name == "PKS_TASK_RESET_PENDING":
        return _validate_int_range(value, 0, 1, "PKS_TASK_RESET_PENDING")

    if var_name == "PKS_MERGE_SUMMARIZE_PER_WORKER":
        return _validate_int_range(value, 0, 1, "PKS_MERGE_SUMMARIZE_PER_WORKER")

    if var_name == "PKS_MERGE_SUMMARIZE_MIN_MESSAGES":
        return _validate_int_range(value, 1, 10_000_000, "PKS_MERGE_SUMMARIZE_MIN_MESSAGES")
...
def sample_valid_test_value(var_name: str, var_info: EnvVarEntry) -> str:
    ...
    if var_name == "CTF_NAME":
        sample = _first_pksbench_ctf_name_canonical()
        if sample:
            return sample
    if var_name.endswith("_IP") and "API" not in var_name:
        return "10.11.12.13"
    ...
    if var_name.endswith("_PORT") or var_name == "PKS_AUTH_DEVICE_PORT":
        return "9090"
    if var_name == "PKS_PARALLEL":
        return "2"
    if var_name == "PKS_DEBUG":
        return "1"
    if var_name == "PKS_TEMPERATURE":
        return "0.5"
    if var_name == "PKS_TOP_P":
        return "0.9"
    if var_name in ("PKS_MAX_TURNS", "PKS_MAX_INTERACTIONS"):
        return "inf"
    if var_name in CATALOG_BOOL_VAR_NAMES:
