...
def _validate_ctf_name(value: str) -> Optional[str]:
    ...
    if v.lower() not in known:
        return (
            ...
            "(same names as in pks.caibench ctf_configs.jsonl)."
        )
    return None
...
def _validate_model_value(value: str) -> Optional[str]:
    ...
    from pks.repl.commands.model import get_predefined_model_names, load_all_available_models
...
def validate_catalog_value(var_name: str, value: str, var_info: EnvVarEntry) -> Optional[str]:
    ...
    if var_name == "PKS_PRICE_LIMIT":
        s = (value or "").strip().lower()
        if s == "inf":
            return None
        return _validate_float_range(value, 0.0, 1e9, "PKS_PRICE_LIMIT")
