"""
Temperature and Top-P commands for PKS REPL.
...
from pks.sdk.agents.model_settings import get_default_temperature, get_default_top_p

console = Console()


def _pks_panel(body: str, *, title: str, border_style: str = _PKS_GREEN) -> Panel:
...
def _repl_resolved_temperature_for_display() -> float:
    """REPL temperature: active agent's resolved value, else PKS_TEMPERATURE."""
    try:
...
def _repl_resolved_top_p_for_display() -> float:
    """Top_p shown in REPL: active agent's resolved settings, else PKS_TOP_P."""
    try:
        from pks.sdk.agents.simple_agent_manager import AGENT_MANAGER
...
class TemperatureCommand(Command):
    ...
    def handle(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args:
            ...
            if os.getenv("PKS_TUI_MODE") != "true":
                panel_body += (
                    f"\n\n[dim]REPL: updates [bold {z}]PKS_TEMPERATURE[/bold {z}] and the active "
                    "agent's model_settings; some providers may clamp or ignore.[/dim]"
                )
            console.print(_pks_panel(panel_body, title="Temperature"))
            console.print(f"\n[bold {z}]Usage:[/bold {z}]")
                ...
                if app and hasattr(app, 'session_manager'):
                    ...
                    if terminal_number is not None:
                        ...
                        change_message = (
                            ...
                            f"set [bold {z}]PKS_TEMPERATURE[/bold {z}] globally.[/white]\n"
                            f"[bold {z}]{new_temp:.1f}[/bold {z}] — [white]{desc}[/white]"
                        )
                        # Set global temperature as fallback
                        os.environ["PKS_TEMPERATURE"] = str(new_temp)
        ...
        console.print(_pks_panel(change_message, title="Temperature"))
...
class TopPCommand(Command):
    ...
    def handle(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args:
            ...
            if os.getenv("PKS_TUI_MODE") != "true":
                panel_body += (
                    f"\n\n[dim]REPL: updates [bold {z}]PKS_TOP_P[/bold {z}] and the active "
                    "agent's model_settings; some providers may clamp or ignore.[/dim]"
                )
            console.print(_pks_panel(panel_body, title="Top-P"))
            console.print(f"\n[bold {z}]Usage:[/bold {z}]")
        ...
        z = _PKS_GREEN
        os.environ["PKS_TOP_P"] = str(new_top_p)
        if os.getenv("PKS_TUI_MODE") != "true":
        ...
        console.print(_pks_panel(change_message, title="Top-P"))
