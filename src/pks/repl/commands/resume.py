"""
Resume and sessions commands for PKS REPL.

This module provides commands for managing and resuming sessions:
- /resume: Pick or resume a session (replaces former ``pks --resume`` CLI flags)
...
from pks.repl.session_resume import (
    ...
    find_newest_pks_jsonl_by_filename_prefix,
...
class ResumeCommand(Command):
    ...
    def handle(self, args: Optional[list[str]] = None) -> bool:
        ...
        - ``last``: most recent log with messages (session logs under ``~/.pks/logs``).
        ...
        - ``<token>`` (no slashes, not a file): substring match in session ``pks_*.jsonl`` files.
        """
        args = args or []
...
class SessionsCommand(Command):
    ...
    def _show_session_details(self, session_arg: str) -> bool:
        ...
        if session_arg.endswith(".jsonl") or "/" in session_arg or "\\" in session_arg:
            ...
            log_path = find_newest_pks_jsonl_by_filename_prefix(session_arg)
        ...
        _model, input_tokens, output_tokens, _active, _idle = get_session_stats(log_path)
        _kv("Tokens", f"{input_tokens:,} in / {output_tokens:,} out")
