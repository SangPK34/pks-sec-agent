...
from pks.sdk.agents.mcp import (
    MCPServer,
    MCPServerSse,
    MCPServerSseParams,
    MCPServerStdio,
...
from pks.sdk.agents.tool import FunctionTool

console = Console()

_MCP_TABLE_HEADER = f"bold {_PKS_GREEN}"
...
def _mcp_table(**kwargs: Any) -> Table:
    """Rich table with rounded corners and PKS palette border (aligned with help panels)."""
    defaults: Dict[str, Any] = {
        "box": box.ROUNDED,
        "border_style": _PKS_GREEN,
        "show_header": True,
...
class GlobalMCPUtil(MCPUtil):
    ...
    def to_function_tool(cls, tool, server_name: str) -> FunctionTool:
        """Convert an MCP tool to a PKS function tool using server name instead of object."""
        ...
        async def invoke_with_fresh_connection(config, context, input_json):
            ...
            from pks.sdk.agents.exceptions import AgentsException, ModelBehaviorError
            from pks.sdk.agents.mcp import MCPServerSse, MCPServerStdio
...
class MCPCommand(Command):
    ...
    def handle(self, args: Optional[List[str]] = None) -> bool:
        ...
        _mcp_emit_panel(
            ...
            f"[#9aa0a6]Supported commands are listed under[/] [bold {_PKS_GREEN}]/mcp help[/bold {_PKS_GREEN}]"
            f"[#9aa0a6].[/]",
            title="MCP",
    ...
    def handle_load(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args or len(args) < 2:
            _mcp_emit_panel(
                ...
                f"[bold {_PKS_GREEN}]/mcp load <url> <name>[/bold {_PKS_GREEN}]\n\n"
                "[white]SSE (legacy)[/white]\n"
                f"[bold {_PKS_GREEN}]/mcp load sse <url> <name>[/bold {_PKS_GREEN}]\n\n"
                "[white]stdio[/white]\n"
                f"[bold {_PKS_GREEN}]/mcp load stdio <name> <command>[/bold {_PKS_GREEN}] [dim][args…][/dim]",
                title="MCP — load",
                border_style=_MCP_PANEL_ERROR_BORDER,
            )
            return False
    ...
    def _load_sse_server(self, url: str, name: str) -> bool:
        ...
        if name in _GLOBAL_MCP_SERVERS:
            _mcp_emit_panel(
                f"[{_MCP_COL_MUTED}]Server[/] [bold {_PKS_GREEN}]{escape(name)}[/bold {_PKS_GREEN}] "
                f"[{_MCP_COL_MUTED}]is already loaded.[/]\n\n"
                f"[white]To reload, remove it first:[/white] [bold {_PKS_GREEN}]/mcp remove "
                f"{escape(name)}[/bold {_PKS_GREEN}]",
                title="MCP — load",
        ...
        console.print(
            f"[{_MCP_COL_MUTED}]Connecting to SSE[/] [bold {_PKS_GREEN}]{escape(url)}[/bold {_PKS_GREEN}]"
            f"[{_MCP_COL_MUTED}]…[/]"
        )
            ...
            try:
                ...
                with requests.get(endpoint, headers=headers, stream=True, timeout=5) as resp:
                    ...
                    if "text/event-stream" not in ct:
                        console.print(
                            f"[{_MCP_PANEL_WARN_BORDER}][PKS] Warning:[/] "
        ...
        try:
            ...
            console.print(
                f"[green][PKS] Connected to SSE server [/][bold #ff3355]{name}[/bold #ff3355]"
                f"[green] at [/][bold white]{url}[/bold white]"
            )
            console.print(
                f"[#9aa0a6][PKS] Available tools:[/] [bold #ff3355]{len(tools)}[/bold #ff3355]"
            )
            ...
            if tools:
                ...
                console.print(f"[#9aa0a6][PKS] Tools:[/] [white]{', '.join(tool_names)}[/white]")
    ...
    def _load_stdio_server(self, name: str, command: str, cmd_args: List[str]) -> bool:
        ...
        _mcp_emit_panel(
            f"[white]Starting stdio server[/white] [bold {_PKS_GREEN}]{escape(name)}[/bold {_PKS_GREEN}]\n"
    ...
    def handle_list(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not _GLOBAL_MCP_SERVERS:
            _mcp_emit_panel(
                ...
                f"[white]Load one with[/white] [bold {_PKS_GREEN}]/mcp load <url> <name>[/bold {_PKS_GREEN}] "
                f"[white]or[/white] [bold {_PKS_GREEN}]/mcp load stdio <name> <cmd>[/bold {_PKS_GREEN}] "
                f"[dim][args…][/dim]\n"
                f"[white]Full syntax:[/white] [bold {_PKS_GREEN}]/mcp help[/bold {_PKS_GREEN}]",
                title="MCP — list",
            )
            return True
    ...
    def handle_add(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args or len(args) < 2:
            _mcp_emit_panel(
                ...
                f"[bold {_PKS_GREEN}]/mcp add <server_name> <agent_name_or_number>[/bold {_PKS_GREEN}]",
                title="MCP — add",
                border_style=_MCP_PANEL_ERROR_BORDER,
            )
            return False
        ...
        if server_name not in _GLOBAL_MCP_SERVERS:
            _mcp_emit_panel(
                ...
                f"[bold {_PKS_GREEN}]/mcp list[/bold {_PKS_GREEN}][#9aa0a6].[/]",
                title="MCP — add",
                border_style=_MCP_PANEL_ERROR_BORDER,
            )
            return False
        ...
        try:
            ...
            try:
                ...
                _mcp_emit_panel(
                    ...
                    f"[white]Pick a name from[/white] [bold {_PKS_GREEN}]/agent list[/bold {_PKS_GREEN}] "
        ...
        _mcp_emit_panel(
            f"[{_MCP_COL_MUTED}]Adding tools from[/] [bold {_PKS_GREEN}]{escape(server_name)}[/bold {_PKS_GREEN}] "
            f"[{_MCP_COL_MUTED}]→[/] [bold {_PKS_GREEN}]{escape(agent_display_name)}[/bold {_PKS_GREEN}]"
            f"[{_MCP_COL_MUTED}] …[/]",
            title="MCP — add",
            padding=(0, 1),
        )
        ...
        try:
            ...
            async def validate_connection():
                try:
                    ...
                    tools = await server.list_tools()
                    console.print(
                        f"[green]✓ Reconnected to[/] [bold {_PKS_GREEN}]{escape(server_name)}[/bold {_PKS_GREEN}]"
                    )
                    return tools
    ...
    def handle_remove(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args:
            _mcp_emit_panel(
                ...
                f"[bold {_PKS_GREEN}]/mcp remove <server_name>[/bold {_PKS_GREEN}]",
                title="MCP — remove",
                border_style=_MCP_PANEL_ERROR_BORDER,
            )
            return False
        ...
        if server_name not in _GLOBAL_MCP_SERVERS:
            _mcp_emit_panel(
                f"[red bold]Unknown server[/red bold] [white]{escape(server_name)}[/white]\n\n"
                f"[#9aa0a6]Loaded servers:[/] [bold {_PKS_GREEN}]/mcp list[/bold {_PKS_GREEN}]",
                title="MCP — remove",
                border_style=_MCP_PANEL_ERROR_BORDER,
            )
            return False
    ...
    def handle_status(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not _GLOBAL_MCP_SERVERS:
            _mcp_emit_panel(
                ...
                f"[bold {_PKS_GREEN}]/mcp load …[/bold {_PKS_GREEN}] [#9aa0a6]then re-run[/] "
                f"[bold {_PKS_GREEN}]/mcp status[/bold {_PKS_GREEN}]",
                title="MCP — status",
            )
            return True
            ...
            try:
                ...
                try:
                    console.print(
                        ...
                        f"[bold {_PKS_GREEN}]{escape(name)}[/bold {_PKS_GREEN}]"
    ...
    def handle_tools(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args:
            _mcp_emit_panel(
                "[red bold]Missing server name[/red bold]\n\n"
                f"[bold {_PKS_GREEN}]/mcp tools <server_name>[/bold {_PKS_GREEN}]",
                title="MCP — tools",
                border_style=_MCP_PANEL_ERROR_BORDER,
            )
            return False
        ...
        if server_name not in _GLOBAL_MCP_SERVERS:
            _mcp_emit_panel(
                f"[red bold]Unknown server[/red bold] [white]{escape(server_name)}[/white]\n\n"
                f"[#9aa0a6]See[/] [bold {_PKS_GREEN}]/mcp list[/bold {_PKS_GREEN}]",
                title="MCP — tools",
                border_style=_MCP_PANEL_ERROR_BORDER,
            )
            return False
        ...
        try:
            ...
            if not tools:
                _mcp_emit_panel(
                    f"[{_MCP_COL_MUTED}]Server[/] [bold {_PKS_GREEN}]{escape(server_name)}[/bold {_PKS_GREEN}] "
    ...
    def handle_associations(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not _AGENT_MCP_ASSOCIATIONS:
            _mcp_emit_panel(
                ...
                f"[white]After[/white] [bold {_PKS_GREEN}]/mcp load …[/bold {_PKS_GREEN}][white], attach tools with[/white]\n"
                f"[bold {_PKS_GREEN}]/mcp add <server> <agent>[/bold {_PKS_GREEN}]",
    ...
    def handle_test(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args:
            _mcp_emit_panel(
                "[red bold]Missing server name[/red bold]\n\n"
                f"[bold {_PKS_GREEN}]/mcp test <server_name>[/bold {_PKS_GREEN}]",
                title="MCP — test",
                border_style=_MCP_PANEL_ERROR_BORDER,
            )
            return False
        ...
        _mcp_emit_panel(
            ...
            f"[bold {_PKS_GREEN}]{escape(server_name)}[/bold {_PKS_GREEN}][{_MCP_COL_MUTED}]…[/]",
            title="MCP — test",
            padding=(0, 1),
        )
        ...
        try:
            ...
            async def test_server():
                ...
                tools = await server.list_tools()
                console.print(
                    f"[green]✓[/] [{_MCP_COL_MUTED}]Found[/] [bold {_PKS_GREEN}]{len(tools)}[/bold {_PKS_GREEN}]"
                )
                ...
                if tools:
                    ...
                    console.print(
                        ...
                        f"[bold {_PKS_GREEN}]{escape(test_tool.name)}[/bold {_PKS_GREEN}]"
                        f"[{_MCP_COL_MUTED}]…[/]"
                    )
            ...
            _mcp_emit_panel(
                ...
                f"[bold {_PKS_GREEN}]{escape(server_name)}[/bold {_PKS_GREEN}]",
                title="MCP — test",
                border_style=_PKS_GREEN,
                padding=(0, 1),
            )
