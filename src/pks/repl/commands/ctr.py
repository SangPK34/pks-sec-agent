...
def _ensure_ctr_imports():
    ...
    if ctr_experiment is None:
        from pks.ctr import experiment as _ctr_experiment
        ctr_experiment = _ctr_experiment
    if visualize_baseline_results is None:
        from pks.ctr.visualization import visualize_baseline_results as _vis
        visualize_baseline_results = _vis
    if get_ctr_output_base_dir is None:
        from pks.ctr.paths import get_ctr_output_base_dir as _get_dir
...
class CTRCommand(Command):
    ...
    This command serves as the primary interface between PKS's REPL and the CTR
    ...
    def run_full_analysis(self, log_path: Optional[str] = None) -> bool:
        ...
        """
        console.print(f"[bold {_PKS_GREEN}]Running Cut The Rope analysis...[/bold {_PKS_GREEN}]")
        ...
        # If still no log path, try to find recent logs in the standard PKS logs directory
        # This is the final fallback, using the most recently modified log file
        if not log_path:
            import glob
            log_dir = os.path.expanduser("~/.pks/logs")
        ...
        def _focus_ctr_tab_and_load(run_dir: Optional[str]) -> None:
            ...
            TUI Integration
            This function handles the interaction with PKS's Terminal User Interface.
            ...
            app = getattr(PKSTerminal, "_current_app", None)
            if not app:
        ...
        in_tui = os.getenv("PKS_TUI_MODE") == "true"
        loop_running = False
        try:
            ...
            def _run_and_report():
                try:
                    ...
                    def _notify_success():
                        if results_dir:
                            console.print(
                                f"[bold {_PKS_GREEN}]✓ CTR analysis complete. Results saved to:[/bold {_PKS_GREEN}] {results_dir}"
                            )
                            console.print(
                    ...
                    # Run UI update; rely on PKSTerminal app
                    _notify_success()
    ...
    def handle_graph(self, args: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            if os.path.exists(graph_llm_png):
                ...
                console.print(
                    f"[bold {_PKS_GREEN}]✓ LLM attack graph visualization found:[/bold {_PKS_GREEN}] {graph_llm_png}"
                )
            elif os.path.exists(graph_individual_png):
                graph_to_open = graph_individual_png
                console.print(
                    f"[bold {_PKS_GREEN}]✓ Individual attack graph visualization found:[/bold {_PKS_GREEN}] {graph_individual_png}"
                )
            elif os.path.exists(graph_cleaned_png):
                graph_to_open = graph_cleaned_png
                console.print(
                    f"[bold {_PKS_GREEN}]✓ Cleaned attack graph visualization found:[/bold {_PKS_GREEN}] {graph_cleaned_png}"
                )
            ...
            if os.path.exists(graph_info_file):
                ...
                if match:
                    try:
                        ...
                        console.print(f"\n[bold {_PKS_GREEN}]═══ Attack Graph Structure ═══[/bold {_PKS_GREEN}]")
                        ...
                        node_table = Table(
                            show_header=True, header_style=f"bold {_PKS_GREEN}"
                        )
    ...
    def handle_list(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not runs:
            ...
            return False
        table = Table(show_header=True, header_style=f"bold {_PKS_GREEN}")
    ...
    def handle_use(self, args: Optional[List[str]] = None) -> bool:
        ...
        console.print(f"[bold {_PKS_GREEN}]Active CTR run set to:[/bold {_PKS_GREEN}] {selected}")
        return True
    ...
    def handle_open(self, args: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            console.print(f"[bold {_PKS_GREEN}]Opened CTR runs folder:[/bold {_PKS_GREEN}] {parent}")
            return True
        except Exception as e:
