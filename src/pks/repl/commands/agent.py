"""
Agent "command" for PKS CLI abstraction
...
# Local imports
from pks.agents import get_agent_module, get_available_agents
from pks.config import DEFAULT_AGENT_TYPE
from pks.repl.ui.agent_notices import (
...
from pks.repl.commands.env_catalog import HELP_REFERENCE_MATCH_TABLE_KWARGS
from pks.repl.ui.banner import _PKS_GREEN, _quick_guide_subpanel_title
from pks.sdk.agents import Agent
from pks.util import visualize_agent_graph
from pks.util.cli_palette import PKS_GREEN, GREY_TEXT

console = Console()

# Legacy ``PKS_AGENT_TYPE`` values (old catalog / .env).
_PKS_AGENT_TYPE_ENV_ALIASES = {
    "one_tool": "ctf_agent",
    "one_tool_agent": "ctf_agent",
    "selection_agent": "root_agent",
}

# Agents that default to handoff routing (``PKS_AGENT_ROUTE_MODE=auto``).
_AUTO_ROUTE_AGENT_TYPES = frozenset({"root_agent", "orchestration_agent"})
...
class AgentCommand(Command):
    ...
    def _get_model_display(self, agent_name: str, agent: Agent) -> str:
        ...
        env_var_name = f"PKS_{agent_name.upper()}_MODEL"
    ...
    def handle_list(self, args: Optional[List[str]] = None) -> bool:  # pylint: disable=unused-argument # noqa: E501
        ...
        agents_table.add_column("Name", style=f"bold {PKS_GREEN}")
        ...
        if parallel_patterns:
            ...
            patterns_table.add_column("Name", style=f"bold {PKS_GREEN}")
    ...
    def handle_select(self, args: Optional[List[str]] = None) -> bool:  # pylint: disable=too-many-branches,line-too-long # noqa: E501
        ...
        agent_id = _PKS_AGENT_TYPE_ENV_ALIASES.get(args[0], args[0])
                ...
                if pattern_type_str == "parallel":
                    ...
                    if hasattr(pattern, "configs") and pattern.configs is not None:
                        ...
                        try:
                            ...
                            os.environ["PKS_PATTERN_DESCRIPTION"] = pattern.description or ""
                    ...
                    if hasattr(pattern, "entry_agent") and pattern.entry_agent:
                        ...
                        if agent_key:
                            os.environ["PKS_AGENT_TYPE"] = agent_key
                            os.environ["PKS_AGENT_ROUTE_MODE"] = (
        ...
        # IMPORTANT: Don't set PKS_AGENT_TYPE for parallel patterns as they don't change the current agent
        if "selected_agent_key" in locals() and not (
            ...
            os.environ["PKS_AGENT_TYPE"] = selected_agent_key
        ...
        from pks.repl.commands.parallel import PARALLEL_CONFIGS, PARALLEL_AGENT_INSTANCES
        from pks.sdk.agents.simple_agent_manager import AGENT_MANAGER
        ...
        if PARALLEL_CONFIGS:
            ...
            if (
                ...
                prev_agent_type = os.getenv("PKS_AGENT_TYPE", "")
        ...
        console.print(
            f"[bold {PKS_GREEN}]Switched to agent:[/bold {PKS_GREEN}] [white]{final_agent_name}[/white]",
            end="",
        )
        console.print(
        ...
        console.print(f"\n[bold {PKS_GREEN}]System Prompt:[/bold {PKS_GREEN}]")
    ...
    def handle_info(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args:
            console.print(
                Panel(
                    ...
                    f"[{GREY_TEXT}]Usage:[/] [bold {PKS_GREEN}]/agent info <agent_key|number>[/bold {PKS_GREEN}]",
        ...
        info_table.add_column("Property", style=f"bold {PKS_GREEN}", width=22)
        ...
        def _styled_instruction_text(raw: str) -> Text:
            ...
            for part in parts:
                ...
                if part.startswith("**") and part.endswith("**") and len(part) >= 4:
                    out.append(part[2:-2], style=f"bold {PKS_GREEN}")
                else:
    ...
    def handle_current(self, args: Optional[List[str]] = None) -> bool:
        ...
        if parallel_enabled and has_parallel_configs:
            ...
            parallel_content = []
            parallel_content.append(f"[bold {PKS_GREEN}]Active Pattern:[/bold {PKS_GREEN}] {pattern_name}")
            ...
            for idx, config in enumerate(PARALLEL_CONFIGS):
                ...
                config_info = ""
                if config.model:
                    config_info = f" [{_resolve_model_name(config.model)}]"
            ...
            tui_mode = os.getenv("PKS_TUI_MODE") == "true"
            ...
            if tui_mode:
                try:
                    ...
                    app = getattr(PKSTerminal, '_instance', None)
                        ...
                        if session_manager and hasattr(session_manager, 'terminal_runners') and session_manager.terminal_runners:
                            ...
                            for term_num in sorted(session_manager.terminal_runners.keys()):
                                ...
                                if runner and hasattr(runner, 'agent') and runner.agent:
                                    ...
                                    content = []
                                    content.append(f"[bold {PKS_GREEN}]Agent:[/bold {PKS_GREEN}] {agent_name}")
            ...
            if not multi_terminal_shown:
                ...
                current_agent_key = (
                    os.getenv("PKS_AGENT_TYPE", DEFAULT_AGENT_TYPE)
                    or DEFAULT_AGENT_TYPE
                ).strip()
                alt_key = _PKS_AGENT_TYPE_ENV_ALIASES.get(current_agent_key)
                ...
                main_content = []
                main_content.append(f"[bold {PKS_GREEN}]Active Agent:[/bold {PKS_GREEN}] {agent_name}")
                ...
                from pks.sdk.agents.model_settings import DEFAULT_TEMPERATURE, get_default_top_p
                ...
                if hasattr(current_agent, "model") and hasattr(current_agent.model, "temperature"):
                    ...
                    temperature = float(os.getenv("PKS_TEMPERATURE", str(DEFAULT_TEMPERATURE)))
        ...
        console.print(
            f"• [bold {PKS_GREEN}]/agent list[/bold {PKS_GREEN}] - Show all available agents and patterns"
        )
        console.print(
            f"• [bold {PKS_GREEN}]/agent select <name>[/bold {PKS_GREEN}] - Switch to a different agent or pattern"
        )
        console.print(
            f"• [bold {PKS_GREEN}]/agent info <name>[/bold {PKS_GREEN}] - Show detailed agent information"
        )
        console.print(
            f"• [bold {PKS_GREEN}]/agent new[/bold {PKS_GREEN}] - Create a new agent interactively"
        )
        if parallel_enabled:
            console.print(
                f"• [bold {PKS_GREEN}]/parallel[/bold {PKS_GREEN}] - Manage parallel agent configuration"
            )
        else:
            console.print(
                f"• [bold {PKS_GREEN}]/parallel add[/bold {PKS_GREEN}] - Configure parallel agents"
            )
    ...
    def handle_new(self, args: Optional[List[str]] = None) -> bool:
        ...
        from pks.tui.pks_terminal import is_tui_mode
        ...
        if is_tui_mode():
            ...
            from pks.agents.agent_builder import AgentBuilder
...
def _sync_tui_agent_selection(agent_name: str) -> None:
    ...
    active_terminal_env = os.getenv("PKS_ACTIVE_COMMAND_TERMINAL", "").strip()
