"""
Virtualization command for PKS cli.
...
# PKS REPL accent (aligned with banner / completer menu: #ff3355)
_VIRT_ACCENT_BOLD = "bold #ff3355"
_VIRT_BORDER = "#ff3355"
_VIRT_OK = "green"
_VIRT_WARN = "yellow"

# Default Docker images for PKS
DEFAULT_IMAGES = {
...
class DockerManager:
    ...
    def run_container(image_name: str, container_name: str = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            workspace_dir = os.getenv("PKS_WORKSPACE_DIR", os.getcwd())
            ...
            elif image_name == "pks-container":
                # For PKS container, add any specific flags needed
    ...
    def set_active_container(container_id: str) -> None:
        """Set the active container for PKS.
        ...
        os.environ["PKS_ACTIVE_CONTAINER"] = container_id
...
class VirtualizationCommand(Command):
        ...
        if not docker_manager.is_docker_installed():
            console.print(
                Panel(
                    ...
                    title="[bold #ff3355]Docker Not Found[/bold #ff3355]",
        ...
        if not docker_manager.is_docker_running():
            console.print(
                Panel(
                    ...
                    title="[bold #ff3355]Docker Not Running[/bold #ff3355]",
                    border_style=_VIRT_BORDER,
                )
            )
            return True
    ...
    def show_active_environment(self, active_container: str) -> None:
        ...
        if active_container:
            ...
            elif "pks" in image_name.lower():
                icon = "⭐"  # Star for PKS container

            title = f"[bold #ff3355]Active Environment: {icon} {env_name}[/bold #ff3355]"
            border_style = _VIRT_BORDER
        else:
            env_name = "Host System"
            env_info = "Commands are executing directly on the host where PKS is running"
            title = "[bold #ff3355]Active Environment: 💻 Host System[/bold #ff3355]"
            border_style = _VIRT_BORDER

        # Show the panel with environment information
        panel_content = [
            f"[bold #ff3355]Current active environment:[/bold #ff3355] [#ff3355]{env_name}[/]",
            f"[#ff3355]Details:[/] [dim]{env_info}[/dim]",
            "",
            f"[bold #ff3355]Select an environment from below to switch:[/bold #ff3355]",
        ]
    ...
    def show_useful_images_table(self, show_all: bool = False) -> None:
        ...
        image_table.add_column("Environment", style="bold #ff3355", width=30)
        ...
        host_status = (
            "[bold #ff3355]CURRENT ENVIRONMENT[/bold #ff3355]"
            if is_host_active
            else "[dim]Available[/dim]"
        )
        display_host = (
            "💻 [bold #ff3355]Host System ⭐ ACTIVE[/bold #ff3355]"
            if is_host_active
            else "💻 [dim]Host System[/dim]"
        )

        image_table.add_row(
            "[bold #ff3355]host[/bold #ff3355]",
            display_host,
            ...
            if image_name in DEFAULT_IMAGES:
                ...
                for container in self.cached_containers:
                    if container.get("Image", "") == image_name:
                        ...
                        if "up" in container_status:
                            status = f"[bold #ff3355]RUNNING[/bold #ff3355] (ID: {container_id})"
                        elif "exited" in container_status or "created" in container_status:
                            status = f"[{_VIRT_WARN}]STOPPED[/] (ID: {container_id})"
                        else:
                            status = f"[#ff3355]CONTAINER[/] (ID: {container_id})"
                        break
                ...
                if not container_id:
                    if image_name in available_images:
                        ...
                        status = f"[#ff3355]Available[/] (ID: {image_id_short})"
                ...
                if image_name == active_image:
                    display_name = f"[bold #ff3355]{icon} {image_name} ⭐ ACTIVE[/bold #ff3355]"
                    status = "[bold #ff3355]CURRENT ENVIRONMENT[/bold #ff3355]"

                # Add row to table
                image_table.add_row(
                    f"[bold #ff3355]{image_id}[/bold #ff3355]", display_name, info["description"], status
                )
        ...
        # If not showing all, add a note about additional images
            ...
            console.print("\n[dim #ff3355]Only showing essential environments.[/]")
    ...
    def _show_all_images_by_category(self, available_images, active_image):
        ...
        console.print("\n[bold #ff3355]All Available Security Environments:[/bold #ff3355]\n")
            ...
            for name, info in images:
                ...
                if not container_id:
                    if name in available_images:
                        ...
                        status = f"[#ff3355]Available[/] (ID: {image_id})"

                # Add special icon for different category
                icon = "🔷"  # Default icon
                if category == "PKS Official":
                    icon = "⭐"
                ...
                if name == active_image:
                    image_name_display = f"[bold #ff3355]{icon} {name} ⭐ ACTIVE[/bold #ff3355]"
                    description_display = f"[#ff3355]{info['description']}[/]"
                    status = "[bold #ff3355]CURRENT ENVIRONMENT[/bold #ff3355]"

                image_table.add_row(
                    f"[bold #ff3355]{image_id}[/bold #ff3355]", image_name_display, description_display, status
                )
    ...
    def show_usage_info(self) -> None:
        ...
        console.print(f"\n[{cmd}]Important:[/]")
        console.print(
            "[dim #ff3355]When a container is active, all shell commands will execute inside that "
    ...
    def handle_list_subcommand(self, args: Optional[List[str]] = None) -> bool:
        ...
        table.add_column("Status", style="#ff3355")
        table.add_column("Names", style="dim #ff3355")

        active = os.getenv("PKS_ACTIVE_CONTAINER", "")
        for container in self.cached_containers:
            cid = container.get("ID") or ""
            cid_short = cid[:12] if cid else ""
            row_id = (
                f"[bold #ff3355]{cid_short}[/bold #ff3355]"
    ...
    def handle_activate_image(self, image_identifier: str) -> bool:
        ...
        if image_identifier.lower() in ["host", "system", "none"]:
            if "PKS_ACTIVE_CONTAINER" in os.environ:
                # Clear the active container
                previous = os.environ.pop("PKS_ACTIVE_CONTAINER")
                console.print(
        ...
        if image_name == "kalilinux/kali-rolling":
            ...
            for img_name, img_info in DEFAULT_IMAGES.items():
                if img_name == image_name and "id" in img_info:
                    container_name = f"pks-{img_info['id']}"
                    break
        ...
        if not success:
            ...
            #     )
                ...
                # os.environ["PKS_ACTIVE_CONTAINER"] = dummy_id
    ...
    def _handle_parrotsec(self) -> bool:
        ...
        fixed_image_name = "pks-fixed-parrotsec"
        container_name = "pks-parrotsec"
