...
ADDITIONAL_VARS = {
    ...
    'PKS_TOOL_OUTPUT_MAX': {
        ...
        'description': 'Maximum characters stored per tool result in LLM memory',
        'default': '20000',
    },
    'PKS_MODEL_MAX_INPUT_TOKENS': {
    ...
    'PKS_CONTINUATION_FALLBACK_MODEL': {
        ...
        'description': 'Fallback model when gpt-5.6-terra unavailable for continuation',
        'default': None,
    },
    ...
    'OPENAI_BASE_URL': {
        ...
        'description': 'OpenAI or OpenAI-compatible gateway base URL',
...
def get_variables_by_category() -> Dict[str, List[str]]:
    ...
    Keeps the OpenAI-compatible credential pair explicit.
...
def prompt_for_variable(var_info: Dict, current_value: str) -> Optional[str]:
    ...
    if 'API_KEY' in var_name or var_name.endswith('_KEY'):
        ...
        try:
            ...
            # Ensure gpt-5.6-terra is at the top of the list for easy access
            if 'gpt-5.6-terra' in all_models:
                all_models.remove('gpt-5.6-terra')
                all_models.insert(0, 'gpt-5.6-terra')
            
            if not all_models:
                all_models = ['gpt-5.6-terra']  # Fallback if loading fails
        except Exception:
        ...
        result = questionary.autocomplete(
            ...
            choices=all_models,
            default=current_value if current_value else 'gpt-5.6-terra',
            style=custom_style,
    ...
    elif var_name == 'PKS_REASONING_EFFORT':
        return questionary.select(
            f"{description}{display_current}",
            choices=["off", "low", "medium", "high", "xhigh", "max"],
            default=current_value if current_value else "off",
            style=custom_style,
        ).ask()
        ...
        try:
            ...
            if 'gpt-5.6-terra' in all_models:
                ...
                all_models.insert(0, 'gpt-5.6-terra')
            if not all_models:
                all_models = ['gpt-5.6-terra', 'o3-mini', 'gpt-4o', 'gpt-5.6-terra']
        except Exception:
            all_models = ['gpt-5.6-terra', 'o3-mini', 'gpt-4o', 'gpt-5.6-terra', 'claude-3.5-sonnet']
