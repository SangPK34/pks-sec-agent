...
def merge_flush_histories() -> Dict[str, List[Any]]:
    ...
    try:
        from pks.sdk.agents.models.openai_chatcompletions import get_all_agent_histories
    except ImportError:
        return {}
...
def _repl_should_validate_flush_targets() -> bool:
    ...
    return os.getenv("PKS_TUI_MODE") != "true"
...
class FlushCommand(Command):
    ...
    def handle(
        ...
        if not args and os.getenv("PKS_TUI_MODE") == "true":
    ...
    def handle_current_agent(self) -> bool:
        ...
        current_agent = os.getenv("PKS_CURRENT_AGENT", "Current Agent")
    ...
    def _notify_runner_busy(self, runner: Any) -> None:
        ...
        if (
            ...
            and os.getenv("PKS_BROADCAST_MODE") != "true"
        ):
    ...
    def _get_tui_app(self) -> Optional[Any]:
        """Return the active PKS Terminal application if available."""
        try:
            from pks.tui.cai_terminal import PKSTerminal

            app = getattr(PKSTerminal, "_current_app", None) or getattr(
                PKSTerminal, "_instance", None
            )
            if app:
                return app
            ...
            try:
                ...
                if running and isinstance(running, PKSTerminal):
                    return running
            except Exception:
                return None
        except Exception:
    ...
    def show_flush_help(self) -> bool:
        ...
        tree = Tree("[bold #ff3355]Flush Command - Available Agents[/bold #ff3355]")
        ...
        for agent_name, history in sorted(combined_histories.items()):
            ...
            if msg_count > 0:
                tree.add(
                    f"[bold #ff3355]{agent_name}{id_str}[/bold #ff3355] "
        ...
        console.print(tree)
        console.print(
            f"\n[#9aa0a6][PKS] Total messages across all agents:[/] "
            f"[bold #ff3355]{total_messages}[/bold #ff3355]"
        )

        console.print("\n[#9aa0a6][PKS] Usage:[/]")
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/flush <agent_name>[/bold #ff3355]"
            "[#9aa0a6] - Clear specific agent's history[/]"
        )
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/flush <ID>[/bold #ff3355]"
            "[#9aa0a6] - Clear agent by ID (e.g., /flush P2)[/]"
        )
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/flush all[/bold #ff3355]"
            "[#9aa0a6] - Clear all agents' histories[/]"
        )
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/flush agent <name>[/bold #ff3355]"
    ...
    def _get_client(self):
        """Get the PKS client from the global namespace.

        This function avoids circular imports by accessing the client
        at runtime instead of import time.

        Returns:
            The global PKS client instance or None if not available
        """
        try:
            # Import here to avoid circular import
            from pks.repl.repl import (
