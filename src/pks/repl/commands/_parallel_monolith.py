"""
Parallel command for PKS CLI abstraction.
...
from pks.config_loader import AgentsConfigError, extract_agent_definitions, load_agents_config
...
from pks.sdk.agents.models.openai_chatcompletions import (
...
from pks.sdk.agents import Agent, Runner
from pks.util.cli_palette import ORANGE_WARN
from pks.util.hint_renderables import build_pks_markup_line
...
class ParallelCommand(Command):
    ...
    def _sync_to_env(self):
        ...
        if len(PARALLEL_CONFIGS) >= 2:
            ...
            os.environ["PKS_PARALLEL_AGENTS"] = ",".join(agent_names)
            ...
            os.environ["PKS_PARALLEL"] = "1"
            os.environ["PKS_PARALLEL_AGENTS"] = ""
    ...
    def handle_no_args(self) -> bool:
        ...
        if PARALLEL_CONFIGS:
            ...
            for idx, config in enumerate(PARALLEL_CONFIGS, 1):
                ...
                model_info = f" [{_resolve_model_name(config.model)}]"
            ...
            console.print(
                "• [bold #ff3355]/parallel add <agent>[/bold #ff3355] - Add another agent"
            )
            console.print(
                "• [bold #ff3355]/parallel list[/bold #ff3355] - Show detailed configuration"
            )
            console.print(
                "• [bold #ff3355]/parallel clear[/bold #ff3355] - Clear all agents"
            )
            console.print(
                "• [bold #ff3355]/parallel remove <index/ID>[/bold #ff3355] - Remove specific agent "
                "(e.g. /parallel remove P2)"
            )
            console.print(
                "• [bold #ff3355]/parallel prompt <ID> <prompt>[/bold #ff3355] - Set custom prompt for agent"
            )
            console.print(
                "• [bold #ff3355]/parallel override-models[/bold #ff3355] - Make all agents use global model"
            )
            console.print(
                "• [bold #ff3355]/parallel merge <agents/IDs...>[/bold #ff3355] - Merge message histories "
    ...
    def _handle_no_args_tui(self) -> bool:
        ...
        try:
            from pks.tui.pks_terminal import PKSTerminal
            app = PKSTerminal._current_app
            ...
            status_text += (
                "• [bold #ff3355]/parallel add <agent>[/bold #ff3355] - Open new terminal with agent\n"
            )
            status_text += (
                "• [bold #ff3355]/parallel list[/bold #ff3355] - Show active terminals\n"
            )
            status_text += (
                "• [bold #ff3355]/agent <name>[/bold #ff3355] - Change agent in current terminal\n"
            )
            status_text += (
                "• [bold #ff3355]/model <name>[/bold #ff3355] - Change model in current terminal\n\n"
            )
    ...
    def _handle_add_tui(self, agent_name: str, model: Optional[str] = None, 
        ...
        """
        try:
            # Get the running app from PKSTerminal
            from pks.tui.pks_terminal import PKSTerminal
            app = PKSTerminal._current_app if hasattr(PKSTerminal, '_current_app') else None
        ...
        if instance_count > 1:
            console.print(
                build_pks_markup_line(
                    f"[#9aa0a6]Added [/][bold #ff3355]{display_name} #{instance_count}[/bold #ff3355]"
            ...
            console.print(
                build_pks_markup_line(
                    f"[#9aa0a6]Added [/][bold #ff3355]{display_name}[/bold #ff3355]"
                    "[#9aa0a6] to parallel configuration.[/]"
                )
            )
        ...
        if len(PARALLEL_CONFIGS) >= 2:
            console.print(
                build_pks_markup_line(
                    ...
                    f"[bold #ff3355]{len(PARALLEL_CONFIGS)}[/bold #ff3355][#9aa0a6] agents.[/]"
                )
            )
        else:
            console.print(
    ...
    def handle_list(self, args: Optional[list[str]] = None) -> bool:
        ...
        table = Table(
            title=f"[bold #ff3355]Configured Parallel Agents ({'ENABLED' if parallel_enabled else 'DISABLED'})[/bold #ff3355]",
        ...
        table.add_column("ID", style="bold #ff3355", width=4)
        table.add_column("Agent", style="white")
        table.add_column("Display Name", style="white")
        table.add_column("Model", style="bold #ff3355")
        ...
        for idx, config in enumerate(PARALLEL_CONFIGS, 1):
            ...
            table.add_row(
                ...
                _resolve_model_name(config.model),
        ...
        if parallel_enabled:
            ...
            console.print(
                build_pks_markup_line(
                    "[#9aa0a6]Parallel execution is [/][bold #ff3355]ACTIVE[/bold #ff3355]"
                )
            )
            console.print(
                build_pks_markup_line(
        ...
        try:
            ...
            for term in sorted(terminals, key=lambda x: x['number']):
                ...
                table.add_row(
                    f"T{term['number']}",
                    term['agent'],
                    _resolve_model_name(term['model']),
                    status
                )
    ...
    def handle_clear(self, args: Optional[list[str]] = None) -> bool:
        ...
        console.print(f"[bold #ff3355]Cleared {count} parallel configurations[/bold #ff3355]")
    ...
    def handle_override_models(self, args: Optional[list[str]] = None) -> bool:
        ...
        global_model = os.getenv("PKS_MODEL", "gpt-5.6-terra")
        count = 0
    ...
    def _merge_per_worker_summaries_enabled(no_worker_summary_flag: bool) -> bool:
        if no_worker_summary_flag:
            return False
        raw = (os.getenv("PKS_MERGE_SUMMARIZE_PER_WORKER", "1") or "1").strip().lower()
    ...
    def _worker_qualifies_for_merge_summary(history: List[dict], force: bool) -> bool:
        ...
        if force:
            return True
        try:
            min_msg = int(os.getenv("PKS_MERGE_SUMMARIZE_MIN_MESSAGES", "20"))
        except ValueError:
            min_msg = 20
    ...
    def _truncate_worker_history(self, hist: List[dict], max_msgs: int = 48) -> List[dict]:
        ...
        head = {
            "role": "system",
            "content": (
                f"[PKS merge] Transcript truncated to the last {max_msgs} messages "
    ...
    async def _summarize_one_worker_history_async(
        ...
        from pks.repl.commands._memory_monolith import MEMORY_COMMAND_INSTANCE
        from pks.repl.commands.compact import get_compact_model
        ...
        try:
            model_name = get_compact_model() or os.getenv("PKS_MODEL", "gpt-5.6-terra")
    ...
    async def _apply_per_worker_merge_summaries_async(
        ...
        for agent in agents_to_merge:
            ...
            console.print(
                build_pks_markup_line(
                    f"[#9aa0a6]Building per-worker merge digest for[/] [bold #ff3355]{agent}[/] "
        ...
        if any_digest:
            console.print(
                build_pks_markup_line(
                    ...
                    "(disable with [/][bold #ff3355]--no-worker-summary[/bold #ff3355][#9aa0a6] "
                    "or [/][bold #ff3355]PKS_MERGE_SUMMARIZE_PER_WORKER=0[/bold #ff3355][#9aa0a6]).[/]"
                )
            )
        return updated
    ...
    def _collect_merge_histories(self, verbose_merge: bool) -> Dict[str, List[Any]]:
        ...
        if PARALLEL_CONFIGS and (
            ...
            if os.getenv("PKS_TUI_MODE"):
                try:
                    from pks.tui.utils.merge_helper import get_terminal_agent_histories
    ...
    def _print_merge_hint_list(self, all_histories: Dict[str, Any]) -> None:
        ...
        from pks.repl.commands.flush import repl_ordered_nonempty_labels_for_keys
    ...
    def handle_merge(self, args: Optional[list[str]] = None) -> bool:
        ...
        try:
            debug_level = int(os.getenv("PKS_DEBUG", "0") or 0)
        except ValueError:
            debug_level = 0
        ...
        if not all_histories:
            ...
            console.print(
                ...
                "(e.g. run parallel workers or [/][bold #ff3355]/load parallel[/bold #ff3355][dim]).[/dim]"
            )
            return False
        ...
        if not agent_names:
            ...
            console.print(
                "[dim]Use tab completion after [/][bold #ff3355]/merge[/bold #ff3355][dim], "
                "or specify [/][bold #ff3355]all[/bold #ff3355][dim] to merge every agent with history.[/dim]"
            )
        ...
        if "all" in agent_names:
            ...
            if missing_agents:
                ...
                console.print(
                    ...
                    "(use tab completion after [/][bold #ff3355]/merge[/bold #ff3355][dim]).[/dim]"
                )
                self._print_merge_hint_list(all_histories)
                return False
        ...
        if len(agents_to_merge) < 2:
            ...
            console.print(
                ...
                "(see [/][bold #ff3355]/h merge[/bold #ff3355][dim] or tab-complete after [/]"
                "[bold #ff3355]/merge[/bold #ff3355][dim]).[/dim]"
            )
            self._print_merge_hint_list(all_histories)
            console.print(
                "[dim]You can load histories with [/][bold #ff3355]/load parallel[/bold #ff3355][dim] "
        ...
        console.print(
            build_pks_markup_line(
                f"[#9aa0a6]Merging histories from:[/] [bold #ff3355]{', '.join(agents_display)}[/]"
            )
        )
        if verbose_merge:
        ...
        if not merged_history:
            console.print(build_pks_markup_line("[#9aa0a6]No messages found to merge.[/]"))
            return False
        ...
        summary = f"[bold #ff3355]Successfully merged {len(agents_to_merge)} agents[/bold #ff3355]\n\n"
        ...
        if merge_to_all_sources:
            ...
            summary += f"[#9aa0a6]Use [/][bold #ff3355]/history {target_agent}[/bold #ff3355][#9aa0a6] to view the merged history[/]"
    ...
    def _save_merged_history(
        ...
        from pks.agents import get_agent_by_name, get_available_agents
        ...
        if not target_exists_in_configs:
            ...
            console.print(
                build_pks_markup_line(
                    f"[#9aa0a6]Created merged history for [/][bold #ff3355]{target_agent}[/bold #ff3355]"
            ...
            if existing_model:
                ...
                console.print(
                    build_pks_markup_line(
                        ...
                        f"[bold #ff3355]{target_agent}[/bold #ff3355][#9aa0a6].[/]"
                    )
                )
            else:
    ...
    def _save_merged_history_to_sources(
        ...
        for agent_name in source_agents:
            ...
            if os.getenv("PKS_TUI_MODE"):
                try:
                    from pks.tui.utils.merge_helper import update_terminal_agent_history
            ...
            if not updated_in_tui:
                if existing_model:
                    ...
                    console.print(
                        build_pks_markup_line(
                            f"[bold #ff3355]Updated {agent_name}[/bold #ff3355]"
    ...
    def handle_run(self, args: Optional[list[str]] = None) -> bool:
        ...
        table = Table(
            title="[bold #ff3355]Parallel Execution[/bold #ff3355]",
            show_header=True,
            header_style="bold white",
            border_style=_PKS_GREEN,
            row_styles=["none", "#9aa0a6"],
        ...
        table.add_column("ID", style="bold #ff3355", width=6)
        table.add_column("Agent", style="white")
        table.add_column("Model", style="bold #ff3355", width=18)
        table.add_column("Prompt", style="white")

        for cfg in PARALLEL_CONFIGS:
            pid = cfg.id or "?"
            model_label = _resolve_model_name(cfg.model)
            prompt_short = (
        ...
        console.print(
            build_pks_markup_line(
                f"\n[#9aa0a6]Launching [/][bold #ff3355]{len(PARALLEL_CONFIGS)}[/bold #ff3355]"
