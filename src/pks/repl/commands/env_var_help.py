...
_VAR_EXAMPLES: Dict[str, List[str]] = {
    "PKS_MODEL": [
        "[bold #ff3355]export PKS_MODEL=alias1[/bold #ff3355]",
    ...
    "PKS_AGENT_TYPE": [
        ...
        "[bold #ff3355]export PKS_AGENT_TYPE=selection_agent[/bold #ff3355]  [dim]# handoff-only router[/dim]",
        "[bold #ff3355]/agent select redteam_agent[/bold #ff3355]  [dim]# also updates this env[/dim]",
        "[bold #ff3355]/env set 10 one_tool_agent[/bold #ff3355]",
    ],
    ...
    "CTF_NAME": [
        "[bold #ff3355]export CTF_NAME=my_challenge_image_tag[/bold #ff3355]  [dim]# pentestperf / caibench[/dim]",
        "[bold #ff3355]/env set 1 kiddoctf[/bold #ff3355]",
    ],
    "CSI_CUSTOM_ENDPOINT": [
        "[bold #ff3355]export CSI_CUSTOM_ENDPOINT=http://127.0.0.1:8080/v1[/bold #ff3355]  [dim]# e.g. CSI + PKS[/dim]",
    ],
    "ALIAS_API_URL": [
        "[bold #ff3355]export ALIAS_API_URL=https://your-gateway.example/v1/[/bold #ff3355]",
        "[bold #ff3355]/env set ALIAS_API_URL https://your-gateway.example/v1/[/bold #ff3355]",
    ],
}
...
_VAR_NOTES: Dict[str, str] = {
    "PKS_MODEL": (
        "This is the LiteLLM / provider-facing model id PKS passes for most agents unless "
        "overridden by a per-agent [bold]PKS_<AGENT>_MODEL[/bold]. Alias names like "
        "[bold]alias1[/bold] are resolved inside PKS."
    ),
    "PKS_AGENT_TYPE": (
    ...
    "CSI_CUSTOM_ENDPOINT": (
        "When non-empty and the model qualifies (same [bold]pks[/bold] / [bold]alias[/bold] / [bold]csi[/bold] "
        "prefix rule as [bold]ALIAS_API_URL[/bold]), this OpenAI-compatible base wins over [bold]ALIAS_API_URL[/bold]. "
        "Usually set by CSI when launching PKS."
    ),
    "ALIAS_API_URL": (
        "When non-empty and the model qualifies, PKS uses this base for [bold]/chat/completions[/bold] after "
