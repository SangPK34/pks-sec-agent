"""
Auth command for PKS REPL.

This command allows adding users and devices to the persistent
authentication database used by the PKS API (`AuthManager`).
...
from pks.api.auth import AuthManager
...
class AuthCommand(Command):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="/auth",
            description="Manage API auth users and pair devices with the PKS server",
            aliases=[],
        )
        self.add_subcommand(
            "add-user",
    ...
    def handle_add_ip(self, args: Optional[List[str]] = None) -> bool:
        ...
        if ":" in target:
            ...
            host = target
            port = int(os.getenv("PKS_AUTH_DEVICE_PORT", "10101"))

        # Determine API base URL to send to the device.
        # Priority:
        #   1. Explicit PKS_AUTH_BASE_URL
        #   2. PKS_AUTH_PUBLIC_HOST (if set)
        #   3. IP of the local interface used to reach the device
        #   4. PKS_API_HOST / 127.0.0.1 (last-resort fallback)
        base_url_env = os.getenv("PKS_AUTH_BASE_URL")
        api_host_env = os.getenv("PKS_AUTH_PUBLIC_HOST")
        api_port = int(os.getenv("PKS_AUTH_PUBLIC_PORT") or os.getenv("PKS_API_PORT", "8000"))
        if base_url_env:
