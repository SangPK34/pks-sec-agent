...
from pks.repl.commands.env_catalog import set_env_var
from pks.repl.ui.banner import _PKS_GREEN, _quick_guide_subpanel_title
from pks.tools.common import _get_container_workspace_path, _get_workspace_dir
...
def _print_no_active_container_for_copy() -> None:
    ...
    body = (
        ...
        f"[bold {_PKS_GREEN}]/workspace copy[/bold {_PKS_GREEN}] requires "
        f"[dim]PKS_ACTIVE_CONTAINER[/dim] set to a running container ID.\n\n"
        ...
        f"[{g}]/virt <container_id>[/{g}] [dim](sets PKS_ACTIVE_CONTAINER).[/dim]\n"
        "[dim]• From your shell before launching pks:[/dim] "
        "[dim]export PKS_ACTIVE_CONTAINER=<id>[/dim]\n"
...
class WorkspaceCommand(Command):
    ...
    def handle_get(self, _: Optional[List[str]] = None) -> bool:
        ...
        workspace_name = os.getenv("PKS_WORKSPACE", None)
        ...
        g = _ws_accent_open()
        lines = [
            f"Current workspace: [bold {_PKS_GREEN}]{workspace_name or 'None'}[/bold {_PKS_GREEN}]",
        ...
        if not active_container:
            lines.append(
                f"  [dim]copy requires PKS_ACTIVE_CONTAINER — see [/dim][{g}]/h virtualization[/{g}]"
            )
        lines.append("")
        lines.append(
    ...
    def handle_set(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not set_env_var("PKS_WORKSPACE", workspace_name):
        ...
        body = (
            f"Workspace changed to: [bold {_PKS_GREEN}]{workspace_name}[/bold {_PKS_GREEN}]\n"
    ...
    def handle_copy_subcommand(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args or len(args) < 2:
            ...
            body = (
                ...
                f"[dim]Exactly one path must use the[/dim] [bold {_PKS_GREEN}]container:[/bold {_PKS_GREEN}] "
                f"[dim]prefix; requires[/dim] [dim]PKS_ACTIVE_CONTAINER[/dim][dim].[/dim]"
            )
            console.print(_ws_panel(body, "Copy"))
            return False
        ...
        if source.startswith("container:"):
            ...
            try:
                ...
                if result.returncode == 0:
                    console.print(
                        _ws_panel(
                            f"[bold {_PKS_GREEN}]Copied[/bold {_PKS_GREEN}] "
        ...
        body = (
            ...
            f"[dim]Use the[/dim] [bold {_PKS_GREEN}]container:[/bold {_PKS_GREEN}] "
