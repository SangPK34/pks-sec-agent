...
# dependency_id is resolved in repl.commands.environment_reference (pentestperf = pks.caibench importable).
CATEGORY_DISPLAY: Dict[str, Dict[str, Any]] = {
    "Core agent & model": {
        "overview": "[dim]Defaults for which [bold]model[/bold] and [bold]agent type[/bold] run ([bold]orchestration_agent[/bold] when unset, or e.g. selection_agent, redteam_agent, one_tool_agent), optional [bold]PKS_ORCHESTRATION_*[/bold] tuning when the entry agent spawns specialist workers, sampling (temperature / top_p), debug verbosity, and output shaping. These affect most interactive and headless sessions.[/dim]",
    },
    "CTF (capture-the-flag)": {
        ...
        "missing_dependency_note": (
            "[#9aa0a6]The [bold]caibench[/bold] package ([bold]pks.caibench[/bold]) is not available in this environment. "
            "The published wheel often excludes [bold]src/pks/caibench/[/bold] (see [bold]pyproject.toml[/bold] hatch excludes); "
        ...
        "present_dependency_note": (
            "[#9aa0a6][bold]caibench[/bold] is loaded. If you do not run CTF or benchmark flows, consider a minimal PKS install "
...
def is_secret(var_name: str) -> bool:
    ...
    PKS_MCP_TOKEN, PKS_MCP_AUTH_TOKEN) and the user has no in-session way to
    restore them.
    """
...
_CONSTRAINT_LABEL_BY_VAR: Dict[str, str] = {
    ...
    "PKS_DISABLE_USAGE_TRACKING": "bool",
    "PKS_GUARDRAILS": "bool",
    "PKS_PLAN": "bool",
    "PKS_COST_DISPLAYED": "bool",
    "PKS_ENABLE_PRICING_FETCH": "bool",
    "PKS_DEBUG_PRICING": "bool",
    "PKS_PRICING_FILE": "string",
    "PKS_PRICINGS_DIR": "string",
    "PKS_REPORT": "string",
    ...
    "PKS_TOOL_LIVE_SHOW_PRICING": "bool",
...
EXTRA_ENV_VARS: List[Dict[str, Any]] = [
    ...
    {
        "name": "PKS_TOOL_LIVE_SHOW_PRICING",
        "default": "unset (off)",
    ...
    },
    {
        "name": "ALIAS_API_KEY",
        "default": "unset",
        "description": "Primary API key for Alias Robotics / PKS gateway (also check OPENAI_API_KEY fallbacks).",
    },
    {
        ...
        "description": (
            ...
            "(typically injected by CSI with PKS backend). Expect ``/chat/completions`` compatibility."
        ),
    },
    {
        "name": "ALIAS_API_URL",
        "default": "unset",
        "description": (
            "When set (non-empty) and the model qualifies (``pks`` / ``alias`` / ``csi`` prefix rule), "
