...
# Custom style for questionary (Layout 1: active = black on PKS green; question bold white)
custom_style = Style([
    ('qmark', 'fg:#ff3355 bold'),
    ('question', 'bold white'),
    ('answer', 'fg:#000000 bg:#ff3355 bold'),
    ('pointer', 'fg:#000000 bg:#ff3355 bold'),
    ('highlighted', 'fg:#000000 bg:#ff3355 bold'),
    ('selected', 'fg:#000000 bg:#ff3355'),
    ('separator', 'fg:#6c6c6c'),
...
CLI_ONLY_VARIABLES = {
    'PKS_API_HOST', 'PKS_API_PORT', 'PKS_API_CORS', 'PKS_API_KEY_HEADER',
    'PKS_API_LOG_AUTH', 'PKS_API_LOG_REQUESTS', 'PKS_API_LOG_LEVEL',
    'PKS_API_RELOAD', 'PKS_API_WORKERS',
}
...
def is_tui_mode() -> bool:
    ...
    return os.getenv("PKS_TUI_MODE", "").lower() == "true"


def get_current_terminal_id() -> Optional[str]:
    """Get the current terminal ID if in TUI mode."""
    if is_tui_mode():
        return os.getenv("PKS_ACTIVE_COMMAND_TERMINAL_ID")
    return None
