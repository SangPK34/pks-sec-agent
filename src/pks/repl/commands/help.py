...
def cost_help_panel_markup() -> str:
    ...
    return (
        _h_panel_desc(
            "Current session spend and tokens, plus global totals from ~/.pks/usage.json when "
        ...
        f"[dim]Aliases: /costs, /usage. Global file tracking off: PKS_DISABLE_USAGE_TRACKING=true[/dim]"
    )
...
class HelpCommand(Command):
    ...
    def __init__(self):
        ...
        self.add_subcommand("api", "Help for /api: ALIAS_API_KEY in .env (Alias / PKS PRO)", self.handle_api)
        self.add_subcommand(
    ...
    def handle_agent(self, _: Optional[List[str]] = None) -> bool:
        ...
        console.print(
            Panel(
                ...
                "• [bold #ff3355]selection_agent[/bold #ff3355] - Default entry: handoff-only router "
                ...
                "• [bold #ff3355]one_tool_agent[/bold #ff3355] - Basic CTF solver\n"
    ...
    def handle_save(self, _: Optional[List[str]] = None) -> bool:
        ...
        console.print(
            Panel(
                ...
                "• [bold #ff3355]/save ~/notes/cai_thread.jsonl[/bold #ff3355]\n\n"
    ...
    def handle_resume(self, _: Optional[List[str]] = None) -> bool:
        ...
        console.print(
            Panel(
                ...
                "• [bold #ff3355]/resume <token>[/bold #ff3355] — match in [dim]logs/cai_*.jsonl[/dim] filenames "
                "(not a path)\n\n"
                f"[bold {z}]Related:[/bold {z}]\n"
    ...
    def handle_api(self, _: Optional[List[str]] = None) -> bool:
        ...
        console.print(
            Panel(
                _h_panel_desc(
                    "ALIAS_API_KEY for Alias-hosted models (PKS PRO). "
                ...
                f"• HTTP API server (FastAPI) may use [bold {z}]PKS_API_KEY[/bold {z}] as a fallback "
