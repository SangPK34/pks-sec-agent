...
titles, PKS palette (see ``banner.py``).
"""
...
from pks.repl.commands.env_catalog import ENV_VARS, HELP_REFERENCE_MATCH_TABLE_KWARGS
...
from pks.repl.commands.env_var_help import usage_markup_bold
from pks.repl.ui.banner import _PKS_GREEN, _GREY, _quick_guide_subpanel_title, environment_reference_outer_title
...
def _extra_vars_panel() -> Optional[Panel]:
    ...
    for idx, entry in enumerate(remaining):
        ...
        table.add_row(
            Text(name, style=f"bold {_PKS_GREEN}"),
            Text(str(entry.get("default", "—")), style=_PKS_GREEN),
            Text(constraints_line(name, desc), style=body_style),
            Text(effective_label(name), style=body_style),
            Text(desc, style=body_style),
        )
