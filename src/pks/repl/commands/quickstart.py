"""
Quickstart command for PKS REPL.
Provides essential setup information and guidance for new users.
Automatically runs on first launch if ~/.pks doesn't exist.
"""
...
def _qs_state_cell(ok: bool, label: str) -> Text:
    """Positive states in PKS green; missing/off in dim (not green)."""
    return Text(label, style=f"bold {_PKS_GREEN}" if ok else "dim white")
    ...
    def show_quickstart(self) -> bool:
        ...
        ollama_inner.append(
            Text.from_markup(
                ...
                f"[white]· PKS[/white] [bold {z}]/model llama3.1[/bold {z}]"
            )
        )
        ...
        examples_table.add_row("CTF", "/agent select ctf_agent", "Describe the challenge.")
        ...
        pks_dir = Path.home() / ".pks"
        pks_note = "will be created on first run" if not pks_dir.exists() else "exists"
        next_panel = Panel(
            Text.from_markup(
                "[white]Then open[/white] "
                f"[bold {z}]/help topics[/bold {z}] [white]for commands by category and /help <topic> hints.[/white]\n"
                f"[dim]Logs and state under {pks_dir} ({pks_note}). "
        ...
        intro = Text.from_markup(
            f"[white]PKS (Cybersecurity AI): pentest, bug bounty, CTF. "
