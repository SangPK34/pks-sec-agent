...
from pks.util import COST_TRACKER, get_ollama_api_base, get_ollama_auth_headers

console = Console()

_MODEL_HEADER = f"bold {_PKS_GREEN}"
_MODEL_MUTED = "#9aa0a6"
...
def _print_model_selection_error(body: str, *, title: str = "Invalid selection") -> None:
    """Error panel for invalid ``/model`` name or index (red border, PKS title style)."""
    console.print(
        Panel(
            Text.from_markup(body, overflow="fold"),
            title=_quick_guide_subpanel_title(title),
...
def get_predefined_model_categories() -> Dict[str, List[Dict[str, str]]]:
    ...
    across the PKS system. Other modules should import and use this function
...
def _execute_model_show(show_args: Optional[List[str]] = None) -> bool:  # pylint: disable=too-many-locals,too-many-branches,too-many-statements,line-too-long # noqa: E501
    ...
    try:
        with console.status(f"[bold {_PKS_GREEN}]Fetching model data...[/]"):
            response = requests.get(LITELLM_URL, timeout=5)
        ...
        model_table.add_column("Model", style=_PKS_GREEN)
        ...
        summary_text = (
            f"\n[bold {_PKS_GREEN}]Showing {displayed_models} of {total_models} models"
        )
        if show_only_supported:
            summary_text += " with function calling support"
        if search_term:
            summary_text += f" matching '{search_term}'"
        summary_text += f"[/bold {_PKS_GREEN}]"
...
class ModelCommand(Command):
    ...
    def handle_model_command(self, args: List[str]) -> bool:
        ...
        if not args:  # pylint: disable=too-many-nested-blocks
            ...
            model_info = os.getenv("PKS_MODEL", "Unknown")
            console.print(
                _model_info_panel(
                    f"Current model: [bold {_PKS_GREEN}]{model_info}[/bold {_PKS_GREEN}]",
                    "Active Model",
                )
            )
        ...
        if not model_arg:
            _print_model_selection_error(
                ...
                f"Run [bold {_PKS_GREEN}]/model[/bold {_PKS_GREEN}] for a short list or "
                f"[bold {_PKS_GREEN}]/model show[/bold {_PKS_GREEN}] for the full catalog.",
        ...
        if model_arg.isdigit():
            ...
            if 0 <= model_index < n:
                ...
                _print_model_selection_error(
                    ...
                    f"Run [bold {_PKS_GREEN}]/model[/bold {_PKS_GREEN}] or "
                    f"[bold {_PKS_GREEN}]/model show[/bold {_PKS_GREEN}] for the numbered list.",
            ...
            _print_model_selection_error(
                ...
                f"Run [bold {_PKS_GREEN}]/model[/bold {_PKS_GREEN}] or "
                f"[bold {_PKS_GREEN}]/model show[/bold {_PKS_GREEN}] to pick a valid name.",
                title="Unknown model",
            )
            return True
