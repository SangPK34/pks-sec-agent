...
try:
    if os.getenv("PKS_TUI_MODE") == "true":
        from pks.tui.core.prompt_queue import PROMPT_QUEUE as TUI_PROMPT_QUEUE
        from pks.tui.core.terminal_queue import TERMINAL_QUEUE_MANAGER
    else:
...
class QueueCommand(Command):
    ...
    def handle(self, args: Optional[list[str]] = None) -> bool:
        ...
        if TERMINAL_QUEUE_MANAGER and os.getenv("PKS_TUI_MODE") == "true":
    ...
    def _move_in_queue(self, from_idx: int, to_idx: int) -> bool:
        ...
        move_panel = Panel(
            f"[bold #ff3355]Item moved successfully[/bold #ff3355]\n\n"
        ...
        if args[0] == "--agent":
            ...
            if candidate in available:
                ...
                for key in sorted(available):
                    ...
                    console.print(f"  [bold #ff3355]{key}[/bold #ff3355] — {name}")
                return False
    ...
    def _interactive_agent_select(self, available: dict) -> bool:
        ...
        table = RichTable(
            title="[bold #ff3355]Available Agents[/bold #ff3355]",
            show_header=True,
        ...
        table.add_column("Key", style="bold #ff3355")
    ...
    def _handle_run(self, args: Optional[list[str]] = None) -> bool:
        ...
        from rich.table import Table as RichTable
        table = RichTable(
            title="[bold #ff3355]Executing Queue[/bold #ff3355]",
            show_header=True,
            header_style="bold white",
            border_style=_PKS_GREEN,
            box=None,
        ...
        table.add_column("#", style="#9aa0a6", width=4)
        table.add_column("Agent", style="bold #ff3355", width=20)
        ...
        console.print(table)
        console.print(
            f"\n[bold #ff3355]Starting sequential execution "
            f"of {len(queue_items)} prompt(s)...[/bold #ff3355]"
        )
    ...
    def _show_queue(self, args: Optional[list[str]] = None) -> bool:
        ...
        table = Table(
            title="[bold #ff3355]Prompt Queue[/bold #ff3355]",
            show_header=True,
            header_style="bold white",
            border_style=_PKS_GREEN,
            title_style="bold #ff3355",
        ...
        table.add_column("#", style="bold #ff3355", width=4, justify="center")
        if has_agents:
            table.add_column("Agent", style="bold #ff3355", width=18)
        ...
        for i, item in enumerate(queue_items, 1):
            ...
            if prompt_text.startswith("/"):
                prompt_display = f"[bold #ff3355]{prompt_text}[/bold #ff3355]"
            ...
            row: list[str] = [f"[bold #ff3355]{i}[/bold #ff3355]"]
            if has_agents:
        ...
        # Add helpful tip
        console.print(
            "\n[#9aa0a6]Tip: [/][bold #ff3355]/queue add <prompt>[/bold #ff3355]"
            "[#9aa0a6] to add, [/][bold #ff3355]/queue run[/bold #ff3355]"
            "[#9aa0a6] to execute, [/][bold #ff3355]/queue remove <index>[/bold #ff3355]"
            "[#9aa0a6] to remove[/]"
        )
    ...
    def _get_queue_items(self) -> List[dict]:
        ...
        if os.getenv("PKS_TUI_MODE") == "true":
            try:
                from pks.tui.core.prompt_queue import PROMPT_QUEUE as TUI_QUEUE
                # Get from TUI queue
                items = []
    ...
    def _add_to_queue(self, prompt: str, agent: Optional[str] = None) -> bool:
        ...
        success_panel = Panel(
            f"[bold #ff3355]Prompt added successfully![/bold #ff3355]\n\n"
            f"[white]Position: [bold #ff3355]#{queue_len}[/bold #ff3355]{agent_label}[/white]\n"
    ...
    def _remove_from_queue(self, index: int) -> bool:
        ...
        remove_panel = Panel(
            f"[bold #ff3355]Item removed from queue[/bold #ff3355]\n\n"
    ...
    def _get_next(self) -> bool:
        ...
        content = f"[bold #ff3355]Next in queue[/bold #ff3355]\n\n"
    ...
    def _show_help(self) -> None:
        ...
        help_table.add_column("Command", style="bold #ff3355")
    ...
    def _show_terminal_queues(self) -> bool:
        ...
        # Create main table
        table = Table(
            title="[bold #ff3355]Terminal Queue Status[/bold #ff3355]",
            show_header=True,
            header_style="bold white",
            border_style=_PKS_GREEN,
            row_styles=["none", "#9aa0a6"],
        ...
        table.add_column("Terminal", style="bold #ff3355", width=10)
        ...
        for terminal_num in sorted(all_status.keys()):
            ...
            queue_text = f"[bold #ff3355]{queue_len}[/bold #ff3355]" if queue_len > 0 else "[dim]0[/dim]"
            
            table.add_row(
                f"[bold #ff3355]T{terminal_num}[/bold #ff3355]",
                status_text,
                current_text,
                queue_text,
            )
    ...
    def _clear_terminal_queue(self, terminal_num: int) -> bool:
        ...
        if count > 0:
            success_panel = Panel(
                f"[bold #ff3355]Cleared {count} prompt(s) from terminal {terminal_num} queue[/bold #ff3355]",
    ...
    def _clear_all_terminal_queues(self) -> bool:
        ...
        if count > 0:
            success_panel = Panel(
                f"[bold #ff3355]Cleared {count} prompt(s) from all terminal queues[/bold #ff3355]",
...
def load_queue_from_file(file_path: str) -> int:
    ...
    try:
        ...
        if os.getenv("PKS_TUI_MODE") == "true":
            try:
                from pks.tui.core.prompt_queue import PROMPT_QUEUE as TUI_QUEUE, QueuedPrompt
        ...
        console.print(
            f"[bold #ff3355]Loaded {loaded_count} prompts from {file_path}[/bold #ff3355]"
        )
...
def load_queue_from_env():
    """Load queue from file specified in PKS_QUEUE_FILE environment variable"""
    queue_file = os.getenv("PKS_QUEUE_FILE")
    ...
    if queue_file:
        ...
        if os.path.exists(queue_file):
            ...
            if loaded > 0:
                console.print(
                    f"[#9aa0a6]Auto-loaded {loaded} prompts from PKS_QUEUE_FILE[/]"
                )
        else:
            console.print(
                f"[yellow]PKS_QUEUE_FILE set but file not found: {queue_file}[/yellow]"
            )
