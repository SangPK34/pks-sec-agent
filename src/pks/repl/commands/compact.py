...
from pks.repl.commands.model import (
...
class CompactCommand(Command):
    ...
    def handle_model(self, args: Optional[List[str]] = None) -> bool:
        ...
        if not args:
            ...
            all_model_names, _ = load_all_available_models()
            ...
            for i, model in enumerate(all_predefined, 1):
                model_table.add_row(
                    str(i), model["name"], model["provider"], model["description"],
                )
    ...
    def _show_help_menu(self):
        ...
        console.print(
            Panel(
                "[bold #ff3355]Compact Command - Memory Summarization[/bold #ff3355]\n\n"
                ...
                title="[bold #ff3355]Compact Settings[/bold #ff3355]",
                border_style="#ff3355",
            )
        )

        console.print("\n[#9aa0a6][PKS] Available commands:[/]")
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/compact[/bold #ff3355]"
            "[#9aa0a6] - Summarize current conversation[/]"
        )
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/compact model[/bold #ff3355]"
            "[#9aa0a6] - Configure model for compaction[/]"
        )
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/compact prompt[/bold #ff3355]"
            "[#9aa0a6] - Set custom summarization prompt[/]"
        )
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/compact status[/bold #ff3355]"
            "[#9aa0a6] - Show current settings[/]"
        )
        console.print("\n[#9aa0a6][PKS] Quick usage:[/]")
        console.print(
            "  [#9aa0a6]• [/][bold #ff3355]/compact --model o3-mini[/bold #ff3355]"
            "[#9aa0a6] - Compact with specific model[/]"
        )
        console.print(
            '  [#9aa0a6]• [/][bold #ff3355]/compact --prompt "Focus on..."[/bold #ff3355]'
            "[#9aa0a6] - Compact with custom prompt[/]"
        )
        console.print(
            "\n[#9aa0a6][PKS] Note: compacted conversations are saved to [/]"
            "[bold #ff3355]/memory[/bold #ff3355][#9aa0a6] for later use.[/]"
        )
    ...
    def _ask_and_perform_compaction(self) -> bool:
        ...
        if os.getenv("PKS_TUI_MODE") == "true":
            ...
            from pks.tui.core import terminal_tracking
            ...
            if not agent_name or agent_name == "Unknown Agent":
                try:
                    ...
                    if not agent_name or agent_name == "Unknown Agent":
                        ...
                        env_agent = os.getenv("PKS_AGENT_TYPE", "root_agent")
        ...
        console.print(
            f"\n[#9aa0a6][PKS] Compact current conversation? [/]"
            f"[bold white]({msg_count} messages)[/bold white]"
        )
        confirm = console.input(
            "[#9aa0a6][PKS] Compact conversation? [/][bold #ff3355](y/N): [/]"
        )
    ...
    def _perform_parallel_compaction(self) -> bool:
        ...
        from pks.repl.commands.parallel import PARALLEL_CONFIGS
        from pks.sdk.agents.simple_agent_manager import AGENT_MANAGER
        from pks.sdk.agents.models.openai_chatcompletions import get_agent_message_history
    ...
    def _perform_compaction(
        ...
        **kwargs,
        ...
        Returns:
            True if successful
        """
        from pks.repl.commands.memory import MEMORY_COMMAND_INSTANCE
        from pks.sdk.agents.simple_agent_manager import AGENT_MANAGER
        from pks.sdk.agents.models.openai_chatcompletions import (
        ...
        if os.getenv("PKS_TUI_MODE") == "true" and agent_name:
            ...
            if "(T" in agent_name and ")" in agent_name:
                ...
                from pks.sdk.agents.models.openai_chatcompletions import ACTIVE_MODEL_INSTANCES
                from pks.tui.core.terminal_tracking import get_current_terminal_id
        ...
        original_env_model = os.environ.get("PKS_MODEL", "gpt-5.6-terra")
    ...
    def _dispatch_to_ui(self, callback) -> None:
        ...
        try:
            ...
            try:
                ...
                tui_app = getattr(PKSTerminal, "_instance", None) or getattr(
                    PKSTerminal, "_current_app", None
                )
            except Exception:
                tui_app = None
