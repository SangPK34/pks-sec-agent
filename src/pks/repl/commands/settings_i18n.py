"""
Internationalization (i18n) support for PKS settings command.
...
UI_STRINGS: Dict[str, Dict[str, str]] = {
    'en': {
        # Main UI
        'title': 'PKS Interactive Settings',
        ...
        'tui_settings_title': 'PKS settings',
    ...
    'es': {
        # Main UI
        'title': 'Configuración Interactiva de PKS',
        ...
        'choice_value_masked': '{masked}',
        'tui_settings_title': 'Ajustes de PKS',
    ...
    'ru': {
        'title': 'Интерактивные настройки PKS',
        ...
        'cat_core': 'Основные настройки PKS',
    ...
    'de': {
        # Main UI
        'title': 'PKS Interaktive Einstellungen',
        ...
        'cat_core': 'PKS-Kerneinstellungen',
    ...
    'pt': {
        # Main UI
        'title': 'Configurações Interativas do PKS',
    ...
    'fr': {
        # Main UI
        'title': 'Paramètres Interactifs de PKS',
        ...
        # Categories
        'cat_ctf': 'Variables CTF',
        'cat_core': 'Paramètres Principaux de PKS',
...
FAQ_CONTENT: Dict[str, Dict[str, Dict[str, Any]]] = {
    'en': {
            ...
            'steps': [
                ...
                },
                {
                    ...
                    'env_vars': {
                        ...
                        'PKS_MODEL': 'ollama/llama3.2',
                    },
                },
            ],
            'docker_notes': {
                'title': 'Docker-specific configuration',
                'description': 'When running PKS in Docker, localhost refers to the container, not your host',
                'solutions': {
        ...
        'streaming': {
            ...
            'description': 'Understanding PKS streaming options',
            'related_commands': [
                {'command': '/env set PKS_STREAM', 'description': 'Toggle LLM inference streaming'},
                {'command': '/env set PKS_TOOL_STREAM', 'description': 'Toggle tool output streaming'},
            ],
            'variables': {
                'PKS_STREAM': {
        ...
        'parallel': {
            ...
            'related_commands': [
                ...
                {'command': '/env set PKS_PARALLEL', 'description': 'Set number of parallel agents'},
                {'command': '/env set PKS_PARALLEL_AGENTS', 'description': 'Specify agent names for parallel'},
            ],
            'variables': {
                'PKS_PARALLEL': 'Number of parallel instances (1-20)',
                'PKS_PARALLEL_AGENTS': 'Comma-separated agent names',
                'PKS_AUTO_RUN_PARALLEL': 'Auto-start parallel agents',
            },
            'common_issues': [
                {
                ...
                },
                {
                    ...
                    'fix': 'Reduce PKS_PARALLEL or add delays',
                },
            ],
        },
        ...
        'memory': {
            ...
            'related_commands': [
                ...
                {'command': '/env set PKS_COMPACTED_MEMORY', 'description': 'Enable (true) or disable (false) prompt injection'},
            ],
            'requirements': [],
            'variables': {
                'PKS_COMPACTED_MEMORY': 'true/false — inject /compact summaries into system prompts',
            },
            'setup_steps': [
                'Run /compact (or TUI compaction) to create a summary',
                'Set PKS_COMPACTED_MEMORY=true so new agents load the summary into their prompt',
            ],
        },
        ...
        'tui': {
            ...
            'related_commands': [
                {'command': 'pks --tui', 'description': 'Start PKS in TUI mode'},
                {'command': '/env set PKS_TUI_MODE', 'description': 'Enable/disable TUI mode'},
            ...
            'variables': {
                'PKS_TUI_MODE': 'Enable TUI mode',
                'PKS_TUI_MAX_LINES': 'Maximum output lines',
                'PKS_TUI_MAX_RERENDERS_PER_SEC': 'Rendering performance',
            },
            'cli_only_vars': [
                'PKS_API_HOST', 'PKS_API_PORT', 'PKS_API_WORKERS',
                'These variables only apply to CLI mode',
            ],
            'tui_only_vars': [
                'PKS_TUI_MODE', 'PKS_TUI_STARTUP_YAML', 'PKS_TUI_SHARED_PROMPT',
                'PKS_TUI_MAX_LINES', 'PKS_TUI_MAX_RERENDERS_PER_SEC',
            ],
        },
        ...
        'connection': {
            ...
            'related_commands': [
                ...
                {'command': '/settings validate', 'description': 'Validate API keys'},
                {'command': '/env set PKS_SKIP_NETWORK_CHECK', 'description': 'Skip network checks (not recommended)'},
            ],
            'checks': [
                {
            ...
            'skip_check': {
                'variable': 'PKS_SKIP_NETWORK_CHECK',
    ...
    'de': {
        ...
        'ollama': {
            ...
            'docker_notes': {
                ...
                'description': 'Wenn Sie PKS in Docker ausführen, bezieht sich Localhost auf den Container, nicht auf Ihren Host',
                'solutions': {
        ...
        'streaming': {
            ...
            'description': 'Verständnis der PKS-Streaming-Optionen',
            'related_commands': [
                {'command': '/env set PKS_STREAM', 'description': 'LLM-Streaming umschalten'},
                {'command': '/env set PKS_TOOL_STREAM', 'description': 'Tool-Ausgabe-Streaming umschalten'},
            ],
            'variables': {
                'PKS_STREAM': {
        ...
        'parallel': {
            ...
            'related_commands': [
                ...
                {'command': '/env set PKS_PARALLEL', 'description': 'Anzahl paralleler Agenten setzen'},
                {'command': '/env set PKS_PARALLEL_AGENTS', 'description': 'Agentennamen für Parallelität setzen'},
            ],
            'variables': {
                'PKS_PARALLEL': 'Anzahl der parallelen Instanzen (1-20)',
                'PKS_PARALLEL_AGENTS': 'Kommagetrennte Agentenamen',
                'PKS_AUTO_RUN_PARALLEL': 'Auto-Start parallele Agenten',
            },
            'common_issues': [
                {
                ...
                },
                {
                    ...
                    'fix': 'Reduzieren Sie PKS_PARALLEL oder fügen Sie Verzögerungen hinzu',
                },
            ],
        },
        ...
        'memory': {
            ...
            'related_commands': [
                ...
                {'command': '/env set PKS_COMPACTED_MEMORY', 'description': 'Prompt-Einblendung aktivieren (true) oder deaktivieren (false)'},
            ],
            'requirements': [],
            'variables': {
                'PKS_COMPACTED_MEMORY': 'true/false — /compact-Zusammenfassungen in System-Prompts einfügen',
            },
            'setup_steps': [
                '/compact ausführen (oder TUI-Kompaktierung), um eine Zusammenfassung zu erzeugen',
                'PKS_COMPACTED_MEMORY=true setzen, damit neue Agenten die Zusammenfassung laden',
            ],
        },
        ...
        'tui': {
            ...
            'related_commands': [
                {'command': 'pks --tui', 'description': 'PKS im TUI-Modus starten'},
                {'command': '/env set PKS_TUI_MODE', 'description': 'TUI-Modus aktivieren oder deaktivieren'},
            ...
            'variables': {
                'PKS_TUI_MODE': 'TUI-Modus aktivieren',
                'PKS_TUI_MAX_LINES': 'Maximale Ausgabezeilen',
                'PKS_TUI_MAX_RERENDERS_PER_SEC': 'Rendering-Leistung',
            },
            'cli_only_vars': [
                'PKS_API_HOST', 'PKS_API_PORT', 'PKS_API_WORKERS',
        ...
        'connection': {
            ...
            'related_commands': [
                ...
                {'command': '/settings validate', 'description': 'API-Schlüssel prüfen'},
                {'command': '/env set PKS_SKIP_NETWORK_CHECK', 'description': 'Netzwerkprüfungen überspringen (nicht empfohlen)'},
            ],
            'checks': [
                {
    ...
    'fr': {
        ...
        'ollama': {
            ...
            'docker_notes': {
                ...
                'description': 'Lors de l\'exécution de PKS dans Docker, localhost fait référence au conteneur, pas à votre hôte',
                'solutions': {
        ...
        'streaming': {
            ...
            'description': 'Comprendre les options de diffusion en continu de PKS',
            'related_commands': [
                {'command': '/env set PKS_STREAM', 'description': 'Activer ou désactiver le streaming LLM'},
                {'command': '/env set PKS_TOOL_STREAM', 'description': 'Activer ou désactiver le streaming des outils'},
            ],
            'variables': {
                'PKS_STREAM': {
        ...
        'parallel': {
            ...
            'related_commands': [
                ...
                {'command': '/env set PKS_PARALLEL', 'description': 'Définir le nombre d\'agents parallèles'},
                {'command': '/env set PKS_PARALLEL_AGENTS', 'description': 'Noms d\'agents pour le parallèle'},
            ],
            'variables': {
                'PKS_PARALLEL': 'Nombre d\'instances parallèles (1-20)',
                'PKS_PARALLEL_AGENTS': 'Noms d\'agents séparés par des virgules',
                'PKS_AUTO_RUN_PARALLEL': 'Démarrage automatique des agents parallèles',
            },
            'common_issues': [
                {
                ...
                },
                {
                    ...
                    'fix': 'Réduisez PKS_PARALLEL ou ajoutez des délais',
                },
            ],
        },
        ...
        'memory': {
            ...
            'related_commands': [
                ...
                {'command': '/env set PKS_COMPACTED_MEMORY', 'description': 'Activer (true) ou désactiver (false) l\'injection dans le prompt'},
            ],
            'requirements': [],
            'variables': {
                'PKS_COMPACTED_MEMORY': 'true/false — injecter les résumés /compact dans le prompt système',
            },
            'setup_steps': [
                'Exécuter /compact (ou compaction TUI) pour créer un résumé',
                'Définir PKS_COMPACTED_MEMORY=true pour que les nouveaux agents chargent le résumé',
            ],
        },
        ...
        'tui': {
            ...
            'related_commands': [
                {'command': 'pks --tui', 'description': 'Démarrer PKS en mode TUI'},
                {'command': '/env set PKS_TUI_MODE', 'description': 'Activer ou désactiver le mode TUI'},
            ...
            'variables': {
                'PKS_TUI_MODE': 'Activer le mode TUI',
                'PKS_TUI_MAX_LINES': 'Lignes de sortie maximales',
                'PKS_TUI_MAX_RERENDERS_PER_SEC': 'Performance de rendu',
            },
            'cli_only_vars': [
                'PKS_API_HOST', 'PKS_API_PORT', 'PKS_API_WORKERS',
        ...
        'connection': {
            ...
            'related_commands': [
                ...
                {'command': '/settings validate', 'description': 'Valider les clés API'},
                {'command': '/env set PKS_SKIP_NETWORK_CHECK', 'description': 'Ignorer les vérifications réseau (non recommandé)'},
            ],
            'checks': [
                {
    ...
    'ko': {
        ...
        'ollama': {
            ...
            'docker_notes': {
                'title': 'Docker 특정 구성',
                'description': 'Docker에서 PKS를 실행할 때 localhost는 호스트가 아니라 컨테이너를 의미합니다',
                'solutions': {
        ...
        'streaming': {
            'title': '스트리밍 구성',
            'description': 'PKS 스트리밍 옵션 이해',
            'related_commands': [
                {'command': '/env set PKS_STREAM', 'description': 'LLM 스트리밍 켜기/끄기'},
                {'command': '/env set PKS_TOOL_STREAM', 'description': '도구 출력 스트리밍 켜기/끄기'},
            ],
            'variables': {
                'PKS_STREAM': {
        ...
        'parallel': {
            ...
            'related_commands': [
                ...
                {'command': '/env set PKS_PARALLEL', 'description': '병렬 에이전트 수 설정'},
                {'command': '/env set PKS_PARALLEL_AGENTS', 'description': '병렬용 에이전트 이름 지정'},
            ],
            'variables': {
                'PKS_PARALLEL': '병렬 인스턴스 수 (1-20)',
                'PKS_PARALLEL_AGENTS': '쉼표로 구분된 에이전트 이름',
                'PKS_AUTO_RUN_PARALLEL': '병렬 에이전트 자동 시작',
            },
            'common_issues': [
                {
        ...
        'memory': {
            ...
            'related_commands': [
                ...
                {'command': '/env set PKS_COMPACTED_MEMORY', 'description': '프롬프트 삽입 켜기(true) 또는 끄기(false)'},
            ],
            'requirements': [],
            'variables': {
                'PKS_COMPACTED_MEMORY': 'true/false — /compact 요약을 시스템 프롬프트에 삽입',
            },
            'setup_steps': [
                '/compact(또는 TUI 압축)으로 요약 생성',
                'PKS_COMPACTED_MEMORY=true로 새 에이전트가 요약을 로드하도록 설정',
            ],
        },
        ...
        'tui': {
            ...
            'related_commands': [
                {'command': 'pks --tui', 'description': 'TUI 모드로 PKS 시작'},
                {'command': '/env set PKS_TUI_MODE', 'description': 'TUI 모드 켜기/끄기'},
            ...
            'variables': {
                'PKS_TUI_MODE': 'TUI 모드 활성화',
                'PKS_TUI_MAX_LINES': '최대 출력 라인',
                'PKS_TUI_MAX_RERENDERS_PER_SEC': '렌더링 성능',
            },
            'cli_only_vars': [
                'PKS_API_HOST', 'PKS_API_PORT', 'PKS_API_WORKERS',
        ...
        'connection': {
            ...
            'related_commands': [
                ...
                {'command': '/settings validate', 'description': 'API 키 검증'},
                {'command': '/env set PKS_SKIP_NETWORK_CHECK', 'description': '네트워크 검사 건너뛰기(비권장)'},
            ],
            'checks': [
                {
                    'check': '인터넷 연결',
