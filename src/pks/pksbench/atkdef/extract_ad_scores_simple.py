...
# PKS colors from the LaTeX document
PKS_PRIMARY = '#4C9A99'  # Main PKS color - lighter teal
TIE_COLOR = '#335757'    # Dark teal for ties
ENEMY_COLOR = '#D6D6D6'  # Lighter gray for opponent
RED_LINE = '#D9534F'     # Softer red for central line (pks_danger from main.tex)
...
def create_score_visualization(results, matchup, team1_name, team2_name, output_file):
    ...
    for i, d in enumerate(data):
        ...
        if d['win_pct'] > 0:
            bars1 = ax.barh(i, d['win_pct'], color=PKS_PRIMARY, height=0.75)
    ...
    # Add central vertical line at 50% in PKS color (dashed for subtlety)
    ax.axvline(x=50, color=PKS_PRIMARY, linewidth=1.5, linestyle='--', alpha=0.6, zorder=0)

    # Legend with bigger font and fancy styling (outside the plot area)
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor=PKS_PRIMARY, label=team1_name),
...
def create_overall_summary(results, matchup, team1_name, team2_name, output_file):
    ...
    # Win portion (team 1)
    if win_pct > 0:
        bars1 = ax.barh(y_pos, win_pct, color=PKS_PRIMARY, height=0.75)
        # Add subtle shadow effect
        for bar in bars1:
    ...
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor=PKS_PRIMARY, label=f'{team1_name} Win ({wins})'),
...
def create_combined_visualization(results, matchup, team1_name, team2_name, output_file):
    ...
    for i, d in enumerate(data):
        if d['win_pct'] > 0:
            bars1 = ax1.barh(i, d['win_pct'], color=PKS_PRIMARY, height=0.75)
            # Add subtle shadow effect
            for bar in bars1:
                bar.set_path_effects([patheffects.SimplePatchShadow(offset=(2, -2), shadow_rgbFace='black', alpha=0.3),
                                     patheffects.Normal()])
    ...
    ax1.set_xticklabels([])
    ax1.axvline(x=50, color=PKS_PRIMARY, linewidth=1.5, linestyle='--', alpha=0.6, zorder=0)
...
def create_paginated_score_bars(matchups: List[Dict[str, object]], output_dir: Path, items_per_page: int = 10):
    ...
    for page_num in range(total_pages):
        ...
        for ax_idx, entry in enumerate(page_matchups):
            ...
            for i, d in enumerate(data):
                if d['win_pct'] > 0:
                    bars1 = ax.barh(i, d['win_pct'], color=PKS_PRIMARY, height=0.5)  # Thinner bars
                    for bar in bars1:
                        bar.set_path_effects([patheffects.SimplePatchShadow(offset=(2, -2), shadow_rgbFace='black', alpha=0.3),
                                             patheffects.Normal()])
                    if d['win_pct'] > 8:
...
def create_overall_summary(matchups: List[Dict[str, object]], output_file, team1_name: str, team2_name: str):
    ...
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor=PKS_PRIMARY, label=f'{team1_name} Win ({total_wins})'),
