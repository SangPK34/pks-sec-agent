...
class GameServer:
    ...
    def __init__(self, config_path: str = "ad_config.yml"):
        ...
        # PKS textual servers
        self.pks_servers = {}  # {team_id: {machine_name: {'process': subprocess.Popen, 'port': int, 'status': str}}}
        self.pks_base_port = 23000  # Base port for PKS servers (23000+)
    ...
    def generate_flag(self, team_id: int, service: str) -> str:
        ...
        return f"PKS_CTF{{{flag_hash}_{random_part}}}"
...
{challenge_instructions}"""
    ...
    def initialize_game(self):
        ...
        # Start PKS servers after initial checks are complete (distributed mode only)
        # TODO: Add centralized mode support - start single PKS server per team using agents_team.yml
        if agent_mode == 'distributed':
            self.logger.info("Starting PKS textual servers for all teams (distributed mode)...")
            try:
                self.start_pks_servers()
                self.logger.info("All PKS servers started successfully")
            except Exception as e:
                self.logger.error(f"Failed to start PKS servers: {e}")
                self.stop_game(reason='pks_server_startup_failed')
                raise
        else:
            self.logger.info("Centralized mode: PKS servers must be started manually using agents_team.yml files")
            # TODO: Implement automatic PKS server startup for centralized mode
    ...
    def start_pks_servers(self):
        """Start PKS textual servers for all team/machine combinations (distributed mode)
        ...
        for team_id in self.teams:
            if team_id not in self.pks_servers:
                self.pks_servers[team_id] = {}

            for machine_idx, machine_name in enumerate(self.machines):
                # Calculate port: 8000 + (team_id * 10) + machine_idx
                port = self.pks_base_port + (team_id * 10) + machine_idx

                # Build the YAML path
                yaml_path = f"team_{team_id}/agents_{machine_name}.yml"

                self.logger.info(f"Starting PKS server for Team {team_id} {machine_name} on port {port}")
                ...
                try:
                    ...
                    # Store server info
                    # Use 127.0.0.1 for PKS servers (they bind to 0.0.0.0 but access via localhost)
                    self.pks_servers[team_id][machine_name] = {
                        'process': process,
                        'port': port,
                        'status': 'running',
                    ...
                    if process.poll() is None:
                        self.logger.info(f"✅ PKS server running for Team {team_id} {machine_name} on port {port}")
                    else:
                        _, stderr = process.communicate()
                        error_msg = stderr.decode() if stderr else "Unknown error"
                        self.logger.error(f"❌ PKS server for Team {team_id} {machine_name} failed to start")
                        self.logger.error(f"  Error: {error_msg[:200]}")
                        self.pks_servers[team_id][machine_name]['status'] = 'failed'
                        raise Exception(f"PKS server failed to start for Team {team_id} {machine_name}: {error_msg}")

                except Exception as e:
                    self.logger.error(f"Failed to start PKS server for Team {team_id} {machine_name}: {e}")
                    self.pks_servers[team_id][machine_name] = {
                        'process': None,
                        'port': port,
                        'status': 'failed',
    ...
    def stop_pks_servers(self):
        """Stop all PKS textual servers

        TODO: Add centralized mode support to stop team-level servers
        """
        import subprocess as sp

        self.logger.info("Stopping all PKS servers...")

        for team_id, machines in self.pks_servers.items():
            for machine_name, server_info in machines.items():
                if server_info.get('process'):
                    try:
                        process = server_info['process']
                        if process.poll() is None:  # Still running
                            self.logger.info(f"Stopping PKS server for Team {team_id} {machine_name}")
                            process.terminate()
                            try:
                                process.wait(timeout=5)
                            except sp.TimeoutExpired:
                                self.logger.warning(f"PKS server for Team {team_id} {machine_name} didn't stop gracefully, killing...")
                                process.kill()
                                process.wait()
                        server_info['status'] = 'stopped'
                    except Exception as e:
                        self.logger.error(f"Error stopping PKS server for Team {team_id} {machine_name}: {e}")

        self.logger.info("All PKS servers stopped")
    ...
    def _load_ctf_configs(self) -> Dict:
        ...
        try:
            with open(config_path, 'r') as f:
                data = json.load(f)
                for ctf in data:
                    if ctf.get('pksbench') == 'attack_defense':
                ...
                for team_id, team in self.teams.items():
                    ...
                    for machine_name in machine_names:
                        if machine_name in team.get('machines', {}):
                            ...
                            # Get PKS server info for this machine
                            pks_server_info = None
                            if team_id in self.pks_servers and machine_name in self.pks_servers[team_id]:
                                pks_info = self.pks_servers[team_id][machine_name]
                                ...
                                pks_server_info = {
                                    'port': pks_info.get('port'),
                                    'status': pks_info.get('status', 'unknown'),
                                    'url': pks_info.get('url'),
                                    'model': team_model
                                }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='PKS CTF Attack/Defense Game Server')
