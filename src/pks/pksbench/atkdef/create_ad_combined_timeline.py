...
# PKS colors - matching main.tex color definitions - professional light theme
PKS_PRIMARY = '#4C9A99'     # pks_primary - Main PKS teal
PKS_PRIMARY_LIGHT = '#7DCCCA'  # Lighter teal for gradients
PKS_PRIMARY_DARK = '#3A7876'   # Darker teal for gradients
TEXT_DARK = '#2C2C2C'       # Dark gray for text (more readable)
ENEMY_COLOR = '#BDBDBD'     # Medium gray for opponent
STATUS_OK = '#4C9A99'       # pks_color - service operational (PKS teal)
STATUS_OK_LIGHT = '#8FD8D6' # Lighter for gradient
STATUS_MUMBLE = '#F0AD4E'   # pks_warning - service degraded (amber)
STATUS_MUMBLE_LIGHT = '#F7CA8F'  # Lighter amber for gradient
STATUS_DOWN = '#D9534F'     # pks_danger - service down (red)
...
def create_combined_timeline(
    ...
    for machine_name in ordered_machines:
        ...
        for flag in data['flag_events']:
            ...
            # Determine colors and markers based on flag type (team 1 uses PKS teal palette)
            if flag_type == 'User':
                if attacker == 1:
                    flag_color = PKS_PRIMARY  # Main teal for team 1 user flag
                    border_color = PKS_PRIMARY_LIGHT  # Lighter teal for border
                ...
                if attacker == 1:
                    flag_color = PKS_PRIMARY_DARK  # Darker teal for team 1 root flag
    ...
    # Flags
    flags_elements = [
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor=PKS_PRIMARY,
                   markeredgecolor=PKS_PRIMARY_LIGHT, markersize=10, label=f'{team1_name} User flag', markeredgewidth=2),
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor=PKS_PRIMARY_DARK,
    ...
    for machine_name, positions in machine_positions.items():
        txt1 = ax.text(max_duration + 0.5, positions.get(1, 0), team1_name,
                ha='left', va='center', fontsize=20, color=PKS_PRIMARY_DARK,
