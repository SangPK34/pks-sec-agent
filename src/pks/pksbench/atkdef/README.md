<div align="center">
  <h1><img src="static/pks.svg" alt="PKS Logo" width="60"/>
  PKSBench Attack/Defense CTF Game Server</h1>
...
The PKSBench Attack/Defense CTF Game Server is a sophisticated platform for running Attack/Defense Capture The Flag competitions. It manages team containers, automated service checking, real-time scoring, and provides a comprehensive dashboard for monitoring game progress.
...
cd ../src/pks/pksbench/atkdef

# Review available CTF challenges
cat ../ctf-jsons/ctf_configs.jsonl | grep -E '"name"|"pksbench".*attack_defense'
...
# Team 1
pks --yaml ./src/pks/pksbench/atkdef/team_1/agents.yml --tui

# Team 2
pks --yaml ./src/pks/pksbench/atkdef/team_2/agents.yml --tui

# Team 3 (if configured)
pks --yaml ./src/pks/pksbench/atkdef/team_3/agents.yml --tui
...
git clone https://github.com/SangPK34/pks-sec-agent.git && cd pks-sec-agent
...
docker network ls | grep pksnet || \
  docker network create devcontainer_pksnet --subnet=192.168.3.0/24
```

3. **Pull CTF Images** (if not available locally) using the image configured for
...
# Game Configuration
game:
  name: "PKS CTF Attack/Defense Competition"
...
network:
  ...
  network_name: "devcontainer_pksnet"
...
# Flag Configuration
flags:
  format: "PKS_CTF{[A-Z0-9_]+}" # Flag validation regex
...
curl -X PUT http://localhost:12345/api/submit_flag \
  ...
  -d '{"team_id": 1, "flag": "PKS_CTF{CAPTURED_FLAG}"}'
```
...
# Inspect network
docker network inspect devcontainer_pksnet
...
#### Network Not Found
```bash
Error: network devcontainer_pksnet not found
Solution: docker network create devcontainer_pksnet --subnet=192.168.3.0/24
```
