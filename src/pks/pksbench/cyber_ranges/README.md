...
```bash
# From the PKS repository root
cd src/pks/pksbench/cyber_ranges/<cyber-range-name>

# For example:
cd src/pks/pksbench/cyber_ranges/easy_techcorp2
# or
cd src/pks/pksbench/cyber_ranges/CobaltGroupRansomware
```
...
## Configuring PKS for Cyber Ranges

### Setting the Active Container

When working with cyber ranges, PKS agents operate from within a development container that serves as the "attacker position." You need to configure which container PKS should use.
...
1. **Development container network** (required for attack position)
   ...
   # The devcontainer_pksnet network should exist
   docker network ls | grep pksnet

   # If not present, some cyber ranges will create it automatically
   # or you may need to create it:
   docker network create --driver bridge --subnet 192.168.3.0/24 devcontainer_pksnet
   ```
...
2. **Setting the attacker position**
   ...
   #in .env add
   PKS_ACTIVE_CONTAINER="[CONTAINER ID]"
   ```

3. **PKS environment set up**
   ```bash
   # From the pks repository root
   source pks_env/bin/activate

   #start pks as normal and prompt to agents
   ```
