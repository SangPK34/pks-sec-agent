...
def generate_benchmark_tables_by_pksbench():
    """
    Returns a dict mapping pksbench values to markdown tables for each benchmark.
    pksbench values: 'cybersec', 'base', 'rctf2', 'cyber_range'
    """
    # Map pksbench values to display names and README section markers
    pksbench_map = {
        "cybench": {
    ...
    pksbench_tables = {k: "" for k in pksbench_map}
    ctf_names = set()
    try:
        with open(os.path.join(ctf_jsons_folder, 'ctf_configs.jsonl'), 'r') as f:
            ctf_configs = json.load(f)
            # Group CTFs by pksbench value
            pksbench_ctfs = {k: [] for k in pksbench_map}
            for ctf in ctf_configs:
                ...
                if str(ctf.get("works", "")).lower() != "true":
                    continue
                pksbench_val = ctf.get('pksbench', '').strip().lower()
                if pksbench_val in pksbench_map:
                    pksbench_ctfs[pksbench_val].append(ctf)
                ctf_names.add(name)
            # Sort each list by difficulty
            difficulty_order = {'very easy': 1, 'easy': 2, 'medium': 3, 'hard': 4, 'very hard': 5}
            for pksbench_val, ctf_list in pksbench_ctfs.items():
                ...
                if ctf_list:
                    ...
                    pksbench_tables[pksbench_val] = md
    except (json.JSONDecodeError, FileNotFoundError) as e:
        print(f"Error loading CTF configs: {str(e)}")
    return pksbench_tables

def update_readme_with_benchmark_tables():
    """
    For each pksbench value, finds the corresponding <details> section in README.md and
    inserts the generated table for that benchmark.
    """
    pksbench_tables = generate_benchmark_tables_by_pksbench()
    # Map pksbench values to section markers in README.md
    section_titles = {
    ...
    try:
        ...
        new_content = content
        for pksbench_val, table_md in pksbench_tables.items():
            section_title = section_titles[pksbench_val]
...
def update_main_benchmarks_readme():
    ...
    This function targets the main project README instead of the pksbench-specific one.
    """
    pksbench_tables = generate_benchmark_tables_by_pksbench()
    # Map pksbench values to section markers in README.md
    section_titles = {
