...
Contains:
  ...
  - run_pks_cli()  -- the interactive conversation loop
"""
...
from pks import is_pentestperf_available
from pks.internal.components.metrics import process_metrics
from pks.repl.commands import get_fuzzy_completer, handle_command as commands_handle_command
import pks.repl.commands.exit as _repl_exit_cmd
...
from pks.repl.ui.logging import setup_session_logging
from pks.repl.ui.prompt import consume_repl_stdin_exhausted, get_user_input
from pks.repl.ui.toolbar import get_toolbar_with_refresh
from pks.sdk.agents import Runner, set_tracing_disabled
from pks.sdk.agents.items import ToolCallOutputItem
from pks.sdk.agents.exceptions import (
    OutputGuardrailTripwireTriggered,
    InputGuardrailTripwireTriggered,
    MaxTurnsExceeded,
    UserCancelledCommand,
)
from pks.sdk.agents.run_to_jsonl import get_session_recorder
from pks.sdk.agents.stream_events import RunItemStreamEvent
from wasabi import color
from pks.util.hint_renderables import build_pks_markup_line, build_startup_hint_renderable
from pks.util import (
...
from pks.config import DEFAULT_AGENT_TYPE, get_config as _get_config
from pks.errors import (
    LLMContextOverflow,
...
try:
    from pks.pksbench.ctf import CTFSetupError as _CTFSetupError
...
def _ctf_hotswap_failure_extra_hints(err: BaseException) -> str:
    """Rich markup (Spanish): cómo resolver fallos de registry / pull de imágenes PKSBench."""
    msg = str(err).lower()
    if type(err).__name__ != "CTFSetupError" and not any(
        ...
        return ""
    return (
        "\n\n[bold #ff3355]Cómo solucionarlo[/bold #ff3355]\n"
        "• Define [bold]PKSBENCH_IMG_REGISTRY_TOKEN[/bold] en tu [dim].env[/dim] o con "
        "[bold]export[/bold] antes de lanzar PKS: debe ser un [bold]token de GitLab[/bold] "
        "con permiso de lectura del registry ([dim]read_registry[/dim]) para "
        "[dim]registry.gitlab.com[/dim].\n"
        "• PKSBench usa usuario [dim]gitlab[/dim] y ese token como contraseña al hacer "
        "[dim]docker login[/dim] y [dim]docker pull[/dim] de la imagen del CTF.\n"
        "• Prueba el login a mano: "
        "[bold]echo \"$PKSBENCH_IMG_REGISTRY_TOKEN\" | docker login registry.gitlab.com "
...
def __getattr__(name: str):
    """Lazy ``START_TIME`` from :mod:`pks.util.cli_session_clock` (no import-time side effects)."""
    if name == "START_TIME":
        import pks.util.cli_session_clock as _clk
...
# Parallel execution summary table (CLI): horizontal rule only, PKS green; columns alternate fg
_PARALLEL_SUMMARY_GREEN = "#ff3355"
...
def _new_parallel_execution_summary_table() -> Table:
    """Horizontal rule under header (PKS green); no row backgrounds (zebra is text color only)."""
    return Table(
...
def _resolve_parallel_model_name(config_model: str | None) -> str:
    """Resolve a worker model from its override or the process default."""
    return (config_model or os.getenv("PKS_MODEL") or "gpt-5.6-terra").strip()
...
def run_pks_cli(
    ...
    *,
    ...
    """Run the interactive headless CLI loop for PKS.
    ...
    """
    from pks.util.cli_session_clock import reset_session_clock
    ...
    try:
        from pks.continuous_ops.loop_tick_epilogue import maybe_import_snapshot_before_cli_loop
    ...
    if queue_file:
        from pks.repl.commands.queue import load_queue_from_file
        ...
        if os.path.exists(queue_file):
            ...
            if loaded > 0:
                ...
                console.print(
                    build_pks_markup_line(
                        f"\n[#9aa0a6]Auto-loaded [/][bold #ff3355]{loaded}[/bold #ff3355]"
    ...
    def _single_shot_cli_active() -> bool:
        return os.getenv("PKS_SINGLE_SHOT_CLI", "").lower() in ("1", "true", "yes")

    def _bundled_stdin_prompt_text() -> str:
        path = (os.getenv("PKS_SINGLE_SHOT_STDIN_PROMPT_FILE") or "").strip()
        if path:
            try:
                return Path(path).expanduser().resolve().read_text(encoding="utf-8")
            except OSError:
                pass
        return (os.getenv("PKS_SINGLE_SHOT_STDIN_PROMPT", "") or "").strip()

    def _interactive_cli_input() -> str | None:
        """Return user line, or ``None`` if the REPL must exit (fatal single-shot without TTY)."""
        prompt_file_set = bool((os.getenv("PKS_SINGLE_SHOT_STDIN_PROMPT_FILE") or "").strip())
        loop_child = os.getenv("PKS_CONTINUOUS_OPS_LOOP_CHILD", "").lower() in ("1", "true", "yes")
        ...
        if _single_shot_cli_active() and not sys.stdin.isatty():
            console.print(
                ...
                "PKS_SINGLE_SHOT_STDIN_PROMPT_FILE / PKS_SINGLE_SHOT_STDIN_PROMPT / --prompt text is available. Exiting.[/bold red]"
            )
            return None
        try:
            from pks.repl.ui.terminal_title import set_terminal_activity
            set_terminal_activity(False)   # idle: waiting for the operator's next input
        except Exception:
            pass
        _val = get_user_input(
        ...
        try:
            from pks.repl.ui.terminal_title import set_terminal_activity
            set_terminal_activity(True)    # operator submitted -> a turn is about to run
        except Exception:
            pass
        return _val
    ...
    while True:
                ...
                try:
                    ...
                    console.print(
                        Panel(
                            ...
                            "[yellow]Se revirtió [bold]CTF_NAME[/bold] al valor anterior para que PKS "
                            "siga respondiendo. Si el nombre era incorrecto, usa un id del catálogo "
                            "PKSBench; si el fallo es de red o credenciales, corrígelo y vuelve a "
        ...
        # ---- max-turns check ----
        current_max_turns = os.getenv("PKS_MAX_TURNS", "inf")
        if current_max_turns != str(prev_max_turns):
            max_turns = float(current_max_turns)
            prev_max_turns = max_turns
            if turn_limit_reached and turn_count < max_turns:
                turn_limit_reached = False
                console.print("[green]Turn limit increased. You can now continue using PKS.[/green]")
        ...
        if turn_count >= max_turns and max_turns != float("inf"):
            if not turn_limit_reached:
                ...
                console.print("[yellow]You must increase the limit using: /env set PKS_MAX_TURNS <new_value>[/yellow]")
        ...
        current_max_interactions = os.getenv("PKS_MAX_INTERACTIONS", "inf")
        try:
            check_interaction_limit()
            interaction_limit_reached = False
            ...
            if not interaction_limit_reached:
                ...
                console.print("[yellow]You must increase the limit using: /env set PKS_MAX_INTERACTIONS <new_value>[/yellow]")
                console.print("[yellow]Only CLI commands (starting with '/') will be processed until the limit is increased.[/yellow]")
            if force_until_flag:
                return
        ...
        if interaction_limit_reached:
            ...
            console.print("[yellow]Please use /env to increase PKS_MAX_INTERACTIONS limit.[/yellow]")
        ...
        try:
            ...
            agent_specific_model = os.getenv(f"PKS_{last_agent_type.upper()}_MODEL")
            ...
            # Agent type hotswap
            current_agent_type = os.getenv("PKS_AGENT_TYPE", DEFAULT_AGENT_TYPE)
            ...
            if os.environ.get("PKS_AGENT_SWITCH_HANDLED") == "1":
                os.environ["PKS_AGENT_SWITCH_HANDLED"] = "0"
                from pks.sdk.agents.simple_agent_manager import AGENT_MANAGER
                ...
                if agent:
                    ...
                    if current_agent_type == "continuous_ops_agent":
                        os.environ.pop("PKS_CONTINUOUS_OPS_SETUP_DONE", None)
                else:
                ...
                os.environ["PKS_TASK_RESET_PENDING"] = "1"
                continue
                ...
                try:
                    ...
                    if hasattr(agent, "name"):
                        ...
                        if current_history:
                            ...
                            try:
                                from pks.util.session_compact import prepare_agent_handoff
                    ...
                    if hasattr(agent, "model"):
                        ...
                        if hasattr(agent.model, "suppress_final_output"):
                            agent.model.suppress_final_output = False
                        agent_specific_model = os.getenv(f"PKS_{current_agent_type.upper()}_MODEL")
            ...
            if not force_until_flag and _setup.ctf_init != 0:
                if use_initial_prompt and turn_count == 0:
                    ...
                    if os.getenv("PKS_AUTO_RUN_QUEUE") == "1":
                        auto_run_queue = True
                        use_initial_prompt = False
                elif auto_run_queue:
                    from pks.repl.commands.queue import get_queue, get_next_prompt
                    ...
                    if queue_items:
                        ...
                        if next_item:
                            if isinstance(next_item, dict):
                                ...
                                if item_agent:
                                    from pks.agents import get_available_agents as _get_agents
                                    _avail = _get_agents()
                    ...
                    if turn_count == 0 and os.getenv("PKS_AUTO_RUN_PARALLEL") == "1" and PARALLEL_CONFIGS:
                        user_input = ""
                        console.print(f"[dim white]Auto-running parallel agents with configured prompts...[/dim white]")
                        os.environ.pop("PKS_AUTO_RUN_PARALLEL", None)
                    else:
                        u = _interactive_cli_input()
                        if u is None:
                            return
            ...
            if not hasattr(_setup, "_last_exit_attempt"):
                _setup._last_exit_attempt = 0
            
            if current_time - _setup._last_exit_attempt > 2.0:
                key = "Ctrl+D/Z" if isinstance(e, EOFError) else "Ctrl+C"
                console.print(f"\n[bold yellow]Press {key} again within 2 seconds to exit PKS.[/bold yellow]")
                _setup._last_exit_attempt = current_time
                continue
        ...
        try:
            # Turn limit enforcement on non-command input
            if turn_limit_reached and not _user_input_is_cli_command_block(user_input):
                ...
                console.print("[yellow]Please use /env to increase PKS_MAX_TURNS limit.[/yellow]")
                stop_active_timer()
                start_idle_timer()
                _try_refresh_info_bars()
                continue
            ...
            if PARALLEL_CONFIGS and not _user_input_is_cli_command_block(user_input):
                exec_mode = os.getenv("PKS_PARALLEL_EXEC_MODE", "external").strip().lower()
                if exec_mode == "external":
                    console.print(
                        build_pks_markup_line(
                            "[#9aa0a6]Parallel mode is active (external terminals). "
                            "Use [/][bold #ff3355]/parallel run[/bold #ff3355][#9aa0a6] to launch workers, "
                            "[/][bold #ff3355]/parallel clear[/bold #ff3355][#9aa0a6] to exit parallel mode.[/]"
                        )
                    )
                    continue
                _run_parallel_turn(
            ...
            if (
                os.environ.get("PKS_TUI_MODE", "").lower() != "true"
                and not PARALLEL_CONFIGS
                and last_agent_type == "continuous_ops_agent"
                and not _user_input_is_cli_command_block(user_input)
            ):
                from pks.continuous_ops.wizard import maybe_intercept_continuous_ops_turn
            ...
            if _user_input_is_cli_command_block(user_input):
                ...
                from pks.repl.commands import handle_command_with_autocorrect
                ...
                for line in command_lines:
                    ...
                    if handled:
                        ...
                        import pks.repl.commands._parallel_monolith as _par_mod
                        ...
                        import pks.repl.commands.queue as _queue_mod
            ...
            if os.getenv("PKS_TASK_RESET_PENDING") == "1":
                from pks.util.session_compact import consume_agent_handoff
                ...
                os.environ["PKS_TASK_RESET_PENDING"] = "0"
            ...
            start_idle_timer()
            _try_refresh_info_bars()
            if os.getenv("PKS_SINGLE_SHOT_CLI", "").lower() in ("1", "true", "yes"):
                if os.getenv("PKS_CONTINUOUS_OPS_LOOP_CHILD", "").lower() in ("1", "true", "yes"):
                    try:
                        from pks.continuous_ops.loop_tick_epilogue import (
            ...
            try:
                import importlib
                continue_module = importlib.import_module('pks.repl.commands.continue')
            ...
            _try_refresh_info_bars()
            if (
                os.getenv("PKS_SINGLE_SHOT_CLI", "").lower() in ("1", "true", "yes")
                and os.getenv("PKS_CONTINUOUS_OPS_LOOP_CHILD", "").lower() in ("1", "true", "yes")
            ):
...
def _record_shared_turn_context(result) -> None:
    """Expose a bounded final exchange to a different specialist on a later turn."""
    if result is None:
        return
    try:
        last_agent = result.last_agent
        model = getattr(last_agent, "model", None)
...
def _run_streamed(agent, conversation_input, console, force_until_flag, ctf_global):
    ...
    try:
        completed_result = asyncio.run(process_streamed_response(agent, conversation_input))
        ...
        if "This event loop is already running" in str(e) or "Cannot close a running event loop" in str(e):
            ...
            try:
                completed_result = new_loop.run_until_complete(
...
# ---------------------------------------------------------------------------
# Helper: simple parallel (PKS_PARALLEL > 1)
# ---------------------------------------------------------------------------
...
def _run_simple_parallel(agent, conversation_input, parallel_count, last_agent_type, console, _update_fn):
    async def run_instance(instance_number, context):
        try:
            from pks.agents import get_available_agents, get_agent_by_name
            ...
            if hasattr(instance_agent, "model") and hasattr(agent, "model"):
                if hasattr(instance_agent.model, "model") and hasattr(agent.model, "model"):
                    instance_specific = os.getenv(f"PKS_{last_agent_type.upper()}_{instance_number + 1}_MODEL")
                    agent_specific = os.getenv(f"PKS_{last_agent_type.upper()}_MODEL")
...
def _run_parallel_turn(agent, user_input, console, configs, instances, last_agent_type, _update_fn):
    ...
    if os.environ.get("PKS_PARALLEL_MODE_HINT_SHOWN") != "1":
        console.print(
            build_pks_markup_line(
                "[#9aa0a6]Parallel mode is running in logical mode (single main terminal). "
                "Set [/][bold #ff3355]PKS_PARALLEL_EXEC_MODE=external[/bold #ff3355][#9aa0a6] to launch separate system terminals.[/]"
            )
        )
        os.environ["PKS_PARALLEL_MODE_HINT_SHOWN"] = "1"
    ...
    if not already_has_histories:
        ...
        pattern_description = os.getenv("PKS_PATTERN_DESCRIPTION", "")
    ...
    async def run_agent_instance(config, input_text):
        ...
        try:
            ...
            try:
                from pks.util import finish_tool_streaming, cli_print_tool_output, _LIVE_STREAMING_PANELS
            ...
            try:
                from pks.util import cleanup_agent_streaming_resources
                if instance_agent:
    ...
    old_stream = os.environ.get("PKS_STREAM")
    old_tool_stream = os.environ.get("PKS_TOOL_STREAM")
    os.environ["PKS_STREAM"] = "false"
    os.environ["PKS_TOOL_STREAM"] = "false"
    ...
    try:
        ...
        if old_stream is None:
            os.environ.pop("PKS_STREAM", None)
        else:
            os.environ["PKS_STREAM"] = old_stream
        if old_tool_stream is None:
            os.environ.pop("PKS_TOOL_STREAM", None)
        else:
            os.environ["PKS_TOOL_STREAM"] = old_tool_stream
    ...
    try:
        if backend == "gnome-terminal":
            subprocess.Popen(
                [
                    ...
                    "bash",
                    "-lc",
                    f"{command}; echo; echo '[PKS] Worker finished.'; read -r -p 'Press Enter to close...'",
                ],
        ...
        if backend == "osascript":
            osa_cmd = (
                ...
                + json.dumps(f"{command}; echo; echo '[PKS] Worker finished.'")
            )
            subprocess.Popen(
                ["osascript", "-e", osa_cmd],
                stdout=subprocess.DEVNULL,
...
def _run_parallel_turn_external(console, configs, user_input: str) -> None:
    ...
    if not backend:
        ...
        console.print(build_pks_markup_line(f"[#9aa0a6]{backend_hint}[/]"))
        console.print(
            build_pks_markup_line(
                "[#9aa0a6]Falling back to logical mode. "
                "Note: these are system-level requirements and are not installed via [/]"
                "[bold #ff3355]pyproject.toml[/bold #ff3355][#9aa0a6].[/]"
            )
        )
        old_mode = os.environ.get("PKS_PARALLEL_EXEC_MODE")
        os.environ["PKS_PARALLEL_EXEC_MODE"] = "logical"
        try:
            ...
            if old_mode is None:
                os.environ.pop("PKS_PARALLEL_EXEC_MODE", None)
            else:
                os.environ["PKS_PARALLEL_EXEC_MODE"] = old_mode
    ...
    _pe_timeout = float(os.getenv("PKS_PARALLEL_EXTERNAL_TIMEOUT", "1800"))
    console.print(
        build_pks_markup_line(
            "[dim]External parallel: the main CLI waits up to "
            f"{int(_pe_timeout)}s for each worker result file "
            "(PKS_PARALLEL_EXTERNAL_TIMEOUT). Workers may keep running in their terminals after that. "
            "Only completed worker results are included in the final merge.[/dim]"
        )
    )

    worker_specs = []
    tmp_dir = tempfile.mkdtemp(prefix="pks_parallel_")
    mcp_bootstrap_path: str | None = None
    try:
        from pks.repl.commands.mcp import export_parallel_mcp_bootstrap_dict
    ...
    for idx, config in enumerate(configs, 1):
        ...
        title = f"PKS Parallel {agent_id}"
        worker_py = (
            f"{shlex.quote(sys.executable)} -m pks.parallel_worker "
        ...
        if mcp_bootstrap_path:
            cmd = (
                f"export PKS_PARALLEL_MCP_BOOTSTRAP={shlex.quote(mcp_bootstrap_path)} && "
                f"{worker_py}"
            )
        else:
            cmd = worker_py
    ...
    timeout_s = float(os.getenv("PKS_PARALLEL_EXTERNAL_TIMEOUT", "1800"))
    start = time.time()
    ...
    if pending:
        console.print(
            build_pks_markup_line(
                ...
                "Increase [/][bold #ff3355]PKS_PARALLEL_EXTERNAL_TIMEOUT[/bold #ff3355][#9aa0a6] if needed.[/]"
            )
        )
    console.print(
        build_pks_markup_line(
    ...
    console.print(
        build_pks_markup_line(
            ...
            f"[/][bold #ff3355]/merge[/bold #ff3355][{_PARALLEL_SUMMARY_MUTED}] to consolidate results into the main context "
            f"and exit parallel mode automatically.[/]"
        )
    )
    console.print(
        build_pks_markup_line(
            f"[{_PARALLEL_SUMMARY_MUTED}]Or use [/][bold #ff3355]/parallel clear[/bold #ff3355]"
...
def _handle_exit_interrupt(agent, console, session_logger, idle_time, idle_start_time,
    ...
    try:
        from pks.util import cleanup_all_streaming_resources
    ...
    import pks.util.cli_session_clock as _session_clock
    ...
    try:
        if hasattr(agent.model, "_converter") and hasattr(agent.model._converter, "recent_tool_calls"):
            ...
            try:
                from pks.util import sanitize_message_list as fix
    ...
    # Session summary
    try:
        from pks.util import get_active_time_seconds, get_idle_time_seconds
        ...
        from pks.repl.ui.banner import PKS_GREEN, session_summary_panel_title
        ...
        # Keep the exit summary focused on time and saved session state.
        if logging_path:
        ...
        try:
            from pks.repl.ui.terminal_title import restore_terminal_window_title
...
def _handle_loop_exception(e, agent, console, force_until_flag):
    ...
    try:
        from pks.util import close_all_streaming_panels
    ...
    if isinstance(e, MaxTurnsExceeded):
        max_turns_val = os.getenv("PKS_MAX_TURNS", "unlimited")
        ...
        console.print("[dim white]You can continue with a new conversation or adjust PKS_MAX_TURNS if needed.[/dim white]\n")
        return

    cfg = _get_config()
    if isinstance(e, LLMEmptyAssistantError):
        try:
            from pks.util.streaming import cleanup_all_streaming_resources
        ...
        from pks.util.cli_palette import PKS_GREEN, FINAL_PANEL_BG, GREY_TEXT, YELLOW_WARN
        ...
        if isinstance(det, dict) and "attempts" in det:
            try:
                ...
                n = 3
        body = Text.assemble(
            (f"[bold {PKS_GREEN}]PKS[/bold {PKS_GREEN}]\n\n", ""),
            (
    ...
    if isinstance(e, (LLMProviderUnavailable, LLMTimeout, LLMRateLimited)):
        ...
        if cfg.debug == 2:
            ...
            console.print(
                "\n[bold yellow]The configured model endpoint is unavailable or rate-limited.[/bold yellow]"
            )
            console.print(
                "[dim]Check OPENAI_BASE_URL, endpoint health, and provider rate limits; "
                "then retry.[/dim]"
            )
            console.print(
                "[dim]Error details are not shown in the console; set PKS_DEBUG=2 only if you need a "
    ...
    if (
        ...
        if force_until_flag:
            ...
            from pks.repl.commands.base import handle_command as commands_handle_command
    ...
    try:
        from pks.repl.exception_recovery import (
            is_recovery_enabled,
        ...
        if (
            ...
            def _recovery_agent_run(hint: str, brief: str) -> None:
                from pks.repl.exception_recovery import build_recovery_agent_user_message
...
def _print_input_guardrail_warning(e):
    ...
    if "base64" in reason.lower() or "pattern" in reason.lower():
        ...
        print(f"  2. Type \033[92m/env set PKS_GUARDRAILS false\033[0m to temporarily disable guardrails")
        print(f"  3. Type \033[92m/exit\033[0m to exit PKS")
    else:
...
def _try_refresh_info_bars():
    try:
        from pks.tui.components.info_status_bar_updater import force_refresh_all_info_bars
