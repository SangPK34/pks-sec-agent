...
from pks.repl.ui.startup_hints import StartupHints, mask_key_for_hint
...
from pks.sdk.agents.global_usage_tracker import GLOBAL_USAGE_TRACKER
from pks.sdk.agents.stream_events import RunItemStreamEvent
from wasabi import color
from pks.util.hint_renderables import build_cai_markup_line, build_startup_hint_renderable
...
try:
    from pks.caibench.ctf import CTFSetupError as _CTFSetupError
...
def _resolve_parallel_model_name(config_model: str | None) -> str:
    ...
    env_model = (os.getenv("PKS_MODEL", "alias1") or "alias1").strip()
...
def run_cai_cli(
    ...
    *,
    ...
    # Reset cost tracking
    from pks.util import COST_TRACKER
    ...
    while True:
        ...
        # ---- price limit check ----
        current_price_limit = os.getenv("PKS_PRICE_LIMIT", "inf")
        try:
            ...
            if price_limit != float("inf") and COST_TRACKER.session_total_cost >= price_limit:
                ...
                if not hasattr(run_cai_cli, '_price_limit_warning_shown'):
                    ...
                    console.print("[yellow]You must increase the limit using: /env set PKS_PRICE_LIMIT <new_value>[/yellow]")
                    console.print("[yellow]Only CLI commands (starting with '/') will be processed until the limit is increased.[/yellow]")
                if force_until_flag:
                    return
        except Exception:
        ...
        if price_limit_reached:
            ...
            console.print("[yellow]Please use /env to increase PKS_PRICE_LIMIT limit.[/yellow]")
        ...
        try:
            ...
            # Model hotswap
            current_model = os.getenv("PKS_MODEL", "alias1")
...
def _run_parallel_turn_external(console, configs, user_input: str) -> None:
    ...
    from pks.util.pricing import COST_TRACKER
    from pks.sdk.agents.parallel_isolation import PARALLEL_ISOLATION
    from pks.sdk.agents.simple_agent_manager import AGENT_MANAGER
    ...
    console.print()
    console.print(summary_table)
    # Use the standard PKS pricing footer render (same style as normal flow),
    ...
    try:
        ...
        try:
            from pks.util.streaming import _print_pricing_footer
...
def _handle_exit_interrupt(agent, console, session_logger, idle_time, idle_start_time,
    ...
    # Session summary
    try:
        from pks.util import COST_TRACKER, get_active_time_seconds, get_idle_time_seconds
        ...
        # PKS build: session-cost line removed from the exit summary.
        if logging_path:
        ...
        os.environ["PKS_COST_DISPLAYED"] = "true"
...
def _handle_loop_exception(e, agent, console, force_until_flag):
    ...
    if isinstance(e, (LLMProviderUnavailable, LLMTimeout, LLMRateLimited)):
        ...
        if cfg.debug == 2:
            ...
            # User-facing copy only — no exception text or URLs (see PKS_DEBUG=2 for engineers).
            console.print(
    ...
    if isinstance(e, PriceLimitExceeded):
        ...
        console.print(
            ...
            "/env set PKS_PRICE_LIMIT <new_value>[/yellow]"
        )
        return
