"""
Configuration, environment, and setup utilities for PKS.
"""
...
if is_pentestperf_available():
    import pks.caibench as ptt


def get_config_dir() -> Path:
    """
    Returns the pks configuration directory, creating it if it doesn't exist.
    The directory is located at ~/.pks
    """
    config_dir = Path.home() / ".pks"
    config_dir.mkdir(parents=True, exist_ok=True)
    return config_dir


def get_session_logs_dir() -> Path:
    """Directory where :class:`pks.sdk.agents.run_to_jsonl.DataRecorder` writes session JSONL.

    Kept in sync with ``DataRecorder`` (``~/.pks/logs``). Used by ``/sessions``,
...
def get_debug_log_dir() -> Path:
    """Return ~/.pks/debug/, creating it if needed."""
    d = Path.home() / ".pks" / "debug"
    d.mkdir(parents=True, exist_ok=True)
    return d
...
def get_pricings_dir() -> Path:
    ...
    1. PKS_PRICINGS_DIR env var if set
    2. Package directory (pks/pricings - for pip-installed packages)
    3. Development directory (../../pricings from src/pks - for editable installs)
    ...
    override = os.getenv("PKS_PRICINGS_DIR")
    if override:
    ...
    try:
        package_dir = pathlib.Path(__file__).parent.parent  # src/pks/util -> src/pks
...
def _seed_pricings_dir() -> None:
    ...
    - For pip-installed packages, pricing.json should already be in pks/pricings/
...
def visualize_agent_graph(start_agent):
    ...
    Palette: brand green (``PKS_GREEN``), white body text, grey/dim chrome only —
    no blue/yellow/magenta/red in this tree so it matches headless REPL branding.
    """
    from rich.console import Console
    from rich.tree import Tree

    from pks.util.cli_palette import PKS_GREEN, GREY_HINT, GREY_TEXT
    ...
    root_label = (
        f"[bold {PKS_GREEN}]Agent:[/bold {PKS_GREEN}] "
    ...
    def add_agent_node(agent, parent=None, is_transfer=False):
        ...
        if is_transfer and parent:
            ...
            node = parent.add(
                f"[bold {PKS_GREEN}]{agent.name}[/bold {PKS_GREEN}] "
        ...
        # Add tools
        tools_node = node.add(f"[bold {PKS_GREEN}]Tools[/bold {PKS_GREEN}]")

        # Get all tools from the agent
        all_tools = getattr(agent, "tools", [])

        # Import necessary modules for MCP checking
        from pks.repl.commands.mcp import get_mcp_tools_for_agent, _GLOBAL_MCP_SERVERS
        ...
        # Add handoffs
        transfers_node = node.add(f"[bold {PKS_GREEN}]Handoffs[/bold {PKS_GREEN}]")
            ...
            if callable(handoff_fn) and not hasattr(handoff_fn, "agent_name"):
                ...
                try:
                    ...
                    for module_name, module in sys.modules.items():
                        if module_name.startswith("pks.agents"):
...
def setup_ctf():
    ...
    # Only set PKS_ACTIVE_CONTAINER if CTF_INSIDE is true
