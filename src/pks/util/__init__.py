...
that were previously available from the monolithic ``pks.util`` module.
Existing code such as::

    from pks.util import COST_TRACKER, cli_print_tool_output, load_prompt_template
...
# ---------------------------------------------------------------------------
from pks.util.config_utils import (
    get_config_dir,
...
# (which would re-import pks.util → circular).  We load token_counter.py
...
def _load_token_counter():
    ...
    _here = pathlib.Path(__file__).resolve().parent        # pks/util/
    _file = _here.parent / "sdk" / "agents" / "models" / "chatcompletions" / "token_counter.py"
    spec = importlib.util.spec_from_file_location(
        "pks.sdk.agents.models.chatcompletions.token_counter", str(_file)
    )
...
def __getattr__(name: str):
    if name in _LAZY_REEXPORTS:
        ...
        return val
    raise AttributeError(f"module 'pks.util' has no attribute {name!r}")
