"""
Cost tracking, pricing lookup, and financial utilities for PKS.
"""
...
from pks.util.tokens import get_model_name, get_model_input_tokens
from pks.util.config_utils import get_pricings_dir, _seed_pricings_dir
from pks.util.interaction import get_interaction_counter
from pks.util.session import is_parallel_session

# ============== PRICING DEBUG LOGGER ==============
# Set PKS_DEBUG_PRICING=1 to enable runtime pricing debug logs
...
def _pricing_debug_log(step: str, **kwargs):
    ...
    if os.getenv("PKS_DEBUG_PRICING", "0") != "1":
        return
    ...
    with _PRICING_DEBUG_LOCK:
        try:
            if _PRICING_DEBUG_FILE is None:
                ...
                _PRICING_DEBUG_FILE.write(f"PKS PRICING DEBUG SESSION - {datetime.now().isoformat()}\n")
...
def _ensure_pricing_prefetch_started() -> None:
    ...
    if os.getenv("PKS_ENABLE_PRICING_FETCH", "0").lower() not in ("1", "true", "yes"):
        return
...
class CostTracker:
    ...
    def _warn_unattributed(self, message: str, **context: Any) -> None:
        ...
        Note: Warnings are disabled by default. Set PKS_WARN_UNATTRIBUTED=1 to enable.
        """
        # Warnings disabled - return early
        if not os.environ.get("PKS_WARN_UNATTRIBUTED"):
            return
        ...
        try:
            ...
            log_path = Path(os.environ.get("PKS_UNATTRIBUTED_LOG", Path.home() / ".cai_unattributed.log"))
    ...
    def check_price_limit(self, new_cost: float) -> None:
        ...
        from pks.sdk.agents.exceptions import PriceLimitExceeded

        price_limit_env = os.getenv("PKS_PRICE_LIMIT")
        try:
    ...
    def log_final_cost(self) -> None:
        """PKS build: cost display disabled — never print a session-cost line at exit."""
        return
    ...
    def get_model_pricing(self, model_name: str, *, allow_async: bool = True) -> tuple:
        ...
        Enhancements:
        - Busca pricing local en ./pricings/pricing.json (o ruta en PKS_PRICING_FILE).
        ...
        try:
            pricing_file_env = os.getenv("PKS_PRICING_FILE")
        ...
        if os.getenv("PKS_ENABLE_PRICING_FETCH", "0").lower() not in ("1", "true", "yes"):
            ...
            _pricing_debug_log("GET_MODEL_PRICING: USING DEFAULT (0.0, 0.0)",
                reason="PKS_ENABLE_PRICING_FETCH not enabled")
        ...
        if allow_async:
            ...
            wait_env = os.getenv("PKS_PRICING_ASYNC_WAIT", "").strip()
            wait_seconds = 0.0
            if wait_env:
                try:
            ...
            if not self.pricing_fetch_warned and not _PRICING_PREFETCH_EVENT.is_set():
                ...
                if os.getenv("PKS_PRICING_VERBOSE", "0").lower() in ("1", "true", "yes"):
                    print(
...
def enrich_token_info_for_pricing(
    ...
    *,
    ...
    is_multi_agent = os.getenv("PKS_TUI_MODE") == "true" or is_parallel_session()
    ...
    model_name = get_model_name(
        enriched.get("model") or default_model or os.environ.get("PKS_MODEL", "")
    )
