"""
Terminal output formatting and display utilities for PKS.
"""
...
def _session_wall_seconds() -> float:
    ...
    try:
        from pks.util.cli_session_clock import START_TIME as _start
...
def _tool_capture_output_looks_like_markdown(raw: str) -> bool:
    ...
    grey rail. Enable with PKS_TOOL_OUTPUT_MARKDOWN=true (default).
    """
    if not raw or not isinstance(raw, str) or len(raw.strip()) < 12:
        return False
    if os.getenv("PKS_TOOL_OUTPUT_MARKDOWN", "true").lower() in ("0", "false", "no"):
        return False
...
def _layout1_combined_header(
    ...
    running in parallel (``PKS_PARALLEL > 1``); the static ``AGENT`` sufix has
    been retired as redundant noise.
    """
    from pks.util.cli_palette import PKS_GREEN
    ...
    t.append("● ", style="bold #ff3355")
    if agent_name:
        t.append(agent_name, style="bold #ff3355")
        aid = ""
        if token_info and isinstance(token_info, dict):
        ...
        t.append("Agent", style=f"bold {PKS_GREEN}")
        t.append(" ─ ", style="dim white")
    ...
    t.append(" TOOL", style=f"italic {PKS_GREEN}")
    t.append("   ")
    t.append_text(pills)
    return t
...
def _create_token_info_display(token_info=None):
    ...
    from pks.util.tokens import _create_token_display
...
def _prepare_tool_output_for_display(tool_name, output):
    """Return a UI-only preview without changing the tool result used by the model."""
    if output is None:
        return output

    ui_display_output = output if isinstance(output, str) else str(output)

    lines = ui_display_output.splitlines()
    max_lines = 40
    omitted = max(0, len(lines) - max_lines)
    marker_lines = []
    if len(ui_display_output) > 10000:
        marker_lines.append("... TRUNCATED ...")
    if omitted:
        marker_lines.append(f"  ... ({omitted} lines omitted) ...")
    marker = "\n".join(marker_lines)
    ...
    if len(lines) > max_lines:
        ...
        head_text = "\n".join(head)
        tail_text = "\n".join(tail)
        ui_display_output = f"{head_text}\n{marker}\n{tail_text}"

    if len(ui_display_output) > 10000:
        wrapped_marker = f"\n\n{marker}\n\n"
        content_budget = 10000 - len(wrapped_marker)
        ...
        if omitted:
            ...
            ui_display_output = (
                ui_display_output[:head_size]
                + wrapped_marker
                + ui_display_output[-tail_size:]
            )

    return ui_display_output
...
def _format_tool_args(args, tool_name=None):
    ...
    if isinstance(args, dict):
        ...
        if tool_name == "generic_linux_command":
            ...
            source_label = {"llm": "llm", "env:PKS_TOOL_TIMEOUT": "env", "default": "default"}.get(source, source)
...
def _print_simple_tool_output(tool_name, args, output, execution_info=None, token_info=None):
    ...
    Token details are omitted from tool output panels.
    """
    # Format arguments
...
def _create_tool_panel_content(
    ...
    *,
    ...
    from pks.util.cli_palette import PKS_GREEN, GREY_HINT, GREY_TEXT, PIPE_GREY

    ui_display_output = _prepare_tool_output_for_display(tool_name, output)

    # Sanitize output; strip trailing newlines so we do not render blank lines under tool output
    if ui_display_output:
        ui_display_output = _sanitize_output_for_display(ui_display_output)
        ui_display_output = ui_display_output.rstrip("\n\r")
    ...
    # Add vertical spacing above the command
    body_parts.append(Text("\n"))
    
    # Print the command header
    body_parts.append(Text("  >> COMMAND:", style="bold medium_purple"))
    
    if cmd_display and cmd_display != "(no command)":
        from rich.markdown import Markdown
        from rich.padding import Padding
        # Wrap the command in a Markdown bash block so it renders exactly like normal markdown
        md_text = f"```bash\n{cmd_display}\n```"
        body_parts.append(Padding(Markdown(md_text, justify="left"), (1, 0, 1, 3)))
    else:
        body_parts.append(Text("       (no command)", style="dim white"))
        
    # Add vertical spacing below the command
    body_parts.append(Text(""))
    ...
    if include_tool_wait_hint:
        try:
            from pks.util.wait_hints import get_tool_wait_body_plain
        ...
        if wait_msg and str(wait_msg).strip():
            ...
            out_first = ""
            if ui_display_output and ui_display_output.strip():
                out_first = ui_display_output.strip().split("\n", 1)[0].strip()
            if out_first != wstrip:
    ...
    if ui_display_output and ui_display_output.strip():
        lines = ui_display_output.split("\n")
        while lines and lines[-1].strip() == "":
            lines.pop()
        # Command/tool output is raw text (code, hex, file listings) — render it as plain,
        # left-aligned, wrapped monospace. NEVER through Rich Markdown, which centre/box-frames
        # headings and mangles "===", tables and pipes in real command output.
        for line in lines:
            body_parts.extend(
                _wrap_line_for_tool_rail(line, term_w, pipe_style, GREY_TEXT)
            )

    body_parts.append(Text(""))  # trailing blank line so the next block doesn't butt up against the output
    ...
    if tool_calls:
        ...
        for tool_call in tool_calls:
            ...
            if tool_name and tool_output:
                ...
                from pks.util.cli_palette import GREY_TEXT, PIPE_GREY
