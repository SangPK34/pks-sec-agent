...
def is_parallel_session() -> bool:
    ...
    Mirrors the convention previously inlined in :mod:`pks.util.streaming`,
    :mod:`pks.repl.ui.compact_renderer`, :mod:`pks.util.terminal`, and
    the active runtime session. Honours ``PKS_PARALLEL`` and tolerates a missing
    or non-integer value (treated as single-agent).
    """
    try:
        return int(os.getenv("PKS_PARALLEL", "1")) > 1
