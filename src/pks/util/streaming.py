...
from pks.util.pricing import is_tool_streaming_enabled
...
def _build_pricing_footer_renderable(final=False, *, framed: bool = True):
    ...
    bar_color = PKS_GREEN if ctx_pct < 50 else "yellow" if ctx_pct < 80 else "bold red"
...
def _group_tool_body_with_pricing_footer(panel_body):
    """Return tool body alone, or stack wait hint + pricing when PKS_TOOL_LIVE_SHOW_PRICING is set.
    ...
    """
    if os.getenv("PKS_TOOL_LIVE_SHOW_PRICING", "").lower() in ("1", "true", "yes"):
...
def _print_pricing_footer(console=None, final=False, framed=False):
    ...
    """
    # Disabled by user request: pricing info is now in toolbar
    return  # Never break execution for a display-only feature
    ...
    try:
        ...
        if token_stats:
            ...
            if input_tokens > 0:
                ...
                model_name = context.get("model", os.environ.get("PKS_MODEL", "alias1"))
                try:
        ...
        if is_final:
            ...
            if raw.strip():
                ...
                if getattr(TableElement, "_patched_for_cai", False) is False:
                    ...
                    TableElement._patched_for_cai = True
    ...
    if streaming:
        ...
        if call_id:
            ...
            try:
                ...
                # Flat tool body + optional [PKS] wait line + pricing footer inside Live.
                try:
                    panel = _group_tool_body_with_pricing_footer(content)
                except Exception:
                    panel = content
...
from pks.util.pricing import set_pending_cache_info, get_and_clear_pending_cache_info
