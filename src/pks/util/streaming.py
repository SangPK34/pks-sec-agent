"""
Streaming panel management, live display, and tool output rendering for PKS.

Compact REPL note (q3=b)
------------------------
In compact mode (default for CLI sessions, opt-out via ``PKS_COMPACT_REPL=0``),
...
:mod:`pks.tui.display.wrapper`, so the suppression has no effect on TUI mode.
"""
...
from pks.util._worker_silence import worker_display_silenced
from pks.util.session import is_parallel_session
from pks.util.tokens import (
    get_model_name,
    get_model_input_tokens,
    _create_token_display,
    normalize_token_info,
)
from pks.util.cli_palette import PKS_GREEN, FINAL_PANEL_BG, GREY_HINT, GREY_TEXT, PKS_BORDER
...
_PKSAgentLiveCls = None


def _get_pks_agent_live_class():
    """Return a ``Live`` subclass whose ``stop()`` can skip Rich's trailing ``console.line()``.

    Stock ``Live.stop()`` always calls ``console.line()`` after the final refresh. PKS's
    ``finish_agent_streaming`` always sets ``_pks_suppress_stop_line`` so that extra
    ...
    global _PKSAgentLiveCls
    if _PKSAgentLiveCls is not None:
        return _PKSAgentLiveCls

    from rich.live import Live as _RichLive

    class PKSAgentLive(_RichLive):
        """Sync ``stop`` body with ``rich.live.Live.stop`` when upgrading Rich."""

        _pks_suppress_stop_line: bool = False
                ...
                with self.console:
                    try:
                        ...
                        if (
                            ...
                            and not getattr(self, "_pks_suppress_stop_line", False)
                        ):
                            self.console.line()
...
# ─── Brand color (canonical: pks.util.cli_palette) ─────────────────────────
_DOT = "●"

# ─── Green/white theme for final response Markdown rendering ────────────────
from rich.theme import Theme as _Theme
_PKS_MD_THEME = _Theme({
    "markdown.h1": "bold #f59e0b",
    "markdown.h2": "bold #f59e0b",
    "markdown.h3": "bold #f59e0b",
    "markdown.h4": "bold #f59e0b",
    "markdown.h5": "bold #f59e0b",
    "markdown.h6": "bold #f59e0b",
    "markdown.h7": "dim #f59e0b",
    "markdown.strong": "bold white",
    "markdown.em": "italic white",
    "markdown.code": "bold #eab308 on #1e1e1e",
    "markdown.code_block": "white on #1e1e1e",
    "markdown.item.bullet": "bold #f59e0b",
    "markdown.item.number": "bold #f59e0b",
    "markdown.item": "white",
    "markdown.list": "dim #f59e0b",
    "markdown.link": "#f59e0b underline",
    "markdown.link_url": "dim",
    "markdown.block_quote": "italic dim #f59e0b",
    "markdown.paragraph": "white",
    "markdown.text": "white",
    "markdown.hr": "dim #f59e0b",
})


def _flat_agent_header(agent_name: str, counter: int, model: str = "",
                       provider: str = "") -> Text:
    """Build a one-line header: ● Agent Name"""
    t = Text()
...
def _flat_tool_header(tool_name: str, args_str: str, execution_info: dict = None,
    ...
    if is_handoff:
        ...
        # Surface the concrete task the router delegated (from the handoff `task` arg) so it is
        # transparent exactly what work — and phrasing — was handed to the specialist.
        _htask = ""
        try:
            import json as _hjson
            if args_str:
                _hp = _hjson.loads(args_str)
                if isinstance(_hp, dict):
                    _htask = str(_hp.get("task", "") or "").strip()
        except Exception:
            _htask = ""
        if _htask:
            if len(_htask) > 400:
                _htask = _htask[:397] + "…"
            t.append("\n    ↳ task giao cho sub-agent: ", style="dim yellow")
            t.append(_htask, style="italic white")
    else:
        t.append(tool_name, style=f"bold {PKS_GREEN}")
    ...
    if execution_info:
        ...
        if status == "completed":
            t.append(" [Completed]", style=PKS_GREEN)
        elif status == "error":
...
# Footer bookkeeping retained for compact token/context rendering.
_USAGE_FOOTER_LINES = 0
_usage_footer_printed = False
_last_printed_sticky_footer_lines = 0
...
def _collapse_rich_live_shrink_gap(rich_console, extra_lines: int) -> None:
    ...
    ``● … tool`` line after we dropped footer/[Uu]sage from the pre-tool freeze frame.
    """
    if extra_lines <= 0:
        return
    try:
...
def refresh_tool_wait_displays() -> None:
    """Redraw grouped tool Live panels and/or sticky [Uu]sage footer (tool wait hint)."""
    ...
    with _GROUPED_TOOLS_LOCK:
        for group_info in list(_GROUPED_STREAMING_TOOLS.values()):
            ...
            try:
                ...
                live_panel.update(_group_tool_body_with_usage_footer(panel_raw))
            except Exception:
                pass
    if not has_group_live and _usage_footer_printed:
        try:
            ...
            if has_active_sessions and has_wait_hint:
                _erase_usage_footer()
                _print_usage_footer(Console(), final=False, framed=False)
        except Exception:
            pass
    ...
    with _PANEL_UPDATE_LOCK:
        for call_id, panel_info in list(_LIVE_STREAMING_PANELS.items()):
            ...
            try:
                ...
                panel = _group_tool_body_with_usage_footer(content)
                panel_info.update(panel)
            except Exception:
                pass


def _build_usage_footer_renderable(final=False, *, framed: bool = True):
    del final, framed
    return Text(""), 0
...
def _tool_wait_hint_rich():
    ...
    try:
        from pks.util.wait_hints import get_tool_wait_footer_renderable
...
def _group_tool_body_with_usage_footer(panel_body):
    return panel_body


def _print_usage_footer(console=None, final=False, framed=False):
    del console, final, framed
    return
    ...
    with _GROUPED_TOOLS_LOCK:
        ...
        for group_id, group_info in list(_GROUPED_STREAMING_TOOLS.items()):
            ...
            if live_panel:
                try:
                    live_panel.stop()
                    if _debug_os.getenv("PKS_DEBUG_TOOLS_VIZ") == "true":
...
def _find_or_create_tool_group(call_id, tool_name, args, token_info):
    ...
    # NOTE: We don't check PKS_STREAM here because this function is only called
...
def _update_tool_group(group_id, call_id, output, execution_info=None, token_info=None, args=None):
    ...
    with _GROUPED_TOOLS_LOCK:
        ...
        # Build the combined panel, with [Uu]sage footer embedded for Live display
        panel_raw, all_complete = _build_grouped_panel_content(group_info)
        try:
            panel = _group_tool_body_with_usage_footer(panel_raw)
        except Exception:
            panel = panel_raw
    ...
    with _GROUPED_TOOLS_LOCK:
        ...
        def _enrich(tool_data):
            ...
            if not raw.get("model"):
                raw["model"] = os.environ.get("PKS_MODEL", "")
            return normalize_token_info(raw)
        ...
        # Do not re-print sticky [Uu]sage here — it duplicated rows between tools in logs.
        # Token context appears on the next assistant turn.
...
def restore_terminal_state(
    *,
    ...
    screen=False by default, so PKS normally never enters the xterm alternate buffer; sending
    1049l anyway makes many terminals swap to the main buffer and redraw a stale bash prompt
    in the middle of PKS output while Python is still running. The leave_alternate_screen name
...
def cli_print_tool_call(
    ...
    # Optional token/debug metadata (accepted for compatibility; ignored here)
...
def _prepare_terminal_for_final_agent_output() -> None:
    ...
    """
    try:
        from pks.util.wait_hints import clear_wait_hints
...
def cli_print_agent_messages(
    ...
    cache_creation_tokens=None,  # Tokens written to cache (cache write)
    # Legacy cache token aliases
    cached_tokens=None,
):
    ...
    # :mod:`pks.util._worker_silence`.
    if worker_display_silenced():
        return

    # Check if we're in TUI mode and should use TUI display instead
    import os
    if os.getenv("PKS_TUI_MODE") == "true":
        try:
            from pks.tui.display.integration import display_agent_messages
            ...
            # Build token info for context visibility.
            token_info = {
                ...
                # Propagate model and agent name for attribution.
                "model": model,
            ...
            try:
                from pks.tui.display.integration import get_terminal_id as _get_tid
                _tid = _get_tid()
                if _tid:
    ...
    # Erase sticky [Uu]sage footer before printing new agent content
    _erase_usage_footer()
    ...
    # Header: ● Agent Name
    header = _flat_agent_header(agent_name, counter, model or "")
    ...
    if is_final_response and parsed_message:
        ...
        console.push_theme(_PKS_MD_THEME)
        try:
            console.print(
                Panel(
                    body,
    ...
    # Sticky [Uu]sage footer — Option F style with green separators.
    ...
    if (
        ...
        _print_usage_footer(console, final=True, framed=True)
...
def create_agent_streaming_context(agent_name, counter, model):
    ...
    if os.getenv("PKS_TUI_MODE") == "true":
        ...
        try:
            from pks.tui.display.integration import get_terminal_id
        ...
        if text_delta:
            ...
            if context.get("terminal_id"):
                try:
                    ...
                    from pks.tui.display.integration import update_agent_streaming_content as tui_update
                    tui_update(context, text_delta, token_stats)
                except ImportError:
                    # Fallback to manager if integration not available
                    try:
                        from pks.tui.display.manager import DisplayManager
    ...
    try:
        ...
        if token_stats:
            ...
            if input_tokens > 0:
                ...
                model_name = context.get("model", os.environ.get("PKS_MODEL", "gpt-5.6-terra"))
                try:
            ...
            # Update the footer (disabled, moved to toolbar)
            context["footer"] = Text("")

        # Build flat renderable for Live update without the footer
        from rich.console import Group
        import shutil
        
        display_content = context["content"]
        term_height = shutil.get_terminal_size((80, 24)).lines
        max_lines = max(10, term_height - 10)
        
        if hasattr(display_content, "split"):
            lines = display_content.split("\n")
            if len(lines) > max_lines:
                display_content = Text("\n").join(lines[-max_lines:])
                display_content = Text("... (scrolled up) ...\n", style="dim italic") + display_content

        updated_content = Group(
            context["header"], display_content
        )
        ...
        if context.get("is_started", False) and context.get("live"):
            # PERF: do NOT force a synchronous Live.refresh() on every streamed
...
def _fix_proxy_glued_markdown(raw: str) -> str:
    """Fix proxy-mangled markdown (glued chars / dropped newlines) BEFORE rendering.

    Fenced code blocks are protected: only the ``` fences themselves are un-glued;
    the CODE BETWEEN them is left verbatim, so shell pipes like ``echo x | base64``
    are never split by the list/table/heading rules.
    """
    # Strip leading blank rows the proxy prepends.
    raw = re.sub(r"^\n+", "", raw)

    # 5. Make code fences well-formed / on their own lines FIRST, so the code-vs-text
    #    split below can see them. This only moves the fence, never code content.
    raw = re.sub(r'([^\s\n])```', r'\1\n```', raw)   # "Action```bash" -> "Action\n```bash"
    langs = (
        r'(bash|sh|zsh|python|py|html|json|yaml|yml|js|ts|javascript|typescript|c|cpp|'
        r'java|go|rust|php|sql|xml|css|md|markdown|txt|text|raw|csv|tsv|dockerfile|ini|'
        r'toml|diff|powershell|ps1|ruby|rb|perl|lua|asm|nasm|hex)'
    )
    # "```bashstrings classes.dex" -> "```bash\nstrings classes.dex". Require whitespace in the
    # glued remainder so a well-formed "```python" is NOT mis-split into "```py\nthon".
    raw = re.sub(rf'```{langs}(?=[^\s\n])([^\n]*[ \t][^\n]*)$', r'```\1\n\2', raw, flags=re.MULTILINE)

    # Walk lines; clean only the runs OUTSIDE ``` fences. Code lines pass through verbatim.
    out: list = []
    buf: list = []
    in_code = False
    ...
    def _flush():
        if buf:
            out.append(_clean_non_code_markdown("\n".join(buf)))
            buf.clear()

    for ln in raw.split("\n"):
        if ln.lstrip().startswith("```"):
            _flush()
            out.append(ln)          # fence line, verbatim
            in_code = not in_code
        elif in_code:
            out.append(ln)          # code content, verbatim (pipes/dashes untouched)
        else:
            buf.append(ln)
    _flush()
    return "\n".join(out)
...
def _clean_non_code_markdown(raw: str) -> str:
    """Apply the glue-fixing rules to NON-code text only (see _fix_proxy_glued_markdown)."""
    raw = re.sub(r"[ \t]+\n", "\n", raw)                     # trailing spaces the proxy leaves
    raw = re.sub(r"\n[ \t]*\n(?:[ \t]*\n)+", "\n\n", raw)    # collapse blank-line runs
    raw = re.sub(r"\n{3,}", "\n\n", raw)

    # 1. list items — only ``-``/``+`` bullets. NOT ``*``: a mid-line "* " is almost always the
    #    closing marker of *italic*, and splitting there breaks the emphasis (and spawns a fake bullet).
    raw = re.sub(r'([^\s\n\-])([-+]\s+)', r'\1\n\2', raw)
    # 2. missing space after heading hashes (2+ '#', so "#1" stays literal)
    raw = re.sub(r'^(#{2,})(?=[^\s#])', r'\1 ', raw, flags=re.MULTILINE)
    # 3. missing newline before heading ("Text### Heading")
    raw = re.sub(r'([^\s\n#])(#{2,}\s+)', r'\1\n\n\2', raw)
    # 3b. heading glued with NO space after the hashes ("Reconnaissance###1)")
    raw = re.sub(r'([^\s\n#])(#{2,})(?=[^\s#])', r'\1\n\n\2 ', raw)
    # 4. blockquotes
    raw = re.sub(r'([^\s\n\-])(>[\s#a-zA-Z])', r'\1\n\n\2', raw)
    # 6. horizontal rules (don't split a line that is itself only rule chars)
    raw = re.sub(r'([^\s\n*_\-])([ \t]*[-*_]{3,}[ \t]*)$', r'\1\n\n\2', raw, flags=re.MULTILINE)
    raw = re.sub(r'([^\n])\n([ \t]*---[ \t]*)\n', r'\1\n\n\2\n', raw)
    raw = re.sub(r'\n([ \t]*---[ \t]*)\n([^\n])', r'\n\1\n\n\2', raw)
    # 7. tables
    raw = re.sub(r'^([^|\n]*?[^\s|\n][^|\n]*?)(\|[ \t]*[^|\n]+[ \t]*\|)', r'\1\n\n\2', raw, flags=re.MULTILINE)
    # 8. "Step N" section headers welded to preceding text
    raw = re.sub(r'([^\s\n])(Step\s*\d+\s*[—–\-:\)])', r'\1\n\n\2', raw)
    # 9. status markers welded to end of previous line (")✅" -> ")\n✅")
    raw = re.sub(r'([^\s\n])(✅|❌|☑️|☑|🚩|⚠️)', r'\1\n\2', raw)
    return raw


_FLAG_RE = re.compile(r"[A-Za-z][A-Za-z0-9_]{1,24}\{[^{}\n]{1,160}\}")


def _md_inline_resilient(line):
    """Inline markdown (**bold**, *italic*, `code`, ~~strike~~) -> Rich Text, forgiving.

    Borrowed from strix's line-based styler: an unterminated marker falls through
    as plain text instead of corrupting the whole render (which Rich's strict
    Markdown parser does when the proxy drops a character).
    """
    result = Text()
    i, n = 0, len(line)

    def _bnd_before(pos):
        # left side of a marker is a "word boundary" if it's start-of-line or a non-alnum char
        return pos == 0 or not line[pos - 1].isalnum()

    while i < n:
        # FLAG: render flag-shaped tokens (picoCTF{...}, flag{...}, HTB{...}) verbatim in YELLOW.
        # Never split or italicise their underscores — the exact flag text must survive intact.
        fm = _FLAG_RE.match(line, i)
        if fm:
            result.append(fm.group(0), style="bold yellow")
            i = fm.end()
            continue
        two = line[i:i + 2]
        # Bold: ``**`` anywhere; ``__`` ONLY at word boundaries, so identifiers/flags like
        # picoCTF{a__b} or snake__case are never eaten. FLAG SAFETY: never rewrite intra-word _.
        if two == "**" or (two == "__" and _bnd_before(i)):
            end = line.find(two, i + 2)
            if end != -1 and (two == "**" or end + 2 >= n or not line[end + 2].isalnum()):
        ...
        if line[i] == "`":
            ...
            if end != -1:
                _inner = line[i + 1:end]
                _cs = "bold yellow" if _FLAG_RE.fullmatch(_inner) else "bold cyan"
                result.append(_inner, style=_cs)
                i = end + 1
                continue
        # Italic: ``*`` anywhere; ``_`` ONLY at word boundaries. FLAG SAFETY: this is what keeps
        # picoCTF{w3lc0m3_t0_c4d} intact instead of turning "_t0_" into italics and dropping the _.
        ch = line[i]
        if ch == "*" or (ch == "_" and _bnd_before(i)):
            if i + 1 < n and line[i + 1] != ch:
                end = line.find(ch, i + 1)
                if end != -1 and (end + 1 >= n or line[end + 1] != ch):
                    if ch == "*" or end + 1 >= n or not line[end + 1].isalnum():
...
def _render_markdown_resilient(raw, justify="left"):
    """Forgiving line-based markdown -> Rich renderable (inspired by strix).

    Rich's ``Markdown()`` is a strict CommonMark parser that visibly breaks when
    the upstream proxy drops characters, glues words, or loses blank lines. This
    renderer processes text line-by-line and degrades gracefully: malformed markup
    becomes readable text instead of a corrupted block. Handles headings (a distinct
    colour per level — terminals can't vary font *size*, only style/colour), code
    fences, block quotes, bullet/numbered lists, horizontal rules, inline styles,
    and real markdown tables (rendered via rich.table). Returns a Text, or a Group
    when tables are present. Toggle with PKS_RESILIENT_MARKDOWN (default on).
    """
    from rich.console import Group
    from rich.table import Table
    from rich import box as _rbox

    # Longest prefix first. Each level gets its own colour so the hierarchy is
    # visible even though every glyph is the same physical size in a terminal.
    level_styles = {
        1: "bold bright_cyan",
        2: "bold cyan",
        3: "bold green",
    ...
    def _try_header(line):
        ...
        rest = line[j:]
        if rest.strip() == "":
            return None  # a run of only '#' is decorative -> handled as a horizontal rule
        # A heading needs a space after the hashes, OR (proxy-glue) 2+ hashes welded to text.
        # A single '#' welded to text (e.g. "#1") stays literal. 7+ hashes clamp to H6.
        if rest[:1] == " " or j >= 2:
            # A header line is already bold+coloured, so inline **bold**/__ markers inside it are
            # redundant and would otherwise show literally (headers don't run inline formatting).
            rest_clean = rest.lstrip().replace("**", "").replace("__", "")
            return Text(rest_clean, style=level_styles[min(j, 6)])
        return None

    def _is_hr(line):
        # Horizontal rule / decorative divider: 3+ of the same rule char (ignoring interior
        # spaces), e.g. "---", "***", "___", "======", or a run of only "#########".
    ...
    def _render_line(line):
        ...
        if h is not None:
            return h
        if _is_hr(line):
            return Text("─" * 40, style="dim cyan")
        if line.startswith("> "):
            t = Text(); t.append("┃ ", style="cyan"); t.append_text(_md_inline_resilient(line[2:])); return t
        if line.startswith(("- ", "* ", "+ ")):
            t = Text(); t.append("• ", style="cyan"); t.append_text(_md_inline_resilient(line[2:])); return t
        if len(line) > 2 and line[0].isdigit() and line[1:3] in (". ", ") "):
            t = Text(); t.append(line[:2] + " ", style="cyan"); t.append_text(_md_inline_resilient(line[3:])); return t
        return _md_inline_resilient(line)
    ...
    def _is_sep(line):
        ...
        return bool(cells) and any(c for c in cells) and all(
            re.fullmatch(r":?-{1,}:?", c) for c in cells if c != ""
        )

    def _build_table(header, rows):
        tbl = Table(box=_rbox.SIMPLE_HEAVY, show_edge=True, header_style="bold cyan",
                    pad_edge=False, expand=False)
        ncols = max([len(header)] + [len(r) for r in rows]) if rows else max(len(header), 1)
        ncols = max(ncols, 1)
        for c in range(ncols):
            tbl.add_column(_md_inline_resilient(header[c]) if c < len(header) else "")
        for r in rows:
            tbl.add_row(*[(_md_inline_resilient(r[c]) if c < len(r) else Text("")) for c in range(ncols)])
        return tbl

    lines = raw.split("\n")
    parts: list = []
    run: list = []      # current run of non-table lines (each already a Text)
    in_code = False

    def _flush():
        if run:
            block = Text(justify=justify)
            for k, seg in enumerate(run):
                if k:
                    block.append("\n")
                block.append_text(seg)
            parts.append(block)
            run.clear()
    ...
    while i < n:
        ...
        if not in_code and line.count("|") >= 2 and i + 1 < n and _is_sep(lines[i + 1]):
            _flush()
            header = _split_row(line)
            i += 2
            rows = []
            while i < n and lines[i].count("|") >= 2 and not _is_sep(lines[i]):
                rows.append(_split_row(lines[i]))
                i += 1
            parts.append(_build_table(header, rows))
            continue
        if line.startswith("```"):
            # fence marker (```bash): quiet, so the code body is the focal point
            run.append(Text(line, style="dim cyan"))
            in_code = not in_code
            i += 1
            continue
        if in_code:
            # code body: light text on a subtle dark box -> reads as a distinct code block,
            # clearly different from the coloured headers, without being noisy.
            run.append(Text(line if line else " ", style="#ffa657"))
            i += 1
            continue
        run.append(_render_line(line))
        i += 1
    _flush()

    if not parts:
        return Text("", justify=justify)
    return parts[0] if len(parts) == 1 else Group(*parts)


def _render_agent_markdown(raw, justify="left"):
    """Render already-cleaned agent markdown; resilient by default, safe fallback to Rich Markdown."""
    if os.getenv("PKS_RESILIENT_MARKDOWN", "true").strip().lower() not in ("0", "false", "no"):
        try:
            return _render_markdown_resilient(raw, justify=justify)
        except Exception:
            pass
    from rich.markdown import Markdown
    return Markdown(raw, justify=justify)
...
def finish_agent_streaming(context, final_stats=None):
    ...
    Args:
        ...
        final_stats: Optional dictionary with token statistics
    """
    if not context:
        return False
    ...
    if context.get("is_tui"):
        ...
        if context.get("terminal_id"):
            try:
                ...
                from pks.tui.display.integration import finish_agent_streaming as tui_finish
    ...
    try:
        ...
        if is_final:
            ...
            if raw.strip():
                raw = _fix_proxy_glued_markdown(raw)
                
                from rich.markdown import Markdown, Heading, TableElement
                # Monkey-patch Rich Heading to completely remove boxing, underlining, and centering!
                if getattr(Heading, "_patched_for_pks", False) is False:
                    def _new_rc(self, console, options):
                        text = self.text
                        text.justify = "left"
                        yield text
                    Heading.__rich_console__ = _new_rc
                    Heading._patched_for_pks = True
                    
                # Monkey-patch TableElement to remove top/bottom margin spaces (show_edge=False)
                if getattr(TableElement, "_patched_for_pks", False) is False:
                    def _new_te(self, console, options):
                        from rich.table import Table
                        import rich.box as box
                        table = Table(box=box.SIMPLE_HEAVY, show_edge=False)
                        if getattr(self, "header", None) is not None and getattr(self.header, "row", None) is not None:
                        ...
                        yield table
                    TableElement.__rich_console__ = _new_te
                    TableElement._patched_for_pks = True
                
                body = _render_agent_markdown(raw, justify="left")
            else:
                body = context["content"]
            # Apply green/white markdown theme for the final panel.
            console.push_theme(_PKS_MD_THEME)
            final_content = Panel(
                body,
                title=context["header"],
                title_align="left",
                border_style=PKS_BORDER,
            ...
            if hasattr(_ct, "plain") and _ct.plain:
                ...
                if _trim:
                    _trim = _fix_proxy_glued_markdown(_trim)
                    _trim += "\n\n"  # Prevent tool header from concatenating on the same line, and add visual spacing
                    
                    from rich.markdown import Markdown
                    # The Heading monkey-patch is already applied above
                    context["content"] = _render_agent_markdown(_trim, justify="left")
            final_content = Group(
                context["header"],
                context["content"],
            )
        ...
        if _compact_suppresses_verbose() and not context.get("is_started", False):
            ...
            if final_stats is not None:
                _print_usage_footer(final=is_final, framed=is_final)
            return True
        ...
        if context.get("is_started", False):
            try:
                _live = context["live"]
                if hasattr(_live, "_pks_suppress_stop_line"):
                    _live._pks_suppress_stop_line = True
                _live.transient = True
                _live.stop()
                if hasattr(_live, "_pks_suppress_stop_line"):
                    _live._pks_suppress_stop_line = False
                
                # After clearing the Live text, print the final renderable directly
                console.print(final_content)
            except Exception as e:
                context["error"] = str(e)
                try:
                    _live2 = context["live"]
                    if hasattr(_live2, "_pks_suppress_stop_line"):
                        _live2._pks_suppress_stop_line = False
                    _live2.stop()
                except Exception:
                    pass
        ...
        # Sticky [Uu]sage after tool interrupts printed two extra rows here; the next UI
...
def cli_print_tool_output(
    ...
    Args:
        ...
        token_info: Optional token information with keys:
            ...
            - context_usage_pct: optional context percentage
    ...
    if token_info is not None:
        token_info = normalize_token_info(token_info)
    ...
    try:
        import os as _os
        if _os.getenv("PKS_TUI_MODE") == "true":
            ...
            if not terminal_id:
                try:
                    from pks.tui.core.terminal_tracking import get_current_terminal_id as _get_tid
                    from pks.tui.core.execution_context import get_terminal_id_context as _get_tid_ctx
            ...
            if terminal_id:
                try:
                    from pks.tui.display.manager import DisplayManager as _TuiDisplayManager
    ...
    if is_session_command and _debug_os.getenv("PKS_DEBUG_TOOLS_VIZ") == "true":
...
# For tools that have a 'command' field (like generic_linux_command)
    ...
    if streaming:
        ...
        if call_id:
            ...
            try:
                ...
                # Flat tool body + optional [PKS] wait line + [Uu]sage footer inside Live.
                try:
                    panel = _group_tool_body_with_usage_footer(content)
                except Exception:
                    panel = content
                ...
                is_container = bool(os.getenv("PKS_ACTIVE_CONTAINER", ""))
                    ...
                    with _PANEL_UPDATE_LOCK:
                        ...
                        if isinstance(panel_info, dict) and panel_info.get("type") == "static":
                            ...
                            if execution_info and execution_info.get("is_final", False):
                                # Debug output
                                if os.getenv("PKS_DEBUG_STREAMING"):
                                ...
                                if not enriched_token_info.get("model"):
                                    enriched_token_info["model"] = os.environ.get("PKS_MODEL", "")
                                enriched_token_info = normalize_token_info(enriched_token_info)
                            ...
                            if call_id in _LIVE_STREAMING_PANELS:
                                ...
                                if is_static:
                                    ...
                                    try:
                                        ...
                                        panel_info.transient = True
                                        panel_info.stop()
                                        
                                        # Print final panel cleanly
                                        console.print(panel)
                    ...
                    with _PANEL_UPDATE_LOCK:
                        ...
                        if is_parallel:
                            ...
                            if call_id not in _LIVE_STREAMING_PANELS:
                                ...
                                if execution_info and execution_info.get("is_final", False):
                                    # Final snapshot: body only (no embedded [Uu]sage footer) — avoids
    ...
    if not streaming:
        ...
        if command_key in cli_print_tool_output._displayed_commands:
            ...
            # Don't skip based on PKS_STREAM setting alone
...
def start_tool_streaming(tool_name, args, call_id=None, token_info=None):
    ...
    if tool_name == "execute_code" and is_parallel and isinstance(args, dict) and "code" in args:
        ...
        code_header = Text()
        code_header.append("• ", style=PKS_GREEN)
        code_header.append(f"{agent_name}", style="bold #ff3355")
...
def finish_tool_streaming(tool_name, args, output, call_id, execution_info=None, token_info=None):
    ...
    if tool_name == "execute_code" and isinstance(args, dict) and "code" in args:
        ...
        out_header = Text()
        out_header.append("• ", style=PKS_GREEN)
        out_header.append(f"{agent_name}", style="bold #ff3355")
        if status == "completed":
        ...
        # Print [Uu]sage footer after execute_code output
        _print_usage_footer(console, final=False, framed=False)
        _print_cli_gap_after_completed_tool(False, None, call_id)
    ...
    # [Uu]sage suppressed from individual tool outputs — shown only in agent messages
...
def create_claude_thinking_context(agent_name, counter, model):
    """Create a compact reasoning activity row without a second Rich Live."""
    from pks.output import OUTPUT, TaskStartEvent

    thinking_id = f"thinking_{agent_name}_{counter}_{uuid.uuid4().hex[:8]}"
    context = {
    ...
    try:
        ...
        from pks.repl.ui.compact_wiring import is_compact_enabled
        from pks.util.wait_hints import wait_hints_enabled
...
def update_claude_thinking_content(context, thinking_delta):
    """Update the compact reasoning row while retaining only counters for UI."""
    if not context or not isinstance(thinking_delta, str):
        return False
    from pks.output import OUTPUT, TaskUpdateEvent

    if thinking_delta:
        context["char_count"] += len(thinking_delta)
        context["line_count"] += thinking_delta.count("\n")
        if not context["has_content"]:
            context["has_content"] = True
            context["line_count"] += 1
    lines = max(1, context["line_count"])
    status = context.get("status")
    if status is not None:
        try:
            elapsed = max(0, int(time.time() - context["started_at"]))
            status.update(
                build_model_wait_hint_renderable(
    ...
    try:
        OUTPUT.emit(
            TaskUpdateEvent(
                task_id=context["thinking_id"],
                label=f"Thinking… ({lines} {'line' if lines == 1 else 'lines'})",
            )
        )
    except Exception:
        pass
...
def finish_claude_thinking_display(context):
    """Complete and remove a compact reasoning activity row."""
    if not context:
        return False
    from pks.output import OUTPUT, TaskCompleteEvent, TaskUpdateEvent

    thinking_id = context.get("thinking_id", "")
    _CLAUDE_THINKING_PANELS.pop(thinking_id, None)
    duration = max(0.0, time.time() - context.get("started_at", time.time()))
    lines = max(1, int(context.get("line_count", 0)))
    status = context.get("status")
    if status is not None:
        try:
            status.stop()
    ...
    try:
        OUTPUT.emit(
            TaskUpdateEvent(
                task_id=thinking_id,
                label=f"Thought for {duration:.1f}s · {lines} {'line' if lines == 1 else 'lines'}",
            )
        )
        OUTPUT.emit(
            TaskCompleteEvent(
...
def detect_claude_thinking_in_stream(model_name):
    """Return whether the selected catalog model exposes reasoning effort."""
    if not model_name:
        return False
    try:
        from pks.model_catalog import find_model

        model = find_model(str(model_name))
        if model is not None:
            return bool(model.efforts)
    except Exception:
        pass
    effort = os.getenv("PKS_REASONING_EFFORT", "").strip().lower()
    return bool(effort and effort not in {"off", "none", "false", "0"})
...
from pks.util.interaction import signal_handler
