...
# PKS: default to YOUR local gateway (never a third party). Honors OPENAI_API_BASE/URL.
DEFAULT_ALIAS_LLM_API_BASE = (
    os.getenv("OPENAI_API_BASE") or os.getenv("OPENAI_BASE_URL") or "http://localhost:20128/v1"
)

# Model ids for which ``CSI_CUSTOM_ENDPOINT`` / ``ALIAS_API_URL`` may apply (prefix match, case-insensitive).
_ALIAS_API_URL_MODEL_PREFIXES: tuple[str, ...] = ("pks", "alias", "csi")
...
def model_qualifies_for_alias_api_url(model: str | None) -> bool:
    """True when ``CSI_CUSTOM_ENDPOINT`` / ``ALIAS_API_URL`` may apply (pks, alias, or csi prefix).
...
def explicit_custom_llm_api_base_configured(model: str | None = None) -> bool:
    ...
    effective = (model if model is not None else os.getenv("PKS_MODEL")) or ""
...
def resolve_llm_openai_compatible_base(model: str | None = None) -> str:
    ...
    1. ``CSI_CUSTOM_ENDPOINT`` if non-empty (e.g. CSI + PKS backend).
    ...
    ``model`` defaults to ``PKS_MODEL`` from the environment when omitted.
    """
    effective = (model if model is not None else os.getenv("PKS_MODEL")) or ""
    if model_qualifies_for_alias_api_url(effective):
...
def resolve_llm_openai_compatible_api_key(model: str | None = None) -> str:
    ...
    Rules:
    - Alias-family models (``pks*``, ``alias*``, ``csi*``) MUST use ``ALIAS_API_KEY``.
