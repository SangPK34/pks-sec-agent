...
(requests-per-minute) caps. Without client-side pacing, a moderately busy PKS
...
silent lag while PKS reactively retries.

This module enforces those caps **before** each outbound request, queueing
calls until the sliding 60s window has capacity.

Limits source of truth: ``pks.continuous_ops.rate_plan.get_rate_limits()``,
resolved against the ``PKS_ALIAS_RATE_TIER`` env (``pro`` → 500K TPM / 60 RPM,
...
If the user changes ``PKS_ALIAS_RATE_TIER`` mid-session (e.g. via ``/env``),
...
from pks.errors import LLMContextOverflow
...
def get_gateway_rate_limiter() -> GatewayRateLimiter:
    ...
    Lazily instantiated so ``PKS_ALIAS_RATE_TIER`` is read from the environment
    ...
    if _GATEWAY_RATE_LIMITER is None:
        with _LIMITER_INIT_LOCK:
            if _GATEWAY_RATE_LIMITER is None:
                from pks.continuous_ops.rate_plan import (
                    get_rate_limits,
                    resolve_rate_tier,
                )
...
def reset_gateway_rate_limiter() -> None:
    ...
    rebuilds it with the current ``PKS_ALIAS_RATE_TIER``.
...
def make_pace_overlay_callback() -> Callable[[float], None]:
    ...
    # (pks.util.wait_hints imports nothing from this module, but the broader
    # pks.util package has cross-module touches — keep this defensive).
    from pks.util.wait_hints import set_model_wait_retry_overlay
