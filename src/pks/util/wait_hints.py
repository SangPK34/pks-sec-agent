...
Fixed schedules and animations (no PKS_WAIT_HINT_* tuning). Disabled when stderr is not
...
MODEL_DELAY_MESSAGES: tuple[str, ...] = (
    ...
    "If this repeats often, check the provider or local gateway status.",
...
RETRY_TRAILER = (
    ...
    "If this persists, inspect the endpoint logs or switch models."
)
...
def get_tool_wait_body_plain() -> str | None:
    """Human tool-wait line (no PKS badge), for Result/captured rail in Layout 1."""
    with _TOOL_FOOTER_LOCK:
...
def _request_footer_ui_refresh() -> None:
    ...
    try:
        from pks.util.streaming import refresh_tool_wait_displays
...
def wait_hints_enabled() -> bool:
    if os.getenv("PKS_TUI_MODE", "").strip().lower() in ("true", "1", "yes"):
        return False
    try:
...
def tool_wait_hints_enabled() -> bool:
    ...
    if os.getenv("PKS_TUI_MODE", "").strip().lower() in ("true", "1", "yes"):
        return False
    if os.getenv("PKS_DISABLE_TOOL_WAIT_HINTS", "").strip().lower() in (
        "1",
        "true",
        "yes",
    ):
