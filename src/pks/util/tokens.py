"""
Token counting, model info, and token display utilities for PKS.
"""
...
def get_model_name(model):
    ...
    return os.environ.get("PKS_MODEL", "qwen2.5:72b")
...
def get_model_input_tokens(model):
    ...
    model_tokens_specific = {
        # OpenAI GPT-5.x series — 5.5+ = 1M context (API); 5.0-5.2 = 272K-400K.
        # NOTE: gpt-5.5/5.6 MUST precede "gpt-5" (substring match, first wins).
        "gpt-5.6": 1_000_000,
        "gpt-5.5": 1_000_000,
        "gpt-5.2": 400_000,
        "gpt-5.1": 272_000,
        ...
        "gpt-4-32k": 32_768,
        "gpt-4": 8_192,
        # Claude 4.x / 5.x — ORDER MATTERS (substring match, first wins): most-specific
        # names first, bare "claude-<tier>-4" LAST so 4.5/4.6/4.8 aren't swallowed by it.
        # 4.6/4.8 + Claude 5 = 1M standard; 4.5/4.1 + Haiku = 200K (Claude 4.5 offers 1M
        # only in a Tier-4 beta -> keep 200K to stay safe against context overflow).
        "claude-sonnet-4-20250514": 1_000_000,  # dated Sonnet 4.0 (1M)
        # -- 1M-standard newer models --
        ...
        "claude-5": 1_000_000,
        # -- 200K-standard models (1M only in beta) --
        ...
        "claude-sonnet-4-5": 200_000,
        # -- bare prefixes LAST (catch only true 4.0 variants) --
        "claude-opus-4": 200_000,
        "claude-sonnet-4": 1_000_000,  # Sonnet 4.0 = 1M
        ...
        # DeepSeek — V4 (Pro/Flash) = 1M; V3/V3.2/R1/chat = 128K-164K.
...
def _create_token_display(
    ...
    # Debug: Print cache metrics when PKS_SHOW_CACHE is enabled
    ...
    # PKS_SHOW_CACHE: Always show cache metrics for debugging
    show_cache_always = os.getenv("PKS_SHOW_CACHE", "").lower() in ("true", "1", "yes")
    ...
    # PKS build: token counts only — no cost figures anywhere in the display.
    tokens_text.append(f" | Total: {interaction_total_tokens}", style="bold white")
...
def _create_token_info_display(token_info=None):
    ...
    model = token_info.get("model", os.environ.get("PKS_MODEL", ""))
