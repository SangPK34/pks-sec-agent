...
``PKS_SESSION_COMPACT_SUMMARY``. On agent switch, :func:`prepare_agent_handoff`
...
ENV_SESSION_SUMMARY = "PKS_SESSION_COMPACT_SUMMARY"
ENV_RECENT_FINDINGS = "PKS_SESSION_RECENT_FINDINGS"
ENV_HANDOFF = "PKS_AGENT_HANDOFF_CONTEXT"
ENV_KEEP_RECENT = "PKS_COMPACT_KEEP_RECENT"
...
def get_keep_recent_messages() -> int:
    """Messages to keep verbatim after compaction (env ``PKS_COMPACT_KEEP_RECENT``)."""
...
def _truncate_for_env(text: str, limit: int = MAX_ENV_CHARS) -> str:
    if len(text) <= limit:
        return text
    return text[: limit - 80] + "\n\n[...truncated for PKS_SESSION_COMPACT_SUMMARY env...]"
...
def prepare_agent_handoff(
    ...
    *,
    ...
    """Build one-shot handoff for the next agent (sets ``PKS_AGENT_HANDOFF_CONTEXT``)."""
