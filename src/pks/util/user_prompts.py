...
(``pks/util/interactive_prompt.py``, ``pks/tools/sudo.py``,
``pks/tools/sensitive_command_guard.py``); merging them eliminates a layer
of cross-imports and keeps "what happens when PKS needs the human's
...
# ``pks.tools.common`` re-exports the sudo helpers from this module, and the settings
# package transitively pulls ``pks.tools.common`` back in.
from pks.util.cli_palette import PKS_GREEN, GREY_TEXT, YELLOW_ON, YELLOW_WARN
from pks.util.terminal import console as _console
...
def pause_terminal_owners() -> None:
    ...
    """
    try:
        from pks.util.wait_hints import pause_all_wait_hints

        pause_all_wait_hints()
    except Exception:
        pass
    try:
        from pks.util.streaming import pause_streaming_lives

        pause_streaming_lives()
    except Exception:
        pass
    try:
        from pks.repl.ui.compact_renderer import pause_compact_live
...
def resume_terminal_owners() -> None:
    ...
    """
    try:
        from pks.repl.ui.compact_renderer import resume_compact_live

        resume_compact_live()
    except Exception:
        pass
    try:
        from pks.util.wait_hints import resume_all_wait_hints
...
def _continuous_ops_no_sudo() -> bool:
    ...
    return os.getenv("PKS_CONTINUOUS_OPS_NO_SUDO", "").strip().lower() in ("1", "true", "yes")


def _continuous_ops_loop_child() -> bool:
    """True inside the headless loop child (set only by the generated worker script)."""
    return os.getenv("PKS_CONTINUOUS_OPS_LOOP_CHILD", "").strip().lower() in ("1", "true", "yes")
...
def _privileged_worker_sudo_prompt_timeout() -> float | None:
    ...
    if not _privileged_continuous_ops_worker():
        return None
    try:
        tick = float(os.getenv("PKS_CONTINUOUS_OPS_TICK_SECONDS", "60"))
    except ValueError:
        tick = 60.0
...
def _read_sudo_password_interruptible(
    ...
    * The idle deadline is reset on every keystroke, so it has to be
    ...
    from pks.util.interaction import is_prompt_abort_requested
...
def _show_auth_ok() -> None:
    ...
    _erase_previous_line()
    t = Text()
    t.append(f"{_CHECK} ", style=f"bold {PKS_GREEN}")
    t.append("sudo", style=f"bold {PKS_GREEN}")
    t.append(" ─ ", style="dim white")
    t.append("ok", style=PKS_GREEN)
    _console.print(t)
...
def ensure_sudo_credentials(
    ...
    if _continuous_ops_no_sudo():
        ...
        return (
            "[PKS_CONTINUOUS_OPS_NO_SUDO] This worker was started with sudo-style privileges "
    ...
    from pks.util.interaction import (
...
#
# Controlled by PKS_SENSITIVE_GUARD env var (default: "true").
# PKS_YOLO=1 (or CLI ``--yolo``) disables this guard entirely.
# PKS_AVOID_SUDO=true rejects sudo/su/pkexec/doas regardless of YOLO.
#
# Only active in CLI headless mode (PKS_TUI_MODE != "true").
#
...
def _yolo_enabled() -> bool:
    return os.getenv("PKS_YOLO", "").strip().lower() in ("1", "true", "yes")


def _avoid_sudo_enabled() -> bool:
    """Operator policy: reject privilege-escalation shell commands (see module docstring)."""
    return os.getenv("PKS_AVOID_SUDO", "").strip().lower() in ("1", "true", "yes", "on")


def avoid_sudo_command_blocked(command: str) -> tuple[bool, str]:
    """If ``PKS_AVOID_SUDO`` is on, block ``sudo``/``su``/``pkexec``/``doas`` regardless of YOLO.
    ...
    if _command_has_shell_level_sudo(command):
        return (
            True,
            "Command uses sudo; PKS_AVOID_SUDO is enabled — use non-privileged alternatives "
            "(no sudo/su/pkexec/doas).",
        )
    if re.search(r"\bpkexec\b", command, re.IGNORECASE):
        return (True, "Command uses pkexec; PKS_AVOID_SUDO is enabled.")
    if re.search(r"\bdoas\b", command, re.IGNORECASE):
        return (True, "Command uses doas; PKS_AVOID_SUDO is enabled.")
    if re.search(r"(?:^|[;&|]\s*)\bsu\b(?:\s+|$)", command, re.IGNORECASE):
        return (True, "Command uses su; PKS_AVOID_SUDO is enabled.")
    return (False, "")


def _is_guard_enabled() -> bool:
    # pks CTF build: run fully autonomous by default — never block the agent with a
    # sensitive-command confirmation prompt (that interrupts unattended CTF solving).
    # Re-enable the guard explicitly with PKS_SENSITIVE_GUARD=true if ever needed.
    if os.getenv("PKS_TUI_MODE", "").lower() == "true":
        return False
    if _yolo_enabled():
        return False
    return os.getenv("PKS_SENSITIVE_GUARD", "false").lower() == "true"
...
async def prompt_user_for_sensitive_command(
    ...
    """
    if (
        os.getenv("PKS_CONTINUOUS_OPS_NO_SUDO", "").strip().lower() in ("1", "true", "yes")
        and category == "sudo"
    ):
        return "reject"
...
async def _prompt_user_for_sensitive_command_body(
    ...
    # Lazy import to avoid the pks.tools.common <-> pks.util.user_prompts cycle
    ...
    if answer is None and not questionary_failed:
        if _continuous_ops_worker():
            ...
            return "reject"
        try:
            from pks.util.streaming import restore_terminal_state as _restore
