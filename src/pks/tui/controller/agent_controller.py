"""
AgentController -- agent lifecycle operations extracted from PKSTerminal.
...
Methods here were originally inlined in ``pks_terminal.py``; the App now
...
class AgentController:
    ...
    async def cleanup_before_exit(self) -> None:
        ...
        try:
            ...
            os.environ.pop("PKS_TUI_MODE", None)
    ...
    async def handle_parallel_pattern(self, pattern_num: int, initial_terminal: Any) -> None:
        ...
        try:
            from pks.agents.patterns import get_parallel_patterns
    ...
    async def apply_team_selection(
        ...
        for idx in range(desired_n):
            ...
            try:
                from pks.repl.commands.agent import _sync_tui_agent_selection
                old_env = os.getenv("PKS_ACTIVE_COMMAND_TERMINAL", "")
                os.environ["PKS_ACTIVE_COMMAND_TERMINAL"] = str(term_num)
                _sync_tui_agent_selection(agent_for_term)
                if old_env:
                    os.environ["PKS_ACTIVE_COMMAND_TERMINAL"] = old_env
                else:
