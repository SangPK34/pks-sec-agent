"""
InputController -- keyboard/command routing extracted from PKSTerminal.
...
routing decisions that the App layer (``PKSTerminal``) acts upon.
"""
...
class InputController:
    ...
    def route(self, raw_command: str) -> RouteDecision:
        ...
        if (
            os.environ.get("PKS_META_AGENT", "false").lower() == "true"
    ...
    def parse_terminal_target(args: List[str]) -> Tuple[List[str], Optional[int]]:
        ...
        """
        from pks.tui.utils.terminal_parser import parse_terminal_target
