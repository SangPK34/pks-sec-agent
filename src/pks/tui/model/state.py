...
Extracted from PKSTerminal.__init__ (pks_terminal.py) so that state can be
inspected, serialized, and tested without instantiating the Textual App.

The PKSTerminal class keeps a single ``TUIState`` instance and delegates
...
@dataclass
class TUIState:
    """Mutable state bag for the PKS TUI.

    Every field corresponds to an instance variable that previously lived
    directly on ``PKSTerminal``.  Grouping them here makes the state
    ...
    theme_cycle: List[str] = field(default_factory=lambda: [
        ...
        "solarized-light", "solarized-dark", "pks-dark", "nature",
    ])
