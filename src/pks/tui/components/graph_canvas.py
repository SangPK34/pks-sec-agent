...
- Loads prior CTR runs from tools/cut_the_rope/ctr_pks/output/run_*/
...
from pks.tui.components.ctr_graph_viewport import GraphViewport, NodeSelected
...
class CTRCanvas(Container):
    ...
    def on_mount(self) -> None:
        ...
        preferred_env = os.getenv("PKS_CTR_DEFAULT_RUN") or os.getenv("PKS_CTR_DEFAULT_OUTPUT_DIR")
    ...
    def _output_base(self) -> str:
        ...
        env = os.getenv("PKS_CTR_DEFAULT_OUTPUT_DIR")
        if env:
        ...
        candidates = [
            ...
            os.path.join("tools", "cut_the_rope", "ctr_pks", "output"),  # legacy
            os.path.join(os.getcwd(), "tools", "cut_the_rope", "ctr_pks", "output"),  # legacy 2
        ]
        for p in candidates:
            if os.path.isdir(p):
                return p
    ...
    async def _build_graph_from_current_session(self) -> None:
        ...
        try:
            ...
            try:
                from pks.sdk.agents.run_to_jsonl import (
                    get_session_recorder,
                    get_token_stats,
                    load_history_from_jsonl,
                )
                from pks.ctr.experiment import process_in_memory_session
            ...
            distance_heuristic = os.getenv("PKS_CTR_DISTANCE_HEURISTIC") or None
            if isinstance(distance_heuristic, str):
                distance_heuristic = distance_heuristic.strip() or None
            is_ctf = os.getenv("PKS_CTR_IS_CTF", "false").lower() in {"true", "1", "yes"}
            attack_rates = self._parse_rate_env("PKS_CTR_ATTACK_RATES", "2")
            defense_rates = self._parse_rate_env("PKS_CTR_DEFENSE_RATES", "1")
