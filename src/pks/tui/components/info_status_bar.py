"""
Interactive status bar displaying tokens and context usage.
"""
...
from pks.tui.components.info_status_bar_updater import register_info_bar, unregister_info_bar
from pks.util import get_active_time, get_idle_time
...
class InfoStatusBar(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="status-bar-content"):
            ...
            # Token usage
            yield Static("", classes="info-section", id="token-info", markup=True)
            yield Static("•", classes="separator", markup=True)
    ...
    def on_mount(self) -> None:
        ...
        if terminal and hasattr(terminal, 'state') and hasattr(terminal.state, 'model_name'):
            self.model_name = terminal.state.model_name or os.getenv("PKS_MODEL", "default")
        else:
            self.model_name = os.getenv("PKS_MODEL", "default")
    ...
    def _update_info(self) -> None:
        ...
        try:
            ...
            try:
                from pks.util import get_model_input_tokens
            ...
            try:
                ...
                self.auto_compact = (
                    os.getenv("PKS_AUTO_COMPACT", "true").lower() == "true"
                )
            self.memory_enabled = compacted_memory_env_enabled()
            self.streaming_enabled = os.getenv("PKS_STREAM", "false").lower() == "true"
            ...
            try:
                ...
                token_widget.update(f"{self.input_tokens} → {self.output_tokens}")
            except Exception:
    ...
    def _update_workspace_info(self) -> None:
        ...
        try:
            ...
            # Get workspace info
            workspace = os.getenv("PKS_WORKSPACE", "")
            if workspace:
    ...
    def _update_token_info(self) -> None:
        ...
        try:
            widget = self.query_one("#token-info", Static)
            ...
            if self.input_tokens > 0 or self.output_tokens > 0:
                ...
                text.append("Tokens: --", style="dim white")
    ...
    def _update_queue_history_info(self) -> None:
        ...
        try:
            ...
            queue_count = 0
            try:
                from pks.repl.commands.queue import get_queue
            ...
            history_count = 0
            try:
                from pks.sdk.agents.models.openai_chatcompletions import message_history
    ...
    def _update_status_info(self) -> None:
        ...
        try:
            ...
            # Tracing
            if os.getenv("PKS_TRACING", "false").lower() == "true":
    ...
    def _update_responsive_display(self) -> None:
        ...
        try:
            ...
            if width < 60:
                # Very narrow - only show agent/model and tokens
                self._hide_sections(["#workspace-info", "#context-memory-info", "#queue-history-info", "#status-info"])
                self._show_sections(["#agent-model-info", "#token-info"])
            elif width < 80:
                # Narrow - hide queue/history and status
                self._hide_sections(["#queue-history-info", "#status-info"])
                self._show_sections(["#agent-model-info", "#workspace-info", "#token-info", "#context-memory-info"])
            elif width < 100:
                # Medium - hide status only
                self._hide_sections(["#status-info"])
                self._show_sections(["#agent-model-info", "#workspace-info", "#token-info", "#context-memory-info", "#queue-history-info"])
            else:
                # Wide - show all
                self._show_sections(["#agent-model-info", "#workspace-info", "#token-info", "#context-memory-info", "#queue-history-info", "#status-info"])
