...
from pks.tui.components.banner_widget import BannerWidget

# Import Observer pattern
from pks.tui.patterns.observer import EventType, terminal_event_manager

# Import error handling
try:
    from pks.tui.display.error_handler import (
...
try:
    # from pks.tui.core.async_streaming_fix import (
...
class UniversalTerminal(Container):
    ...
    UniversalTerminal .terminal-close-button:hover {
        /* Match .sidebar-toggle:hover from pks_terminal.py */
    ...
    def compose(self) -> ComposeResult:
        ...
        from pks.tui.components.streaming_status_bar import ActualActionBar
        ...
        yield RichLog(
            ...
            auto_scroll=True,
            wrap=True,
            max_lines=int(os.getenv("PKS_TUI_MAX_LINES", "0")) or None,  # Default None; can cap via env
        )
    ...
    def on_mount(self) -> None:
        ...
        try:
            ...
            try:
                from pks.repl.commands.virtualization import DockerManager
                dm = DockerManager()
                options = []
            ...
            try:
                from pks.repl.commands.model import get_predefined_model_names
                import os

                canonical_models = get_predefined_model_names()

                # Allow optional comma-separated extras via env
                model_list_env = os.getenv("PKS_MODEL_LIST", "")
                ...
                current_model = self.state.model_name or os.getenv("PKS_MODEL", "gpt-5.6-terra")
                if current_model:
            ...
            try:
                from pks.tui.routing.output_router import register_terminal_output
        ...
        from pks.tui.core.terminal_widget_registry import register_terminal_widget
        register_terminal_widget(self.terminal_id, self)
        
        # Register with tool streaming handler for live output in action bar
        try:
            from pks.tui.display.tool_streaming_handler import get_tool_streaming_handler
    ...
    async def _show_banner_cascade(self) -> None:
        ...
        from pks.repl.ui.banner import get_version

        version = get_version()

        # Full PKS banner - matching CLI colors
        banner_lines = [
            ...
            "[bold blue]                CCCCCCCCCCCCC               ++              IIIIIIIIII[/bold blue]",
            "",
            f"[bold blue]                              Cybersecurity AI (PKS), v{version}[/bold blue]",
    ...
    async def _configure_as_agent(self, agent_name: str) -> None:
        ...
        try:
            ...
            self.agent = get_agent_by_name(
                agent_name,
                agent_id=agent_id,
                model_override=os.getenv("PKS_MODEL", "gpt-5.6-terra")
            )
            ...
            if self.agent:
                ...
                # Show PKS banner with cascade effect - same as main terminal
                await self._show_banner_cascade()
    ...
    async def _configure_as_monitor(self) -> None:
        ...
        # Show PKS banner with cascade effect
        await self._show_banner_cascade()
    ...
    def on_select_changed(self, event) -> None:
        ...
        try:
            ...
            if isinstance(event.control, Select) and event.control.id == f"container-select-{self.terminal_id}":
                ...
                token_info = _get_agent_token_info()
                if token_info and token_info.get("agent_id"):
                    os.environ[f"PKS_ACTIVE_CONTAINER_FOR_{token_info.get('agent_id')}"] = selected
                if token_info and token_info.get("agent_name"):
                    import re
                    safe = re.sub(r"[^A-Za-z0-9_]+", "_", str(token_info.get("agent_name")))
                    os.environ[f"PKS_ACTIVE_CONTAINER_FOR_NAME_{safe}"] = selected
                # Also set default if nothing else
                if not selected:
                    os.environ.pop("PKS_ACTIVE_CONTAINER", None)
                    ...
                    os.environ["PKS_ACTIVE_CONTAINER"] = selected
    ...
    def _get_terminal_tooltip(self) -> str:
        ...
        # Temperature info
        from pks.sdk.agents.model_settings import DEFAULT_TEMPERATURE, DEFAULT_TOP_P
            ...
            if hasattr(app, 'session_manager') and app.session_manager:
                if self.terminal_number in app.session_manager.terminal_runners:
                    ...
                    if hasattr(runner, 'agent') and runner.agent:
                        ...
                        top_p = float(os.getenv("PKS_TOP_P", str(DEFAULT_TOP_P)))
        except:
    ...
    async def _run_agent_command(self, command: str) -> None:
        ...
        with redirect_stdout(captured_output):
            with redirect_stderr(captured_output):
                try:
                    from pks.sdk.agents import Runner
    ...
    async def _run_command_with_cli_logic(self, command: str) -> None:
        ...
        try:
            ...
            if command.startswith("/"):
                ...
                from pks.repl.commands import available_commands
    ...
    def finish_streaming_line(self, line_id: str, final_content: str, stats: Optional[Dict] = None) -> None:
        ...
        try:
            ...
            from pks.tui.display.panel_formatter import PanelFormatter
    ...
    def _log_streaming_error(self, line_id: str, error: Exception):
        ...
        try:
            import traceback
            with open(f"{_PKS_DEBUG_DIR}/pks_streaming_errors.log", "a") as f:
