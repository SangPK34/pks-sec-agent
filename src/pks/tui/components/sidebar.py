    ...
    def refresh_state(self) -> None:
        ...
        try:
            ...
            try:
                from pks.tui.pks_terminal import PKSTerminal
                app = self.app
                if isinstance(app, PKSTerminal) and hasattr(app, 'session_manager') and app.session_manager:
                    ...
                    for terminal_num, runner in session_manager.terminal_runners.items():
                        if runner.agent:
                            ...
                            # Build a friendly terminal label.
            ...
            for source_name in agents_info:
            ...
            for display_name, msgs in agents_info.items():
                ...
                entry = aggregated_view.setdefault(key, {"display": canonical_display, "messages": 0})
                    ...
                    if '=' in line and ('API_KEY' in line.upper() or line.upper().endswith('_KEY')):
                        if not line.startswith('#'):  # Skip comments
                            ...
                            if key_match:
                                ...
                                # Only include non-empty keys and exclude PKS_API_KEY
                                if key_value and key_name != 'PKS_API_KEY':
