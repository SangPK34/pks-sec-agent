"""
PKS Terminal - Textual App that wires together Model, View, and Controller.

This file was reduced from ~4,500 LOC to ~800 LOC by extracting:
- State management     -> pks.tui.model.state (TUIState)
- Session / history    -> pks.tui.model.session (SessionState)
- Input routing        -> pks.tui.controller.input_controller (InputController)
- Agent lifecycle      -> pks.tui.controller.agent_controller (AgentController)
- Layout / CSS / help  -> pks.tui.view.main_view
"""
...
# -- MVC layers ---------------------------------------------------------------
from pks.tui.model.state import TUIState
from pks.tui.model.session import SessionState
from pks.tui.controller.agent_controller import AgentController
from pks.tui.controller.input_controller import InputController, RouteKind
...
# -- Theme / config ------------------------------------------------------------
from pks.tui.theme import ThemeManager, THEMES
from pks.tui.config import TUIConfig
from pks.config import get_config as _get_pks_config

# -- Startup YAML config loader ------------------------------------------------
from pks.config_loader import (
    AgentsConfigError,
...
from pks.tui.components.stable_grid import StableTerminalGrid
from pks.tui.components.universal_terminal import UniversalTerminal
from pks.tui.components.sidebar import Sidebar, AgentDoubleClicked, TeamSelected
from pks.tui.components.prompt_input import PromptInput
from pks.tui.components.command_handler import CommandHandler
from pks.tui.components.agent_manager import AgentManager
from pathlib import Path
from pks.tui.components.agent_selector_panel import (
    AgentSelectorPanel,
    AgentSelectionConfirmed,
    AgentSelectionCancelled,
)
from pks.tui.components.agent_creator_panel import (
    AgentCreatorPanel,
    AgentCreationConfirmed,
    AgentCreationCancelled,
)
from pks.tui.components.info_status_bar import InfoStatusBar
from pks.repl.commands.parallel import ParallelConfig, PARALLEL_CONFIGS
from pks.sdk.agents.models.openai_chatcompletions_integration import (
...
from pks.tui.core.prompt_queue import PROMPT_QUEUE
...
# Theme resolved from PKSConfig (replaces os.getenv("PKS_THEME"))
_theme_name = _get_pks_config().tui_theme
...
def is_tui_mode() -> bool:
    ...
    return _get_pks_config().tui_enabled


# =============================================================================
# PKSTerminal -- the thin Textual App
# =============================================================================

class PKSTerminal(App):
    """Main PKS Terminal application wiring Model / View / Controller."""
    ...
    def __init__(self):
        ...
        PKSTerminal._current_app = self
        PKSTerminal._instance = self
    ...
    def on_mount(self) -> None:
        ...
        try:
            register_pks_themes(self.register_theme)
        except Exception:
            pass
        preferred = _get_pks_config().tui_theme or _tui_config.get_theme() or "tokyo-night"
        ...
        # Display manager
        from pks.tui.display import DisplayManager, DisplayMode
    ...
    def _initialize_after_mount(self) -> None:
        ...
        queue_file = _get_pks_config().queue_file
        if queue_file:
    ...
    async def _maybe_apply_startup_config(self) -> None:
        if self._startup_config_applied:
            return
        yaml_path = _get_pks_config().tui_startup_yaml
        if not yaml_path:
            return
    ...
    async def _apply_startup_config(self, yaml_path: str) -> None:
        ...
        runtime_prompt_override = _get_pks_config().tui_shared_prompt
    ...
    async def _load_and_process_queue_file(self, queue_file: str) -> None:
        ...
        try:
            ...
            if loaded > 0:
                mt.write(f"[cyan]Auto-loaded {loaded} prompts from PKS_QUEUE_FILE[/cyan]")
    ...
    async def _process_command(self, command: str) -> None:
        ...
        if kind == RouteKind.META_AGENT:
            from pks.tui.meta_agent_controller import get_meta_agent_controller
            ...
            for tn, name, _ in active_agents:
                ...
                if term:
                    def _exec(t=term, n=tn, c=command):
                        os.environ["PKS_BROADCAST_MODE"] = "true"
                        try:
                            ...
                            os.environ.pop("PKS_BROADCAST_MODE", None)
    ...
    def update_layout_indicator(self) -> None:
        ...
        self.title = f"PKS Terminal - {lm} Layout | Terminals: {tc}"
    ...
    # -- Session summary helpers (used from run_pks_tui) -----------------------
...
def run_pks_tui():
    ...
    if os.environ.get("PKS_META_AGENT", "false").lower() == "true":
        from pks.tui.meta_agent_controller import get_meta_agent_controller
