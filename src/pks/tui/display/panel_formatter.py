...
class PanelFormatter:
    ...
    def create_agent_panel(
        ...
        # Add compact token stats if available.
        if token_info and (
            ...
            try:
                ...
                token_info = normalize_token_info(token_info, default_model=default_model)
            except Exception:
                pass
            ...
            reasoning_tokens = token_info.get("interaction_reasoning_tokens", 0)
            try:
            ...
            if input_tokens > 0 or output_tokens > 0:
                ...
                if reasoning_tokens:
                    title_parts.append(f" [cyan]R:[/cyan] [yellow]{reasoning_tokens}[/yellow]")
        ...
        # Add token stats as footer if available.
        if token_info and (input_tokens > 0 or output_tokens > 0):
            ...
            if reasoning_tokens:
                stats_text.append(f" R: {reasoning_tokens}", style="yellow")
    ...
    def _create_token_display(cls, token_info: Dict, default_model: Optional[str] = None) -> Optional[Text]:
        ...
        enriched = normalize_token_info(token_info, default_model=default_model)
        ...
        if interaction_tokens == 0 and agent_tokens == 0:
            return None
