...
Extracted from PKSTerminal so the App class only wires things together.
All functions here are pure or take explicit widget arguments; nothing
imports from ``pks_terminal`` to avoid circular dependencies.
"""
...
from pks.tui.components.universal_terminal import UniversalTerminal  # noqa: F401
from pks.tui.components.sidebar import Sidebar
from pks.tui.components.prompt_input import PromptInput
from pks.tui.components.agent_selector_panel import AgentSelectorPanel
from pks.tui.components.agent_creator_panel import AgentCreatorPanel
from pks.tui.components.info_status_bar import InfoStatusBar
from pks.tui.components.graph_canvas import CTRCanvas

if TYPE_CHECKING:
    pass  # avoid circular imports with pks_terminal


# ---------------------------------------------------------------------------
# CSS -- previously the PKSTerminal.CSS class variable (~1 200 lines)
# ---------------------------------------------------------------------------

PKS_TERMINAL_CSS: str = """
...
# ---------------------------------------------------------------------------
# Widget composition -- called from PKSTerminal.compose()
# ---------------------------------------------------------------------------

def compose_main_layout() -> ComposeResult:
    """Yield the full widget tree for PKSTerminal.compose().
        ...
        with Container(id="main-container"):
            ...
            with TabbedContent(initial="terminal", id="main-tabs"):
                with TabPane("Terminal", id="terminal"):
                    ...
                    yield Container(
                        PromptInput(prompt="PKS>", id="main-input"),
                        id="input-area",
                    )
                ...
                with TabPane("Help", id="help"):
                    with Container(id="help-content"):
                        yield Static(
                            "[bold cyan]PKS Quick Start Guide[/bold cyan]",
                            id="help-title",
                            classes="help-title",
                            markup=True,
                        )
...
def register_pks_themes(register_fn: Any) -> None:
    """Register PKS-specific themes. *register_fn* is ``App.register_theme``."""
    nature = TextualTheme(
        name="nature",
        primary="#00ff9c",
        accent="#00ff9c",
...
\u2022 Enter: [bold]OPENAI_API_KEY[/bold] = your_openai_compatible_key_here
\u2022 Click [bold green]Save[/bold green] button
\u2022 Set [bold]OPENAI_BASE_URL[/bold] when using a compatible local gateway
...
\u2022 Select: [bold green]gpt-5.6-terra[/bold green] (recommended)
\u2022 [dim]Alternative: Use command[/dim] [bold]/model gpt-5.6-terra[/bold]
\u2022 [dim]The local catalog also includes other current frontier models[/dim]
...
\u2022 [dim]Recommendation:[/dim] Use [bold]root_agent[/bold] to execute simple work or route by task
...
\u2022 Creates a new terminal with model [bold]gpt-5.6-terra[/bold] and agent [bold]redteam_agent[/bold]
\u2022 Tooltip shows: [bold]Add new terminal[/bold]


[bold yellow]Start Chatting[/bold yellow]
[cyan]Begin your conversation:[/cyan]
\u2022 Type in the input field at bottom: [bold]PKS>[/bold]
...
\u2022 [bold]Terminal[/bold]: Chat interface and command execution
  - [dim]Add +:[/dim] Create a new terminal (defaults: [bold]gpt-5.6-terra[/bold] + [bold]redteam_agent[/bold])
...
\u2022 [bold]Stats Monitoring[/bold]: Track conversation metrics
  - [dim]View tokens and context usage[/dim]
...
- Start with [bold]gpt-5.6-terra[/bold] model and explore different agents to find your perfect AI assistant!
