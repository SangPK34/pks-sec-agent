...
from pks.sdk.agents import Agent, Runner, OpenAIChatCompletionsModel
...
from pks.util import update_agent_models_recursively
from pks.tui.display.manager import DisplayManager
from pks.tui.core.terminal_console import get_terminal_console, set_terminal_output
...
class TerminalRunner:
    ...
    def __init__(self, terminal_widget, config: TerminalConfig):
        ...
        if terminal_widget:
            ...
            from pks.tui.core.terminal_console import TerminalConsole, set_terminal_console
        ...
        async with self._agent_environment():
            try:
                ...
                if self.config.is_parallel and self.config.parallel_config:
                    ...
                    from pks.util import apply_compacted_memory_to_agent
                ...
                if self.agent and hasattr(self.agent, "model"):
                    ...
                    if hasattr(agent_model, "_display_context"):
                        from pks.tui.display.handoff_context import DisplayContext
    ...
    async def _run_agent_async(
        ...
        with open(f"{_PKS_DEBUG_DIR}/cai_recursion_debug.log", "a") as f:
        ...
        from pks.tui.display.context_preservation import ContextPreservingRunner
        ...
        from pks.tui.routing.output_router import set_terminal_context
        from pks.tui.core.terminal_tracking_async import set_current_terminal_id_async
        ...
        from pks.tui.display.handoff_context import DisplayContext, set_display_context
            ...
            async with ContextPreservingRunner(self.config.terminal_id):
                try:
                    ...
                    if result and hasattr(result, "final_output"):
                        ...
                        stream = os.getenv("PKS_STREAM", "false").lower() == "true"
                        if not stream:
                    ...
                    # Reset async context
                    from pks.tui.core.terminal_tracking_async import reset_current_terminal_id_async
                    reset_current_terminal_id_async(async_token)
                    # Clear display context
                    from pks.tui.display.handoff_context import clear_display_context
    ...
    async def switch_agent(self, agent_name: str) -> None:
        ...
        if os.getenv("PKS_TUI_MODE") == "true" and preserved_p_id:
            from pks.sdk.agents.simple_agent_manager import AGENT_MANAGER
        ...
        try:
            if self.agent and hasattr(self.agent, 'model'):
                ...
                if isinstance(self.agent.model, OpenAIChatCompletionsModel):
                    ...
                    if not silent and os.getenv('PKS_BROADCAST_MODE') != 'true':
