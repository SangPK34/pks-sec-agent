...
from pks.tui.core.terminal_runner import TerminalRunner, TerminalConfig
from pks.tui.core.agent_executor import AgentExecutor
...
class SessionManager:
    ...
    def get_instance(cls) -> Optional["SessionManager"]:
        ...
        # Fallback to the active PKSTerminal if available
        try:
            from pks.tui.pks_terminal import PKSTerminal

            app = getattr(PKSTerminal, "_instance", None) or getattr(
    ...
    async def execute_command(self, command: str, terminal_number: int = None) -> None:
        ...
        if terminal_number is None:
            ...
            tui_parallel_mode = os.getenv("PKS_TUI_MODE") == "true" and len(self.terminal_runners) > 1
    ...
    async def _execute_parallel_command(self, command: str) -> None:
        ...
        for idx, config in enumerate(PARALLEL_CONFIGS):
            ...
            if terminal_number in self.terminal_runners:
                ...
                from pks.tui.display.context_preservation import ensure_terminal_context
                ...
                async def run_with_terminal_context(runner, input_text, terminal_id):
                    ...
                    from pks.tui.routing.output_router import route_to_terminal, set_terminal_context
