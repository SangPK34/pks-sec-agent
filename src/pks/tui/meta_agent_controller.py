...
Only active when PKS_META_AGENT=True.
...
# Only activate if PKS_META_AGENT=True
META_AGENT_ENABLED = os.getenv("PKS_META_AGENT", "false").lower() == "true"

# Create a thread pool for blocking operations
executor = ThreadPoolExecutor(max_workers=2)

# Meta Agent System Prompt with comprehensive command documentation
META_AGENT_SYSTEM_PROMPT = """You are the Meta Agent, a hidden orchestrator that operates above the PKS TUI (Terminal User Interface).
...
2. DO NOT change models - the system uses PKS_MODEL automatically
...
### Model Management (DO NOT USE - System manages PKS_MODEL automatically)
- Models are controlled by PKS_MODEL environment variable
...
class MetaAgentController:
    ...
    def __init__(self):
        ...
        # Use PKS_MODEL as default, fallback to PKS_META_MODEL, then to gpt-5.6-terra
        self.model = os.getenv("PKS_META_MODEL", os.getenv("PKS_MODEL", "gpt-5.6-terra"))
        ...
        self._auto_close_grace = float(os.getenv("PKS_META_AUTOCLOSE_GRACE", "1.5"))
    ...
    def _attach_tui_hooks(self) -> None:
        ...
        if self._tui_hooks_attached or not self.tui_app_ref:
            return
        try:
            from pks.tui.patterns.observer import terminal_event_manager, EventType
