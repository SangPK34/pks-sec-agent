"""
PKS-CTR Integration Pipeline - Main Orchestration and Experimental Framework

This module serves as the primary entry point for the PKS-CTR integration, providing 
...
======================
1. Log Processing: Parse JSONL conversation logs with token tracking
2. LLM Graph Extraction: Use PKS agents to generate attack graph structures  
3. Probability Calculation: Compute edge exploitation probabilities based on
   conversation metrics (tokens and message distance)
...
===============================
Offline Mode: P_offline,i = W_msg × S_msg_norm + W_tokens × S_tokens_norm
- Supports both global normalization (across all logs) and individual normalization
- Handles token consumption and temporal message distances
...
INTEGRATION POINTS:
==================
- PKS SDK: Agent framework and model abstraction
...
try:
    # Always prefer Agg when running inside PKS to render to files only.
...
import pickle
import io
from pks.sdk.agents import Agent, OpenAIChatCompletionsModel, Runner
from pks.sdk.agents.model_settings import ModelSettings
from openai import AsyncOpenAI
from pks.sdk.agents.run_to_jsonl import load_history_from_jsonl
# from pks.sdk.agents.run_to_jsonl import load_history_from_json_legacy as load_history_from_jsonl
from pks.sdk.agents.run_to_jsonl import get_token_stats
from pks.ctr.attack_graph import create_graph_from_agent_output, plot_attack_graph
from pks.ctr.probability_computation import compute_edge_probabilities_offline
from pks.ctr.visualization import visualize_baseline_results
from pks.ctr.paths import get_ctr_output_base_dir
import litellm


from pks.ctr.core import main as ctr_core_main
...
# os.environ['PKS_AUTO_COMPACT'] = 'false'
...
def parse_messages_history(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Args:
        messages: List of PKS message dicts (user/assistant/tool etc.)
...
def save_run_information(model_name_log, input_log, inference_time, ctr_time, plotting_time, total_time, graph_structure_output, run_dir, message, probabilities, system_prompt=None):
    """
    Save detailed run information including timing measurements to output files.
...
def run_ctr_baseline_analysis(graph_structure, edge_probabilities, output_dir, attack_rate_list, defense_rate_list):
    ...
    """
    from pks.ctr.core import find_and_add_entry_node, generate_game_elements
    ...
    from pks.ctr.core import merge_targets_with_multi_edges
...
async def extract_attack_graph(input_log: Optional[str],
    ...
    or security assessment conversation logs using PKS's agent framework. This function
    ...
    4. Agent Configuration: Set up PKS agent with model-specific optimizations
    5. LLM Inference: Generate structured graph output with timing tracking
    ...
    Returns:
        tuple: (graph_structure, filtered_messages, inference_time_sec, resource_weight, prompt_used)
            - graph_structure: Pydantic GraphStructure object with nodes/edges
            - filtered_messages: List of processed conversation messages
            - inference_time_sec: LLM inference duration
            - resource_weight: Reserved compatibility value, currently zero
    ...
    Raises:
        ...
        RuntimeError: If LLM inference fails or PKS agent encounters errors
...
Add as many nodes and edges as they represent the pentesting exercise! 
{
  ...
  "edges": [
    ...
    return graph_structure.final_output, filtered_log, inference_time, instructions
...
def compute_initial_to_log_probability(starting_node, first_vulnerable_node, filtered_log, total_tokens_all_logs, total_number_messages):
    """
...
def create_multi_log_graphs(results, total_tokens_all_logs, total_number_messages, main_run_dir, attack_rates, defense_rates):
    """
    ...
    for idx, result in enumerate(results, 1):
        ...
        if not has_vulnerable_nodes:
            ...
            first_prob = compute_initial_to_log_probability(
                ...
                total_tokens_all_logs, total_number_messages
            )
...
async def process_in_memory_session(
    ...
    Specialized handler for active PKS sessions that extracts attack graphs
    ...
    Args:
        messages: List of conversation messages from active PKS session
        ...
        distance_heuristic: Optional distance metric for analysis:
            ...
            - 'hybrid': Balanced token/message weighting
    ...
    model_name_log = os.getenv('PKS_MODEL', model_name)
    total_tokens_real = 0
    ...
    if not used_token_counts:
        # Fallback: estimate tokens heuristically
    ...
    # Apply the selected distance heuristic.
    if distance_heuristic == 'message_uniform':
        # Uniform weight per message
        total_tokens_heuristic = len(messages) * 100  # Fixed tokens per message
    elif distance_heuristic == 'hybrid':
        # Balance tokens and message count.
        msg_weight = len(messages) * 50
        total_tokens_heuristic = int(total_tokens_real * 0.7 + msg_weight * 0.3)
    else:
    ...
    graph_structure, filtered_log, inference_time, instructions = await extract_attack_graph(
        input_log=None,
        is_ctf=is_ctf,
        attack_rate_list=attack_rate_list,
        defense_rate_list=defense_rate_list,
...
def _debug_display_messages(messages: List[Dict[str, Any]], token_counts: Optional[Dict[str, Any]] = None):
    ...
    from pks.repl.commands.history import HistoryCommand
...
async def run(
    ...
    This function is called from ``pks.repl.commands.ctr`` via ``asyncio.run()``.
    It orchestrates the entire CTR analysis pipeline:
    1. Accepts conversation data from PKS (either in-memory or from JSONL files), gen graph and stats
    ...
    Args:
        ...
        output_base_dir: Custom output directory (defaults to ~/.pks_cache/ctr/)
        messages: In-memory conversation history from active PKS session
    """
    ...
    for log_file in jsonl_files:        
        ...
        if using_in_memory:
            ...
            #
            model_name = os.getenv('PKS_MODEL')
            model_name_log = model_name  # assign to model_name_log for logging
            
            # Get real token counts from the file.
            (
                model_name,
                total_prompt_tokens,
            ...
            # #
            # from pks.ctr.probability_computation import get_log_tokens
    ...
    for idx, result in enumerate(results, 1):
        ...
        from pks.ctr.attack_graph import create_cleaned_graph_for_visualization
    ...
    if len(results) > 1:
        multi_log_graphs, multi_log_ctr_time, multi_log_plotting_time = create_multi_log_graphs(
            results,
            total_tokens_all_logs,
