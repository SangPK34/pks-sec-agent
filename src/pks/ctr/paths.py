...
def get_ctr_output_base_dir(override: Optional[str] = None) -> str:
    ...
    - Environment variable `PKS_CTR_OUTPUT_DIR`
    - System temporary directory at `<tempdir>/pks/ctr`
    """
    base = (
        override
        or os.getenv("PKS_CTR_OUTPUT_DIR")
        or os.path.join(tempfile.gettempdir(), "pks", "ctr")
    )
    try:
