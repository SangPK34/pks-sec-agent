...
Environment Variables:
    PKS_CTR_DIGEST_MODE: Set to "llm" for LLM interpretation, "algorithmic" for rule-based (default: llm)
    PKS_CTR_DIGEST_MODEL: Model to use for LLM interpretation (default: gpt-5.6-terra)
"""
...
- Maximum 350 words"""
    ...
    # Use LiteLLM with the configured OpenAI-compatible endpoint.
    import litellm

    model = os.getenv("PKS_CTR_DIGEST_MODEL", os.getenv("PKS_MODEL", "gpt-5.6-terra"))
    ...
    try:
        ...
        # Extract content, including reasoning-only provider responses.
...
async def get_ctr_digest_async(ctr_dir: str, mode: Optional[str] = None) -> str:
    ...
    Args:
        ctr_dir: Path to CTR results directory
        mode: Interpretation mode - "llm" or "algorithmic". If None, uses PKS_CTR_DIGEST_MODE env var
    ...
    # Determine mode
    if mode is None:
        mode = os.getenv("PKS_CTR_DIGEST_MODE", "llm").lower()
...
def get_ctr_digest(ctr_dir: str, mode: Optional[str] = None, use_cache: bool = True) -> Optional[str]:
    ...
    # This ensures that when /tmp/pks/ctr/latest points to a new run directory,
