...
This module implements the core probability computation engine for PKS-CTR integration,
...
P_offline,i = W_msg × S_msg_normalized + W_tokens × S_tokens_normalized

SCORING COMPONENTS:
==================
1. Message Distance Score (S_msg): Temporal proximity in conversation
...
2. Token Score (S_tokens): Information content along attack path
...
1. Global Normalization: Across all logs in multi-log analysis
   - Uses total tokens/messages from all processed logs
...
2. Individual Normalization: Per-log normalization
   - Uses only current log's tokens/messages for normalization
...
======================
- Token counting via litellm integration
- Support for various LLM model token counting (Qwen, GPT, Claude)
- Compatible with PKS JSONL log format and message structure
...
def compute_edge_probabilities_offline(
    ...
    analysis. Uses a weighted combination of temporal proximity and
    information content to model attack path likelihood.
    
    ALGORITHM OVERVIEW:
    1. Weight Validation: Normalize message and token weights
    ...
    Example:
        ...
        >>> probs = compute_edge_probabilities_offline(log, graph, 100, 1)
        >>> assert "0->1" in probs
        >>> assert 0.0 <= probs["0->1"] <= 1.0
    
    Note:
        This function is the mathematical core of the PKS-CTR probability model,
    ...
    total_weight = w_msg + w_tokens
    if total_weight <= 0:
        raise ValueError("Message and token weights must have a positive sum")
    w_msg /= total_weight
    w_tokens /= total_weight
