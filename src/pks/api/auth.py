"""Authentication helpers and storage for the PKS API backend.

This module implements a very small user database and per-device session
tokens, persisted to a JSON file under the PKS config directory
(`~/.pks/api_auth.json`). Both authentication flows described in the
...
header the API already uses for API keys (by default `X-PKS-API-Key`).
The access control helper in `app.py` accepts either:

- The static root API key from `ALIAS_API_KEY` / `PKS_API_KEY`, or
...
from pks.util import get_config_dir
...
class AuthManager:
    ...
    def _load_session_ttl() -> int:
        ...
        Controlled via the PKS_AUTH_SESSION_TTL_SECONDS env var.
        Defaults to 24 hours if not set or invalid.
        """
        default_ttl = 24 * 60 * 60
        raw = os.getenv("PKS_AUTH_SESSION_TTL_SECONDS")
        if raw is None:
            return default_ttl
        try:
            value = int(raw)
    ...
    def _ensure_default_user(self) -> None:
        ...
        print(
            "[PKS API][auth] Default login created for this installation:\n"
