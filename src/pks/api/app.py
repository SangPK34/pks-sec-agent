"""ASGI application factory for the PKS API backend."""
...
def _get_version() -> str:
    try:
        ...
        return os.getenv("PKS_VERSION", "dev")
...
def create_pks_api_app(
    *,
    session_manager: SessionManager | None = None,
    command_executor: CommandExecutor | None = None,
) -> FastAPI:
    os.environ.setdefault("PKS_API_MODE", "true")

    app = FastAPI(
        title="PKS API",
        version=_get_version(),
        description="Backend HTTP state for PKS",
        docs_url="/api/docs",
        openapi_url="/api/openapi.json",
        redoc_url="/api/redoc",
    )

    cors_origins = os.getenv("PKS_API_CORS", "*")
    if cors_origins:
        origins: List[str]
    ...
    api_key_header_name = os.getenv("PKS_API_KEY_HEADER", "X-PKS-API-Key")
    ...
    def _require_api_key(
        ...
        Authentication is disabled unless a root API key is configured via
        ``PKS_API_KEY``. When a root key is present, we accept:

        - The exact root key value, or
        - Any valid ``session_token`` previously issued by :class:`AuthManager`.
        """
        root_key = os.getenv("PKS_API_KEY")
        log_auth = os.getenv("PKS_API_LOG_AUTH", "false").lower() in ("1", "true", "yes")
    ...
    if os.getenv("PKS_API_LOG_REQUESTS", "false").lower() in ("1", "true", "yes"):
    ...
    def list_models() -> ModelsResponse:
        from pks.repl.commands.model import get_all_predefined_models
    ...
    async def send_message_stream(
        ...
        async def _gen():
            ...
            try:
                from pks.sdk.agents.run import Runner, DEFAULT_MAX_TURNS as _DEF_TURNS
    ...
    async def send_message_stream_tokens(
        ...
        from pks.sdk.agents.run import Runner, DEFAULT_MAX_TURNS
        ...
        if getattr(payload, "mcp_sse", None):
            try:
                from pks.sdk.agents.mcp.server import MCPServerSse, MCPServerSseParams
                default_timeout = float(os.getenv("PKS_MCP_SSE_TIMEOUT", "5"))
                default_read_timeout = float(os.getenv("PKS_MCP_SSE_READ_TIMEOUT", str(60 * 5)))
    ...
    def _run_litellm_title_summary(
        *,
        ...
        """Call LiteLLM with gpt-5.6-terra and a single tool call to produce title and summary.
        ...
        # Use the same OpenAI-compatible endpoint as the interactive runtime.
        kwargs = {
            "model": _os.getenv("PKS_MODEL", "gpt-5.6-terra"),
            "messages": llm_messages,
            "tools": [tool],
            "tool_choice": "required",
            "temperature": 0.2,
            "top_p": float(_os.getenv("PKS_TOP_P", "1.0")),
            "api_base": _os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1"),
