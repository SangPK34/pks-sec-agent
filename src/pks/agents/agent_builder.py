...
import textwrap
from pks.agents.available_tools import AVAILABLE_TOOLS
...
class AgentBuilder:
    ...
    def build_agent_file(cls, config: Dict[str, Any]) -> str:
        ...
        code = [
            'load_dotenv()',
            'model_name = os.getenv("PKS_MODEL", "gpt-4o-mini")',
            '',
            '# System prompt',
    ...
    def save_agent_file(cls, config: Dict[str, Any], base_path: str = None) -> str:
        """Save agent file to disk (defaults to ~/.pks/agents/)"""
        if base_path is None:
            base_path = os.path.join(os.path.expanduser("~"), ".pks", "agents")
