...
A re-interpretation for PKS of the original CodeAct concept
from the paper "Executable Code Actions Elicit Better LLM Agents"
at https://arxiv.org/pdf/2402.01030.

Briefly, the CodeAgent PKS Agent uses executable Python code to
...
# Local imports
from pks.agents.meta.local_python_executor import (
...
class CodeAgent(Agent):
    ...
    def __init__(  # pylint: disable=too-many-arguments,too-many-locals # noqa: E501
        ...
        object.__setattr__(self, "pks_instance", None)
...
"""
    ...
    def process_interaction(
        ...
        Args:
            pks_instance (object):
                The PKS instance that is calling the CodeAgent
            messages (List[Dict]):
        ...
        try:
            ...
            code, completion = self._generate_code(pks_instance, messages, debug)
            result = self._execute_code(code, debug)
    ...
    def _generate_code(
        self, pks_instance: object, messages: List[Dict], debug: bool = False
    ) -> str:
        """
        ...
        try:
            ...
            completion = pks_instance.get_chat_completion(
                agent=self,
    ...
    def run(
        ...
        aligning with PKS's expectations.
...
codeagent = CodeAgent(
    ...
    # execution_timeout=int(os.getenv('PKS_CODE_TIMEOUT', '30')),  # Get
