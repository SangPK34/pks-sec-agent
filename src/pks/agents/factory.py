...
Uses PKSConfig singleton for configuration instead of scattered os.getenv() calls [S].
"""
...
from pks.sdk.agents.logger import logger
...
def create_generic_agent_factory(
    ...
    Args:
        agent_module_path: Full module path to the agent (e.g., 'pks.agents.ctf_agent')
        agent_var_name: Name of the agent variable in the module (e.g., 'ctf_agent')
    ...
    def factory(
        ...
        # Get model configuration from PKSConfig singleton [S]
        cfg = get_config()
        ...
        if not model_name:
            ...
            model_name = os.getenv(f"PKS_{agent_key}_MODEL")

        if not model_name:
            # Third priority: global PKS_MODEL via PKSConfig
        ...
        # Only when explicitly enabled via PKS_TOOL_REGISTRY_AUTO=true.
        ...
        if cfg.tool_registry_auto:
            try:
                from pks.tool_registry import TOOL_REGISTRY
...
def discover_agent_factories() -> Dict[str, Callable[[], Agent]]:
    ...
    for importer, modname, ispkg in pkgutil.iter_modules(
        pks.agents.__path__, pks.agents.__name__ + "."
    ):
        if ispkg:
    ...
    patterns_path = os.path.join(os.path.dirname(pks.agents.__file__), "patterns")
    if os.path.exists(patterns_path):
        for importer, modname, ispkg in pkgutil.iter_modules(
            [patterns_path], pks.agents.__name__ + ".patterns."
        ):
            if ispkg:
                continue
    ...
    personal_path = os.path.join(os.path.dirname(pks.agents.__file__), "personal")
    if os.path.exists(personal_path):
        for importer, modname, ispkg in pkgutil.iter_modules(
            [personal_path], pks.agents.__name__ + ".personal."
        ):
            if ispkg:
                continue
...
def get_agent_factory(agent_name: str) -> Callable[[], Agent]:
    ...
    if agent_name_lower == "selection_agent":
        agent_name_lower = "root_agent"
    elif agent_name_lower in {"one_tool", "one_tool_agent"}:
        agent_name_lower = "ctf_agent"

    if agent_name_lower == "ctf_agent":
        return create_generic_agent_factory("pks.agents.ctf_agent", "ctf_agent")
