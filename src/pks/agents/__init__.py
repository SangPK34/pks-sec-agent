"""
PKS agents abstraction layer

PKS abstracts its cybersecurity behavior via agents and agentic patterns.
...
from pks.agents.flag_discriminator import flag_discriminator, transfer_to_flag_discriminator
from pks.sdk.agents import Agent
from pks.sdk.agents.handoffs import handoff

# Extend the search path for namespace packages (allows merging)
__path__ = pkgutil.extend_path(__path__, __name__)

# Get model from environment or use default
model = os.environ.get("PKS_MODEL", "gpt-5.6-terra")
...
def canonical_agent_key(agent_name: str) -> str:
    """Return the public catalog key while preserving legacy aliases."""
...
def get_available_agents() -> Dict[str, Agent]:  # pylint: disable=R0912  # noqa
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if isinstance(attr, Agent) and not attr_name.startswith("_"):
                    # Present the historical one_tool_agent under its domain name.
                    agent_name = canonical_agent_key(attr_name)
    ...
    from pks.agents.patterns import PATTERNS
    ...
    try:
        from pks.agents import META_AGENT_TOGGLE as _MAT
        if _MAT and "meta_agent" not in agents_to_display:
            agents_to_display["meta_agent"] = _MAT
    except Exception:
        pass

    # One canonical order for every catalog consumer: /agent list, numeric
    # selection, autocomplete, /agent info, and TUI selectors. Sort by the
    # human-facing Agent.name rather than module/import/key insertion order.
    # The canonical key is a deterministic tie-breaker for duplicate names.
    return dict(
        sorted(
            agents_to_display.items(),
            key=lambda item: (
                str(getattr(item[1], "name", item[0])).strip().casefold(),
                str(item[0]).casefold(),
            ),
        )
    )
...
try:
    class MetaAgentToggle:
        name = "meta_agent"
        description = "Toggle PKS Meta Agent (global TUI orchestrator). Select to enable PKS_META_AGENT=True."
...
def get_agent_module(agent_name: str) -> str:
    ...
    Returns:
        ...
        is defined (e.g., 'pks.sdk.agents.basic')
    """
    if canonical_agent_key(agent_name) == "ctf_agent":
        return "pks.agents.one_tool"

    agent_name = canonical_agent_key(agent_name)
...
def get_all_agents() -> Dict[str, Agent]:
    """Compatibility alias for callers that need the discovered agent map."""
...
def get_agent_by_name(
    ...
    elif agent_name_normalized == "selection_agent":
        agent_name = "root_agent"
    else:
        agent_name = canonical_agent_key(agent_name_normalized)

    # Import the generic factory system
    from pks.agents.factory import get_agent_factory
    ...
    if hasattr(agent, "model") and hasattr(agent.model, "__class__"):
        try:
            ...
            if model_class.__name__ == "OpenAIChatCompletionsModel":
                ...
                try:
                    from pks.repl.commands.mcp import get_mcp_tools_for_agent
