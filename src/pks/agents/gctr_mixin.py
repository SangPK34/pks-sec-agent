...
from pks.sdk.agents.lifecycle import AgentHooks
from pks.sdk.agents.run_context import RunContextWrapper
...
class CTRHooks(AgentHooks):
    ...
    async def _run_ctr_background(self, in_memory_history, token_counts) -> None:
        try:
            ...
            from pks.ctr import experiment as ctr_experiment
            ...
            if self.last_ctr_results:
                from pks.ctr.digest import mark_ctr_run_complete
...
async def display_and_inject_results(
    ...
    try:
        ...
        from pks.ctr.digest import get_ctr_digest
        ...
        digest_mode = os.getenv("PKS_CTR_DIGEST_MODE", "llm")
        digest = get_ctr_digest(results_dir, mode=digest_mode)

        if digest:
            from pks.ctr.digest import update_session_digest
            session_id = None
            recorder = get_session_recorder()
            if recorder is not None:
                session_id = getattr(recorder, "session_id", None)
...
def make_gctr_agent(
    ...
    *,
    ...
    Args:
        ...
        n_interactions: CTR trigger threshold (default: PKS_GCTR_NITERATIONS or 5).
        team_label: Label used in CTR console output.
    """
    if n_interactions is None:
        n_interactions = int(os.getenv("PKS_GCTR_NITERATIONS", "5"))
