...
class _HandoffTask(BaseModel):
    ...
    Making the task explicit (a) forces the Selection Agent to state exactly what it
    ...
    task: str = Field(
        ...,
        description=(
            "One or two sentences: the CONCRETE task to delegate, phrased in the user's own "
            "intent. Do NOT add scope, drop requirements, or invent targets the user did not "
            "mention — pass the user's ask faithfully."
        ),
    )
...
def _announce_delegation(target_name: str, task: str) -> None:
    ...
    try:
        ...
        console.print(
            Panel(
                body,
                title=f"[bold yellow]Selection Agent  →  {target_name}[/bold yellow]",
                title_align="left",
                border_style="yellow",
                padding=(0, 1),
            )
