...
- For **PCAP** requests: produce real `.pcap`/`.pcapng` or state capture is blocked and give exact remediation (CAP_NET_RAW, `sudo setcap cap_net_raw+eip $(which dumpcap)`, PKS Docker with NET_RAW). Do not write openssl/curl transcripts into `packet_captures/` as stand-ins.
