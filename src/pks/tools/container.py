...
def _default_workspace_base() -> str:
    """Return the default workspace base: ~/.pks/workspace"""
    return os.path.join(os.path.expanduser("~"), ".pks", "workspace")
...
def _get_workspace_dir() -> str:
    ...
    Resolution order:
      1. PKS_WORKSPACE_DIR env var          (explicit override)
      2. ~/.pks/workspace/{PKS_WORKSPACE}   (named workspace)
      3. ~/.pks/workspace/                  (default)
    """
    base_dir_env = os.getenv("PKS_WORKSPACE_DIR")
    ...
    if workspace_name:
        if not all(c.isalnum() or c in ["_", "-"] for c in workspace_name):
            print(color(
                f"Invalid PKS_WORKSPACE name '{workspace_name}'. "
...
def _get_container_workspace_path() -> str:
    ...
    if workspace_name:
        if not all(c.isalnum() or c in ["_", "-"] for c in workspace_name):
            print(color(
                f"Invalid PKS_WORKSPACE name '{workspace_name}' for container. "
...
async def _run_docker_async(
    ...
    try:
        ...
        if stream:
            ...
            in_tui_mode = os.getenv("PKS_TUI_MODE") == "true"
