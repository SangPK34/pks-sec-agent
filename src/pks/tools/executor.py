...
from pks.output import OUTPUT, ToolStartEvent, ToolCompleteEvent, ToolErrorEvent
# PKSConfig integration [B] — centralized config replaces os.getenv
from pks.config import get_config as _get_config

# Instead of direct import
try:
    from pks.cli import START_TIME
except ImportError:
    START_TIME = None

# --- Sibling module imports ---
from pks.tools.streaming import (
    _get_idle_timeout,
    is_tool_streaming_enabled,
    _get_agent_token_info,
)
from pks.tools.container import (
    _get_workspace_dir,
...
async def _run_local_async(
    ...
    try:
        ...
        from pks.util.user_prompts import is_sudo_command, ensure_sudo_credentials
        ...
        if _get_config().tui_enabled:
            try:
                ...
                from pks.tui.core.terminal_tracking_async import get_current_terminal_id_async
                from pks.tui.display.live_output_capture import get_live_output_capture

                with open(f"{_PKS_DEBUG_DIR}/pks_live_capture_debug.log", "a") as f:
                    import datetime
                ...
                import traceback
                with open(f"{_PKS_DEBUG_DIR}/pks_live_capture_error.log", "a") as f:
                    f.write(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] Failed to set up live capture\n")
                    f.write(f"Error: {e}\nTraceback: {traceback.format_exc()}\n")

        if stream:
            from pks.util import start_tool_streaming, update_tool_streaming, finish_tool_streaming
            ...
            try:
                ...
                try:
                    async def read_and_stream():
                        ...
                        if _get_config().tui_enabled:
                            with open(f"{_PKS_DEBUG_DIR}/pks_timeout_debug.log", "a") as f:
                                import datetime
        ...
        if stream and call_id:
            from pks.util import finish_tool_streaming
            parts = command.strip().split(" ", 1)
            cmd_var = parts[0] if parts else ""
    ...
    try:
        ...
        if stream:
            ...
            if capture_id and _get_config().tui_enabled:
                try:
                    from pks.tui.display.execution_interceptor import run_with_live_capture
...
async def run_command_async(
    ...
    from pks.cli import ctf_global
    ctf = ctf_global
    ...
    try:
        ...
        try:
            ...
            if token_info:
                if token_info.get("agent_id"):
                    agent_id_env = os.getenv(f"PKS_ACTIVE_CONTAINER_FOR_{token_info.get('agent_id')}", "")
                if token_info.get("agent_name"):
                    import re
                    safe_name = re.sub(r"[^A-Za-z0-9_]+", "_", str(token_info.get("agent_name")))
                    agent_name_env = os.getenv(f"PKS_ACTIVE_CONTAINER_FOR_NAME_{safe_name}", "")
            active_container = agent_id_env or agent_name_env or (_get_config().active_container or "")
            if not active_container:
                active_container = os.getenv("PKS_ACTIVE_CONTAINER_DEFAULT", "")
        except Exception:
        ...
        # PKS prompt, manifesting as "Enter doesn't work" after long agent
        # turns. The agent still sees the permission-denied error and can
        # re-issue the command with an explicit ``sudo`` prefix.
        import asyncio as _aio
        from pks.util.user_prompts import is_sudo_command as _is_sudo, output_needs_sudo, prompt_sudo_elevation
        _cops_no_sudo = os.getenv("PKS_CONTINUOUS_OPS_NO_SUDO", "").strip().lower() in ("1", "true", "yes")
        _auto_sudo = os.getenv("PKS_AUTO_SUDO_ELEVATION", "").strip().lower() in ("1", "true", "yes", "on")
        ...
        from pks.tools.evidence.capture_notice import apply_packet_capture_notice
...
def run_command(
    ...
    try:
        ...
        # --- Environment detection (via PKSConfig) ---
        ...
        # ``PKS_AUTO_SUDO_ELEVATION`` for the same reason as the async path:
        # the interactive getpass silently hijacks the user's next keystrokes.
        from pks.util.user_prompts import is_sudo_command, output_needs_sudo, prompt_sudo_elevation
        _cops_no_sudo = os.getenv("PKS_CONTINUOUS_OPS_NO_SUDO", "").strip().lower() in ("1", "true", "yes")
        _auto_sudo = os.getenv("PKS_AUTO_SUDO_ELEVATION", "").strip().lower() in ("1", "true", "yes", "on")
