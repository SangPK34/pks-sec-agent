...
by PKS agents as a "read the web" tool after running a search.
...
* SSRF guard blocks loopback / RFC1918 / link-local / cloud-metadata hosts
  and non-http(s) schemes by default. Bypass via ``PKS_FETCH_ALLOW_INTERNAL``.
...
from pks.agents.guardrails import sanitize_external_content
...
def _check_ssrf(url: str, *, allow_internal: bool) -> str:
    ...
    # Cloud metadata is ALWAYS blocked, regardless of PKS_FETCH_ALLOW_INTERNAL.
...
async def fetch_url(
    ...
    * Requests to private / loopback / link-local / cloud-metadata hosts are
      blocked by default. Set ``PKS_FETCH_ALLOW_INTERNAL=true`` to allow
    ...
    * JavaScript is NOT executed; single-page apps that require client-side
    ...
    Args:
        ...
        timeout: Per-request timeout in seconds. ``0`` uses ``PKS_FETCH_TIMEOUT``
            (default 20s).

    Returns:
        The extracted content, wrapped in PKS's external-content delimiters
        to defang any embedded prompt-injection payload. Errors are returned
        as sanitized strings; this function never raises.
    """
    allow_internal = _bool_env("PKS_FETCH_ALLOW_INTERNAL", False)
    max_bytes = _int_env("PKS_FETCH_MAX_BYTES", 5_242_880)
    effective_timeout = timeout if timeout > 0 else _int_env("PKS_FETCH_TIMEOUT", 20)
    user_agent = os.getenv("PKS_FETCH_USER_AGENT") or None
    ...
    try:
        ...
        return sanitize_external_content(
            ...
            "Set PKS_FETCH_ALLOW_INTERNAL=true to permit internal targets."
        )
