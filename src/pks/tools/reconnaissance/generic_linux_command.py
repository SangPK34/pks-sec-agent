...
from pks.tools.common import (run_command, run_command_async,
                              list_shell_sessions,
                              get_session_output,
                              terminate_session,
...
def _compress_output_for_model(result: str, command: str) -> str:
    ...
    Only enabled when PKS_CTX_TRUNC=true environment variable is set.
    ...
    # Only truncate if PKS_CTX_TRUNC=true
    if os.getenv("PKS_CTX_TRUNC", "").lower() != "true":
        return result
...
async def generic_linux_command(
    ...
    Args:
        ...
        working_directory: Optional absolute directory to use as the shell working directory
                 ...
                 this directory instead of the PKS workspace. Ignored when using an active
    ...
    Environment Detection:
        ...
        - Container: Commands run in Docker containers when PKS_ACTIVE_CONTAINER is set
    ...
    if command.strip() == "env info" or command.strip() == "environment info":
        ...
        try:
            from pks.cli_setup import ctf_global
    ...
    guardrails_enabled = os.getenv("PKS_GUARDRAILS", "false").lower() != "false"
    ...
    # 2. PKS_TOOL_TIMEOUT environment variable
    ...
    if effective_timeout is not None:
        ...
        env_timeout = os.getenv("PKS_TOOL_TIMEOUT")
        if env_timeout:
            try:
                effective_timeout = int(env_timeout)
                timeout_source = "env:PKS_TOOL_TIMEOUT"
            except ValueError:
    ...
    # PKS_STREAM is for LLM inference streaming (default: false)
    stream = is_tool_streaming_enabled()

    # Simple heuristic: If PKS_PARALLEL > 1 AND we have a P agent ID, disable streaming
    ...
    # Respect PKS_GUARDRAILS environment variable
    guardrails_enabled = os.getenv("PKS_GUARDRAILS", "false").lower() != "false"
    ...
    # Sudo password handling is delegated to pks.util.user_prompts (in executor.py).
    ...
    if sensitive:
        ...
        if action == "cancel":
            from pks.sdk.agents.exceptions import UserCancelledCommand
    ...
    if cwd_override and os.getenv("PKS_ACTIVE_CONTAINER"):
        return (
            "Error: working_directory applies only to local host execution (no active Docker container). "
            "Unset PKS_ACTIVE_CONTAINER or use absolute paths inside the container in ``command``."
        )
    ...
    skip_sudo_retry = (
        ...
        and result.startswith("[PKS PACKET-CAPTURE FAILURE]")
    )
    ...
    # Enhanced sanitization for curl/wget responses - respect PKS_GUARDRAILS setting
    guardrails_enabled = os.getenv("PKS_GUARDRAILS", "false").lower() != "false"
...
from pks.tool_registry import TOOL_REGISTRY  # noqa: E402
