...
and avoids writing to shared JSON files under ~/.pks.
...
The tool is a no-op unless `PKS_PLAN=true` is set in the environment.
"""

import os
from pks.sdk.agents.tool import function_tool  # pylint: disable=import-error
...
async def Todo_list(todos: list | None = None) -> str:
    ...
    Behavior:
        ...
        - Requires environment variable PKS_PLAN=true to take effect.
    ...
    if not cfg.plan_enabled:
        return "Plan feature disabled. Set PKS_PLAN=true to enable plan tracking."
