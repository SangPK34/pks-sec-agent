"""
Agent Discovery Tool for Root Agent

This tool allows the root agent to dynamically discover and analyze
all available agents in the PKS system to make informed recommendations.
"""
...
from pks.sdk.agents import Agent, function_tool


@lru_cache(maxsize=1)
def _check_available_agents() -> Dict[str, Any]:
    """
    Check all available agents in the PKS system and return their detailed information.
    ...
    for _, name, _ in pkgutil.iter_modules(pks.agents.__path__, pks.agents.__name__ + "."):
        try:
    ...
    if os.path.exists(patterns_path):
        for _, name, _ in pkgutil.iter_modules([patterns_path], pks.agents.__name__ + ".patterns."):
            try:
                module = importlib.import_module(name)
