...
agent (``pks.agents.orchestration_agent``):
...
from pks.sdk.agents import RunConfig, Runner
from pks.sdk.agents.items import ItemHelpers
from pks.sdk.agents.model_settings import ModelSettings
from pks.sdk.agents.tool import function_tool
from pks.config import get_config
from pks.util._worker_silence import silence_worker_display

# === Constants ============================================================

def _configured_worker_max_turns() -> int:
    """Runner max_turns for each specialist worker (env ``PKS_ORCHESTRATION_WORKER_MAX_TURNS``)."""
    try:
...
# comfortably below the per-tool LLM memory cap.
...
async def run_dual_approach_contest(
    ...
    """Run two parallel exploratory approaches on the same user task (max 2 agents; worker turn budget from ``PKS_ORCHESTRATION_WORKER_MAX_TURNS``).

    Use only when comparing orthogonal methodologies, competing hypotheses, volatile evidence,
    or a high-risk fork before committing PKS to a single path. Tactical follow-up actions should
