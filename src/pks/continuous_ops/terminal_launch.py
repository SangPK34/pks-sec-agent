...
def spawn_external_terminal(backend: str, title: str, command: str) -> bool:
    ...
    try:
        if backend == "x-terminal-emulator":
            ...
            tail = "; echo; echo '[PKS] Continuous ops worker finished.'; read -r -p 'Press Enter to close...'"
        ...
        if backend == "gnome-terminal":
            subprocess.Popen(
                [
                    ...
                    "bash",
                    "-lc",
                    f"{command}; echo; echo '[PKS] Continuous ops worker finished.'; read -r -p 'Press Enter to close...'",
                ],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )
        ...
        if backend == "osascript":
            osa_cmd = (
                ...
                + json.dumps(f"{command}; echo; echo '[PKS] Continuous ops worker finished.'")
            )
            subprocess.Popen(
                ["osascript", "-e", osa_cmd],
                stdout=subprocess.DEVNULL,
