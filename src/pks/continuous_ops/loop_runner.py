...
from pks.continuous_ops.mechanical_summary import SUMMARY_REL, write_mechanical_summary_from_log
from pks.continuous_ops.task_queue import (
...
from pks.continuous_ops.tick_context import (
...
_PLAIN_LOG_FLAG = "PKS_CONTINUOUS_OPS_SYSTEMD_PLAIN_LOG"


def _pks_subprocess_env_from_os_environ() -> dict[str, str]:
    """Copy of the process environment for ``pks`` ticks; plain logs when *_PLAIN_LOG_FLAG* is set (systemd unit)."""
    env = os.environ.copy()
...
PKS_TICK_WALL_RC = 124


def tick_wall_timeout_seconds(tick_seconds: int) -> float:
    """Maximum wall time for one ``pks`` subprocess: ``max(120, 2 * TICK)`` seconds."""
    return float(max(120, 2 * int(tick_seconds)))


def effective_privileged_worker(cfg: LoopConfig) -> bool:
    """True only when config asks for privileges and PKS_AVOID_SUDO is not forcing a non-root shell policy."""
    avoid = os.environ.get("PKS_AVOID_SUDO", "").strip().lower() in ("1", "true", "yes", "on")
...
class LoopConfig:
    ...
    def load(run_dir: Path) -> LoopConfig:
        ...
        return LoopConfig(
            ...
            pks_argv=[str(x) for x in raw["pks_argv"]],
...
def _infer_framework_dotenv_paths(cfg: LoopConfig) -> list[Path]:
    ...
    out: list[Path] = []
    for seed in (cfg.pks_argv[0] if cfg.pks_argv else "", cfg.python_interpreter):
        s = (seed or "").strip()
        if not s:
            continue
        try:
            ...
            continue
        try:
            if p.is_file() and p.parent.name == "bin" and p.parent.parent.name == "pks_env":
...
def _load_dotenv_for_loop(cfg: LoopConfig, run_dir: Path) -> None:
    """Load API keys before ``chdir`` so recovery pings match the ``pks`` subprocess."""
    try:
...
def _ping_model(py: str) -> bool:
    """Lightweight reachability check for the configured OpenAI-compatible endpoint."""
    ...
    code = (
        ...
        " if not key:\n"
        "  sys.exit(1)\n"
        " from pks.util.llm_api_base import resolve_openai_base_url\n"
        " m=os.environ.get('PKS_MODEL','gpt-5.6-terra')\n"
        " base=resolve_openai_base_url().rstrip('/')\n"
...
def _recover_until_api_ready(py: str) -> None:
    print(
        "[PKS continuous ops] Tick failed or API unreachable — pausing; "
        "will ping every 60s until the model responds.",
        file=sys.stderr,
    )
    while True:
        if _ping_model(py):
            print("[PKS continuous ops] Model reachable again — resuming iterations.", file=sys.stderr)
            return
        time.sleep(60)
...
def _write_tick_prompt_file(run_dir: Path, cfg: LoopConfig) -> str | None:
    ...
    cur.write_text("\n".join(parts) + "\n", encoding="utf-8")
    os.environ["PKS_SINGLE_SHOT_STDIN_PROMPT_FILE"] = str(cur.resolve())
    return task_id
...
def _run_pks_with_tee(
    ...
    *,
    ...
    """Run PKS; stream stdout+stderr to *log_file* and this process; kill after *timeout_sec* wall clock."""
    ...
    try:
        ...
        print(f"[PKS continuous ops] failed to spawn pks: {e}", file=sys.stderr)
        return 127
    assert p.stdout
    ...
    if pump.is_alive():
        print(
            f"[PKS continuous ops] tick wall limit ({timeout_sec:.0f}s = max(120, 2×TICK)) exceeded — "
            "terminating pks; next iteration will run after the usual sleep.",
            file=sys.stderr,
        )
        try:
            p.kill()
...
def _maintain_logs(py: str, logs_dir: Path) -> None:
    try:
        subprocess.run(
            [py, "-m", "pks.continuous_ops.log_maintenance", str(logs_dir)],
            check=False,
            timeout=600,
            capture_output=True,
        )
...
def _ensure_latest_log_placeholder(run_dir: Path) -> None:
    ...
    try:
        if not boot.is_file():
            boot.write_text(
                "[PKS continuous ops] Placeholder — first tick log not created yet.\n",
                encoding="utf-8",
            )
...
def _write_preloop_summary_stub(
    ...
    lines = [
        ...
        "anomalies_total: 0",
        "last_exit_code: -1",
        "session_note: Worker process reached the main loop; the first PKS subprocess tick has not finished yet. "
        ...
        f"Per-tick hard wall: max(120, 2×TICK) = {tick_wall:.0f}s; overrun kills the pks subprocess (exit {PKS_TICK_WALL_RC}).",
...
def _update_summary(
    ...
    lines = [
        ...
        "tick_note: Wall period is max(TICK, last tick duration) + recovery; TICK is minimum idle after each tick completes. "
        f"Per-tick hard wall: max(120, 2×TICK) seconds; overrun kills the pks subprocess (exit {PKS_TICK_WALL_RC}).",
        "live_log: Summary: watch -n2 cat logs/summary/summary.txt  |  Current tick: less +F logs/full/latest.log",
        "last_10_iterations:",
        tail_lines,
    ]
...
def _model_recovery_turn(
    ...
    *,
    ...
    os.environ["PKS_SINGLE_SHOT_STDIN_PROMPT_FILE"] = str(recf.resolve())
    env = os.environ.copy()
    env["PKS_SINGLE_SHOT_STDIN_PROMPT_FILE"] = str(recf.resolve())
    env["PYTHONUNBUFFERED"] = "1"
    print("[PKS continuous ops] Running model auto-recovery turn…", file=sys.stderr)
    rc = _run_pks_with_tee(
        [*cfg.pks_argv, "--prompt", "recovery"],
        reclog,
        env,
        run_dir,
    ...
    try:
        ...
        pass
    print("[PKS continuous ops] Auto-recovery turn finished (hint saved for next tick).", file=sys.stderr)
    if rc != 0:
        print(f"[PKS continuous ops] recovery turn exit={rc}", file=sys.stderr)
...
def main(run_dir: Path, *, entry_path: Path | None = None) -> int:
    ...
    os.environ["PKS_COPS_LOG_FULL_DAYS"] = str(cfg.log_full_days)
    os.environ["PKS_COPS_LOG_DELETE_AFTER_DAYS"] = str(cfg.log_delete_after_days)
    os.environ["PKS_CONTINUOUS_OPS_TICK_SECONDS"] = str(cfg.tick_seconds)
    ...
    # Config may still say privileged=true from older wizards; PKS_AVOID_SUDO (e.g. systemd unit) means no sudo/TTY.
    effective_privileged = effective_privileged_worker(cfg)
    if os.environ.get("PKS_AVOID_SUDO", "").strip().lower() in ("1", "true", "yes", "on") and cfg.privileged:
        print(
            "[PKS continuous ops] run_loop_config has privileged=true but PKS_AVOID_SUDO is set — "
            "skipping sudo bootstrap; ticks run without elevation (typical under systemd --user).",
            file=sys.stderr,
        )

    if not effective_privileged:
        os.environ["PKS_CONTINUOUS_OPS_NO_SUDO"] = "true"
    else:
        os.environ.pop("PKS_CONTINUOUS_OPS_NO_SUDO", None)

    # Worker agent is configurable via env or run config. Default: root_agent.
    # Root/orchestration agents need 'auto' routing; pinned workers (e.g. blueteam_agent)
    # run their own tools per tick without handoff noise.
    wt = (os.environ.get("PKS_CONTINUOUS_OPS_WORKER_AGENT_TYPE") or "").strip() or cfg.worker_agent_type
    wt = (wt or DEFAULT_AGENT_TYPE).strip() or DEFAULT_AGENT_TYPE
    os.environ["PKS_AGENT_TYPE"] = wt
    if wt == "selection_agent":
        wt = "root_agent"
        os.environ["PKS_AGENT_TYPE"] = wt
    os.environ["PKS_AGENT_ROUTE_MODE"] = "auto" if wt in ("root_agent", "orchestration_agent") else "pinned"
    os.environ["PKS_SINGLE_SHOT_CLI"] = "true"
    os.environ["PKS_CONTINUOUS_OPS_LOOP_CHILD"] = "1"
    ...
    if effective_privileged:
        ...
        print(
            "[PKS continuous ops] Privileged worker: validate sudo once for this terminal session.",
            file=sys.stderr,
        )
        try:
            r = subprocess.run(["sudo", "-v"], timeout=300)
            if r.returncode != 0:
                print("[PKS continuous ops] sudo -v failed — cannot run a privileged worker without sudo.", file=sys.stderr)
                return 1
        except (OSError, subprocess.TimeoutExpired):
            print("[PKS continuous ops] sudo -v failed — cannot run a privileged worker without sudo.", file=sys.stderr)
            return 1
    ...
    while True:
        ...
        if snap_path.is_file():
            os.environ["PKS_COPS_SNAPSHOT_IN"] = str(snap_path.resolve())
        else:
            os.environ.pop("PKS_COPS_SNAPSHOT_IN", None)
        os.environ["PKS_COPS_SNAPSHOT_OUT"] = str(snap_path.resolve())
        ...
        print(
            f"[PKS continuous ops] tick START iter={iteration} ts={_ts_human()} "
            f"log=logs/full/{logfull.name}  wall_limit={tick_wall:.0f}s  "
            "(watch summary: watch -n2 cat logs/summary/summary.txt)",
            file=sys.stderr,
        )

        env = _pks_subprocess_env_from_os_environ()
        env["PKS_SINGLE_SHOT_STDIN_PROMPT_FILE"] = os.environ.get("PKS_SINGLE_SHOT_STDIN_PROMPT_FILE", "")
        env["PYTHONUNBUFFERED"] = "1"
        argv = [*cfg.pks_argv, "--prompt", cfg.tick_prompt]
        last_rc = _run_pks_with_tee(argv, logfull, env, run_dir, timeout_sec=tick_wall)

        print(f"[PKS continuous ops] tick END   iter={iteration} ts={_ts_human()} exit={last_rc}", file=sys.stderr)
...
def _cli() -> int:
    ap = argparse.ArgumentParser(description="PKS Continuous Ops worker loop")
    ...
    try:
        ...
        print("[PKS continuous ops] Fatal error — traceback follows.", file=sys.stderr)
