"""Extra turns + snapshot export at end of a continuous-ops tick (``pks`` subprocess)."""
...
def _max_turns_per_tick() -> int:
    try:
        v = int((os.getenv("PKS_COPS_MAX_TURNS_PER_TICK") or "20").strip())
    except ValueError:
        v = 20
...
def run_continuous_ops_extra_turns(agent, console, force_until_flag, ctf_global) -> None:
    ...
    from pks.cli_headless import _run_single_agent
...
def export_loop_child_snapshot(agent) -> None:
    out = (os.getenv("PKS_COPS_SNAPSHOT_OUT") or "").strip()
    if not out:
        return
    ...
    if not hist:
        return
    try:
        from pks.continuous_ops.session_snapshot import export_snapshot
...
def maybe_import_snapshot_before_cli_loop(agent, history_key: str) -> None:
    ...
    inp = (os.getenv("PKS_COPS_SNAPSHOT_IN") or "").strip()
    ...
    if not path.is_file():
        return
    try:
        from pks.continuous_ops.session_snapshot import apply_snapshot_to_agent, load_snapshot_messages
