"""Conservative tick intervals from configurable TPM/RPM tiers."""
...
def resolve_rate_tier() -> str:
    """Return ``pro`` or ``edu`` from ``PKS_RATE_TIER`` (default: ``pro``)."""
    raw = (os.getenv("PKS_RATE_TIER") or "pro").strip().lower()
