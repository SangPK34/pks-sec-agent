...
class MissionPlan:
    ...
    #: ``planner_api`` = JSON from the configured chat endpoint; ``fallback_*`` = local heuristic plan.
...
def _planner_completion(
    *,
    ...
    """POST ``/chat/completions`` with retries on transient HTTP failures."""
    ...
    if not api_key:
        return _PlannerHttpOutcome(
            ...
            http_status=None,
            error_snippet=None,
            transport_error="OPENAI_API_KEY is not set",
        )
...
def parse_mission_with_planner(user_text: str) -> MissionPlan:
    """Call the configured OpenAI-compatible API for a mission plan."""
    tier = resolve_rate_tier()
