...
_PLANNER_MAX_ATTEMPTS = _env_int("PKS_MISSION_PLANNER_MAX_ATTEMPTS", 3, 1, 8)
_PLANNER_READ_TIMEOUT = _env_float("PKS_MISSION_PLANNER_TIMEOUT", 240.0, 30.0, 600.0)

from pks.continuous_ops.rate_plan import compute_base_tick_seconds, min_allowed_tick_seconds, resolve_rate_tier
...
class MissionPlan:
    ...
    #: ``planner_api`` = JSON from the Alias/PKS chat planner; ``fallback_*`` = local heuristic plan.
...
def _alias_planner_completion(
    *,
    ...
    base = resolve_llm_openai_compatible_base(os.getenv("PKS_MODEL")).rstrip("/")
...
def parse_mission_with_planner(user_text: str) -> MissionPlan:
    ...
    Uses ``ALIAS_API_KEY`` and ``PKS_MODEL`` (e.g. ``alias1``, ``alias2-mini``, ``alias3``) — the same
    gateway as the rest of PKS (``CSI_CUSTOM_ENDPOINT`` / ``ALIAS_API_URL`` for qualifying ``PKS_MODEL``,
