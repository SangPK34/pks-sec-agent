...
def write_systemd_user_unit(
    *,
    ...
    ExecStart uses ``python -m pks.continuous_ops.loop_runner`` so the unit does not
    ...
    mod = "pks.continuous_ops.loop_runner"
    ...
    env_block += "Environment=PKS_CONTINUOUS_OPS_SYSTEMD_PLAIN_LOG=1\n"
    if avoid_sudo:
        env_block += "Environment=PKS_AVOID_SUDO=1\n"
    env_block += _systemd_extra_env_lines(venv_root=venv_root, pythonpath_extra=pythonpath_extra)
    body = (
        "[Unit]\n"
        f"Description=PKS Continuous Ops worker ({service_stem})\n"
