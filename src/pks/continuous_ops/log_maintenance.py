...
def maintain_log_tree(log_root: Path, *, now: float | None = None) -> None:
    ...
    Tunable via ``PKS_COPS_LOG_FULL_DAYS`` and ``PKS_COPS_LOG_DELETE_AFTER_DAYS`` (set by the
    ...
    try:
        full_days = max(1, int(os.getenv("PKS_COPS_LOG_FULL_DAYS", "7")))
    except ValueError:
        full_days = 7
    try:
        delete_after = max(full_days + 1, int(os.getenv("PKS_COPS_LOG_DELETE_AFTER_DAYS", "15")))
    except ValueError:
...
def main(argv: list[str] | None = None) -> int:
    if not argv or len(argv) < 2:
        print("usage: python -m pks.continuous_ops.log_maintenance <LOG_ROOT>", file=sys.stderr)
        return 2
