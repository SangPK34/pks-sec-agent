...
from pks.config import DEFAULT_AGENT_TYPE
from pks.continuous_ops.model_parse import (
    MissionPlan,
...
from pks.continuous_ops.rate_plan import resolve_rate_tier
from pks.continuous_ops.scriptgen import default_pks_argv, render_loop_script
from pks.continuous_ops.task_queue import initialize_from_plan
from pks.continuous_ops.terminal_launch import detect_external_terminal_backend, spawn_external_terminal
...
def _run_dir_as_tilde(run_dir: Path) -> str:
    """``~/.pks/...`` when under the current user's home; else absolute."""
    try:
...
def _probe_pks_import_on_python(python_exe: str) -> tuple[bool, str]:
    """Return (ok, stderr_or_message) after trying to import PKS with the given interpreter."""
    try:
        ...
        proc = subprocess.run(
            [
                python_exe,
                "-c",
                "import pks; import pks.continuous_ops.loop_runner",
            ],
            capture_output=True,
            text=True,
            timeout=45,
...
def _infer_pythonpath_for_running_pks() -> str | None:
    """If ``pks`` is loaded from a checkout (not ``site-packages``), return a ``PYTHONPATH`` dir for systemd.

    Typical layout: ``…/repo/src/pks/__init__.py`` → return ``…/repo/src`` so ``/usr/bin/python3 -m pks`` works
    when the operator started PKS from a repo without activating a venv.
    """
    try:
        import pks

        init = Path(pks.__file__).resolve()
    except Exception:
        return None
    low = str(init).lower()
...
def _probe_pks_import_minimal_env(
    ...
    try:
        proc = subprocess.run(
            [python_exe, "-c", "import pks; import pks.continuous_ops.loop_runner"],
            capture_output=True,
            text=True,
            timeout=45,
            env=env,
...
def _worker_python_bin() -> str:
    ...
    the same interpreter that can import ``pks``.
    """
...
def _wizard_checkout_root_for_loop_python() -> Path | None:
    """Repository root when this module is loaded from a checkout (``…/src/pks/continuous_ops/wizard.py``)."""
    try:
        here = Path(__file__).resolve()
        root = here.parents[3]
        if (root / "pyproject.toml").is_file() and (root / "src" / "pks").is_dir():
            return root
...
def _loop_python_for_run_script(pks_argv: list[str]) -> str:
    """Python that can ``import pks`` for ``run_loop.py`` shebang and ``python_interpreter`` in JSON.

    When the operator launches PKS via ``.../some_env/bin/pks`` but ``VIRTUAL_ENV`` is unset (common in IDEs),
    ``sys.executable`` may still be system Python — then ``run_loop.py`` would die with ``ModuleNotFoundError``
    before creating ``logs/``. Prefer the venv next to the ``pks`` launcher when its path looks like
    ``*/bin/pks`` **and** that interpreter can import ``pks`` (``/usr/bin/pks`` shims must not force
    ``/usr/bin/python3`` if it does not have PKS).

    If ``pks`` resolves to ``python -m pks`` or PATH is wrong, try the checkout's ``pks_env`` / ``.venv``.
    """
    tried: set[str] = set()
    ...
    def _consider(py: str) -> str | None:
        ...
        if key in tried:
            return None
        tried.add(key)
        ok, _err = _probe_pks_import_on_python(key)
        return key if ok else None

    try:
        if pks_argv:
            pks_p = Path(str(pks_argv[0])).expanduser().resolve()
            if pks_p.is_file() and pks_p.name == "pks" and pks_p.parent.name == "bin":
                for name in ("python3", "python"):
                    cand = pks_p.parent / name
                    if cand.is_file():
    ...
    if (
        len(pks_argv) >= 3
        and str(pks_argv[1]) == "-m"
        and str(pks_argv[2]).replace(".__main__", "") == "pks"
    ):
        got = _consider(str(pks_argv[0]))
        if got:
            return got
...
def _green_head(console: Console, title: str) -> None:
    console.print(Text(title, style=f"bold {PKS_GREEN}"))
...
def _attempt_line(console: Console, attempt: int, max_attempts: int) -> None:
    console.print(Text(f"Attempt {attempt}/{max_attempts}", style=f"bold {PKS_GREEN}"))
...
def _parse_mission_with_planner_and_hints(console: Console, user_mission: str) -> MissionPlan:
    ...
    from pks.repl.ui.startup_hints import StartupHints, startup_hints_disabled
    ...
    phases = [
        "Calling the PKS mission planner…",
...
def _build_tick_prompt(plan: MissionPlan, *, privileged: bool) -> str:
    ...
    if not privileged:
        base += (
            "\n\n[Operator policy] Same as the [Privilege] line above (`PKS_CONTINUOUS_OPS_NO_SUDO`). "
...
def run_continuous_ops_wizard(user_mission: str, console: Console) -> bool:
    ...
    if plan.planner_origin != "planner_api":
        _hint(
            console,
            "[yellow]Mission planner did not obtain parseable JSON from the configured model API[/yellow][dim] — a local fallback "
            "plan was used (same as typing without planner expansion). The summary may echo your raw phrase. "
            "Check OPENAI_API_KEY, OPENAI_BASE_URL, PKS_MODEL, and network access. "
            "If you need technical details, enable DEBUG logging for ``pks.continuous_ops.model_parse``. "
    ...
    if tick is None or tick < math.ceil(min_tick):
        ...
        _grey(
            console,
            "You did NOT specify a tick interval, OR it is NOT VALID for the current throughput profile. "
        ...
        console.print(
            Text.assemble(
                ...
                (str(recommended), f"bold {PKS_GREEN}"),
            )
        )
        ...
        if choice is True:
            ...
            if manual is None:
                _hint(
                    console,
                    "No valid tick interval after three attempts — returning to the PKS prompt.",
                )
                return False
            tick = manual
        else:
    ...
    avoid_glob = (os.environ.get("PKS_AVOID_SUDO", "") or "").strip().lower() in ("1", "true", "yes", "on")
    if avoid_glob:
        _grey(
            console,
            "You already have [bold]PKS_AVOID_SUDO[/bold] enabled in this shell. That is [bold]compatible[/bold] with "
            ...
            "interactive sudo in ticks), and the generated unit also sets [bold]PKS_AVOID_SUDO[/bold] for the "
            "service — you do [bold]not[/bold] need to turn PKS_AVOID_SUDO off to use systemd.",
        )
    _grey(
        console,
    ...
    _grey(
        ...
        "shell [italic]sudo[/italic] inside PKS ticks.",
    )
    _grey(
        console,
        "[bold]Does require[/bold] a working stack on this machine: [bold]Linux[/bold] with [bold]systemctl[/bold] on "
        "PATH, and a Python that can [italic]import pks[/italic] under a [bold]minimal environment[/bold] (like systemd). "
        "If you use a venv, activate PKS from it so [bold]VIRTUAL_ENV[/bold] is set. If you run from a git checkout "
    ...
    _grey(
        ...
        "If you answer [bold]yes[/bold] below, PKS will copy the unit to [bold]~/.config/systemd/user[/bold], run "
    ...
    _sd_pp = _infer_pythonpath_for_running_pks()
    py_probe = _loop_python_for_run_script(default_pks_argv())
    ...
    import_ok, import_err = _probe_pks_import_minimal_env(
        py_probe, venv_root=_sd_venv, pythonpath=_sd_pp
    )
    if import_ok:
        console.print(Text("Check: that Python can import pks + loop_runner (OK).", style=f"bold {PKS_GREEN}"))
    else:
        console.print(
            Text(
        ...
        _grey(
            console,
            "Typical fix: [bold]source your venv[/bold] before launching PKS, or [bold]pip install -e .[/bold] into "
            "that interpreter so [italic]import pks[/italic] works. Avoid relying on [bold]/usr/bin/python3[/bold] if "
            "PKS only exists in a project venv.",
        )
    ...
    sd_question = (
        ...
        if import_ok
            ...
            "(almost always crashes until Python can import pks)"
        )
    )
    ...
    if want_systemd_unit:
        ...
        _grey(
            ...
            "[bold]PKS_AVOID_SUDO=true[/bold] for the worker process and bakes the same flag into the generated "
            "[italic].service[/italic] file.",
        )
        os.environ["PKS_AVOID_SUDO"] = "true"
    else:
        _blank(console)
    ...
    run_dir = Path.home() / ".pks" / "continuous_ops" / f"run_{uuid.uuid4().hex[:12]}"
    tick_prompt = _build_tick_prompt(plan, privileged=want_priv)
    _pks_argv = default_pks_argv()
    _py = _loop_python_for_run_script(_pks_argv)
    ...
    if want_systemd_unit:
        from pks.continuous_ops.systemd_unit import (
        ...
        systemd_unit_path = write_systemd_user_unit(
            run_dir=run_dir,
            service_stem=f"pks-cops-{run_dir.name}",
            python_bin=_py,
            avoid_sudo=True,
            venv_root=_sd_venv,
            ...
            if sys.platform.startswith("linux") and shutil.which("systemctl"):
                ...
                if ok:
                    ...
                    if linger is True:
                        ...
                        if lg_ok:
                            console.print(
                                Text("loginctl enable-linger succeeded for this user.", style=f"bold {PKS_GREEN}")
                            )
                        else:
                            _hint(
                                console,
    ...
    session_name = f"pks-cops-{uuid.uuid4().hex[:8]}"
    ...
    worker_colour_note = (
        "[dim]Worker window: colours match PKS when the terminal reports 256 colours and NO_COLOR is unset. "
    ...
    running_footer = (
        f"[bold {PKS_GREEN}]Continuous ops worker is running under systemd --user[/bold {PKS_GREEN}] — "
        f"tick every [white]{tick}[/white]s, privileged=[white]{priv_line}[/white]."
        if systemd_user_active
        else (
            f"[bold {PKS_GREEN}]Continuous ops worker is running[/bold {PKS_GREEN}] — tick every [white]{tick}[/white]s, "
    ...
    panel_body = "\n".join(
        [
            ...
            *systemd_panel,
            ...
            "For direct utility work and general routing we recommend the Root Agent.[/dim]",
        ]
    )
    _blank(console)
    console.print(
        Panel.fit(
            ...
            title=Text("Continuous Ops — summary", style=f"bold {PKS_GREEN}"),
            border_style=PKS_GREEN,
            style=f"on {FINAL_PANEL_BG}",
        )
    )

    os.environ["PKS_CONTINUOUS_OPS_SETUP_DONE"] = "1"
    os.environ["PKS_AGENT_TYPE"] = DEFAULT_AGENT_TYPE
    os.environ["PKS_AGENT_ROUTE_MODE"] = "auto"
    return True
...
def _cheatsheet_markup_lines(
    ...
    *,
    ...
    lines: list[str] = [
        ...
        f"[bold]>> [/bold]pkill -f \"pks.continuous_ops.loop_runner --run-dir {rd}\"",
        "",
        ...
        "",
        "[dim]Tick wall clock: each [italic]pks[/italic] subprocess is killed after [bold]max(120s, 2×TICK)[/bold]. "
    ...
    if systemd_unit is None:
        lines.extend(
            [
                ...
                "[bold]>> [/bold]tmux new -s pks-cops-ssh",
                f"[bold]>> [/bold]cd {rd}",
...
def _spawn_optional_tmux_attach_window(console: Console, session_name: str) -> None:
    ...
    if spawn_external_terminal(backend, "PKS Continuous Ops — worker", inner):
        console.print(
            Text(
...
def _launch_external_terminal(console: Console, run_dir: Path, script_path: Path) -> str:
    ...
    if not backend or not spawn_external_terminal(backend, "PKS Continuous Ops", cmd):
        console.print(
            Text(
...
def maybe_intercept_continuous_ops_turn(user_input: str, console: Console) -> bool:
    ...
    if os.environ.get("PKS_CONTINUOUS_OPS_SETUP_DONE") == "1":
        return False
    if os.environ.get("PKS_CONTINUOUS_OPS_LOOP_CHILD") == "1":
        return False
    try:
