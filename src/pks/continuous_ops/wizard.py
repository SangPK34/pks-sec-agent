...
from pks.continuous_ops.scriptgen import default_cai_argv, render_loop_script
...
def _loop_python_for_run_script(cai_argv: list[str]) -> str:
    ...
    If ``pks`` resolves to ``python -m pks`` or PATH is wrong, try the checkout's ``cai_env`` / ``.venv``.
    """
    tried: set[str] = set()
    ...
    try:
        if cai_argv:
            ...
            if cai_p.is_file() and cai_p.name == "pks" and cai_p.parent.name == "bin":
    ...
    if (
        ...
        and str(cai_argv[2]).replace(".__main__", "") == "pks"
    ):
...
def run_continuous_ops_wizard(user_mission: str, console: Console) -> bool:
    ...
    if plan.planner_origin != "planner_api":
        _hint(
            console,
            "[yellow]Mission planner did not obtain parseable JSON from the Alias/PKS API[/yellow][dim] — a local fallback "
            "plan was used (same as typing without planner expansion). The summary may echo your raw phrase. "
            "Check ALIAS_API_KEY, PKS_MODEL (e.g. alias1, alias2-mini, alias3), and network access. "
