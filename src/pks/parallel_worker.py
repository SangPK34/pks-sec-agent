...
def _parse_args() -> WorkerArgs:
    parser = argparse.ArgumentParser(description="PKS external parallel worker")
...
async def _run(args: WorkerArgs) -> dict:
    ...
    os.environ["PKS_TOOL_STREAM"] = "true"
    ...
    boot = os.environ.get("PKS_PARALLEL_MCP_BOOTSTRAP", "").strip()
    if boot and os.path.isfile(boot):
        try:
            from pks.repl.commands.mcp import apply_parallel_mcp_bootstrap_file

            await apply_parallel_mcp_bootstrap_file(boot)
        except Exception as e:  # pylint: disable=broad-except
            print(f"[PKS worker] MCP bootstrap skipped: {e}", file=sys.stderr)
...
def main() -> int:
    ...
    try:
        ...
        print(f"[PKS worker] failed to write result file: {write_error}", file=sys.stderr)
        return 2
