...
class SimpleAgentManager:
    ...
    def set_active_agent(self, agent, agent_name: str, agent_id: str = None):
        ...
        with self._registry_lock:  # Thread-safe registration
            ...
            if os.getenv("PKS_TUI_MODE") == "true" and agent_id and agent_id.startswith("T"):
                try:
    ...
    def _set_active_agent_internal(self, agent, agent_name: str, agent_id: str = None):
        ...
        if not self._parallel_agents and not agent_id and os.getenv("PKS_TUI_MODE") != "true":
        ...
        if not self._parallel_agents and not (os.getenv("PKS_TUI_MODE") == "true" and agent_id):
            if is_swarm_agent:
        ...
        history_key = self._agent_id if os.getenv("PKS_TUI_MODE") == "true" else agent_name
        ...
        if os.getenv("PKS_TUI_MODE") == "true" and agent:
    ...
    def get_all_histories(self) -> Dict[str, list]:
        ...
        if os.getenv("PKS_TUI_MODE") == "true":
            ...
            for terminal_num, (agent_name, agent_id) in sorted(terminal_to_agent.items()):
                ...
                # Debug logging
                if os.getenv("PKS_DEBUG", "1") == "1" and agent_id in self._message_history:
    ...
    def reset_registry(self):
        ...
        full_reset = (
            ...
            or not hasattr(self, "_terminal_count")
            or self._terminal_count == 0
        )
    ...
    def extract_shareable_context(self, agent_name: str, history: List[Dict[str, Any]]) -> None:
        ...
        # Extract complete user-assistant exchanges, then retain the latest three.
        recent_exchanges = []
        i = 0
        while i < len(history):
            msg = history[i]
            if msg.get("role") == "user":
                ...
                if last_assistant_idx >= 0:
                    ...
                    if "content" in assistant_msg and assistant_msg["content"]:
                        ...
                        if len(assistant_content) > 2400:
                            assistant_content = (
                                assistant_content[:1200]
                                + "\n...[shared response clipped]...\n"
                                + assistant_content[-1200:]
                            )
        ...
        shareable["key_exchanges"] = recent_exchanges[-3:]
    ...
    def get_shared_context_injection(self) -> str:
        ...
        try:
            from pks.util.session_compact import shared_context_supplement
