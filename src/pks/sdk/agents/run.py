...
def _debug_dump_system_prompt(agent: Agent[Any], prompt: str | None, stage: str) -> None:
    """Write the rendered prompt only when explicitly enabled."""
    if os.getenv("PKS_DEBUG_SYSTEM_PROMPT", "").strip().lower() not in {"1", "true", "yes"}:
        return
    ...
    safe_name = "".join(
        ...
        for char in str(getattr(agent, "name", "unknown"))
    )
    path = Path.home() / f"pks-debug-{stage}-{safe_name}.txt"
    path.write_text(text, encoding="utf-8")
    digest = hashlib.sha256(text.encode("utf-8")).hexdigest()[:16]
    print(
        f"[PKS-PROMPT] stage={stage} agent={getattr(agent, 'name', None)!r} "
        f"len={len(text)} sha256={digest} has_pks_ocr={'pks-ocr' in text} path={path}"
    )
...
# PKS_MAX_TURNS must be converted to an int to avoid type mismatch error when comparing.
max_turns_env = os.getenv("PKS_MAX_TURNS")
...
@dataclass
class StreamChunk:
    """Compatibility container used by older streaming integrations."""
...
class Runner:
                ...
                while True:
                    ...
                    if isinstance(turn_result.next_step, NextStepFinalOutput):
                        ...
                        if (
                            ...
                            try:
                                from pks.agents.patterns.utils import is_swarm_pattern
                                ...
                                if is_swarm_pattern(previous_agent) or is_swarm_pattern(
                                    ...
                                    if hasattr(previous_agent, "name") and hasattr(
                                        ...
                                        from pks.sdk.agents.simple_agent_manager import (
                                            AGENT_MANAGER,
                                        )
    ...
    async def _run_single_turn_streamed(
        ...
        _debug_dump_system_prompt(agent, system_prompt, "rendered-stream")
    ...
    async def _run_single_turn(
        ...
        *,
        ...
        system_prompt = await agent.get_system_prompt(context_wrapper)
        _debug_dump_system_prompt(agent, system_prompt, "rendered")
