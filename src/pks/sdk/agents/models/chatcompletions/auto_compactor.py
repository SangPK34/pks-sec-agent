...
Two-phase compaction strategy:
  Phase 1 — Tool Output Truncation (no LLM call):
...
from pks.config import AUTO_COMPACT_THRESHOLD_MAX, get_config
from pks.output import OUTPUT, StatusEvent
from .token_counter import count_tokens_with_tiktoken

if TYPE_CHECKING:
    from pks.sdk.agents.items import TResponseInputItem

# ---------------------------------------------------------------------------
# Number of recent messages to keep verbatim after compaction.
# Override via PKS_COMPACT_KEEP_RECENT (default 10, range 4–16).
# ---------------------------------------------------------------------------
...
def _set_context_usage_env(current_tokens: float | int, max_tokens: int) -> None:
    ...
    os.environ["PKS_CONTEXT_USAGE"] = str(bounded / max_tokens)


def get_model_max_tokens(model_name: str) -> int:
    """Maximum input/context tokens for *model_name*.

    Uses :func:`pks.util.tokens.get_model_input_tokens` so context usage and
    auto-compact thresholds stay aligned.

    """
    # Runtime override: pin effective context window when the provider reports
    # a smaller limit than our local estimate (prevents missing the 80% cap).
    ...
    try:
        from pks.util.tokens import get_model_input_tokens
...
def _print_compact_inicio(
    ...
    *,
    ...
    sep = Text("─" * w, style=f"dim {PKS_GREEN_COMPACT}")
    title = Text()
    title.append("● ", style=f"bold {PKS_GREEN_COMPACT}")
    title.append("Context compact", style=f"bold {PKS_GREEN_COMPACT}")
...
def _build_compact_during_renderable(
    ...
    *,
    ...
    def _row_phase(
        *,
        ...
        if running:
            ...
            sp = Spinner("dots", style=f"bold {PKS_GREEN_COMPACT}")
            rest = Text()
            rest.append(f"{name}", style=f"bold {PKS_GREEN_COMPACT}")
        ...
        t = Text()
        if done:
            t.append("  ● ", style=f"bold {PKS_GREEN_COMPACT}")
            t.append(f"{name}", style=f"bold {PKS_GREEN_COMPACT}")
            t.append(" — ", style="dim")
    ...
    sep2 = Text("─" * w, style=f"dim {PKS_GREEN_COMPACT}")
...
def _print_compact_fin(
    ...
    *,
    ...
    elapsed: float,
    tag: str = "",
    style: str = f"bold {PKS_GREEN_COMPACT}",
    ...
    sep = Text("─" * w, style=f"dim {PKS_GREEN_COMPACT}")
    line = Text()
    line.append("  ● ", style=f"bold {PKS_GREEN_COMPACT}")
    ...
    if tag:
        line.append(f" · {tag}", style=f"dim {PKS_GREEN_COMPACT}")
    parts = [sep, line]
    if success_footer:
        foot = Text()
        foot.append("    ", style="")
...
# ===================================================================
# Phase 1 — tool output truncation on message_history (no LLM call)
# ===================================================================
...
def _compaction_keep_start_index(message_history: list, target_tail: int) -> int:
    ...
    * Contain at least one ``role: user`` whenever any user message exists in
      ``message_history``.  Otherwise LiteLLM / gpt-5.6-terra rejects the request with
...
def _build_old_messages_text(
    ...
    for msg in message_history[:first_kept_index]:
        ...
        if isinstance(content, str) and content.strip():
            # Truncate the summary input to keep the compaction call small.
            if len(content) > 2000:
...
async def _phase2_summarize(
    ...
    *,
    ...
    try:
        ...
        from pks.util.llm_api_base import resolve_openai_api_key
...
async def auto_compact_if_needed(
    *,
    ...
    Phase 1: Truncate large tool outputs in old messages (no LLM call).
    ...
    if not cfg.auto_compact:
        _debug_skip("auto-compact disabled (config / PKS_AUTO_COMPACT)")
    ...
    from pks.util.streaming import register_compaction_live, unregister_compaction_live
    ...
    try:
        ...
        from pks.util.session_compact import get_keep_recent_messages
        ...
        if summary:
            try:
                ...
                from pks.util.session_compact import record_compaction_result
...
def _log_compaction_metrics(
    *,
    ...
    try:
        import pathlib
        log_dir = pathlib.Path.home() / ".pks" / "compaction_logs"
