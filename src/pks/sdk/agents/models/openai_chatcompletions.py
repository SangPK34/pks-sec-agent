...
all existing ``from pks.sdk.agents.models.openai_chatcompletions import X``
...
def _empty_completion_max_failures() -> int:
    ...
    raw = (os.environ.get("PKS_EMPTY_COMPLETION_MAX_FAILURES") or "3").strip()
    try:
        n = int(raw)
    except ValueError:
        n = 3
...
# Force compact after repeated empty responses when the estimated input is
# already large enough that context pressure is a likely cause.
...
def _empty_completion_force_compact_threshold(model_max_tokens: int) -> int:
    ...
    override = (os.environ.get("PKS_EMPTY_COMPLETION_FORCE_COMPACT_MIN_TOKENS") or "").strip()
    if override:
        try:
...
def _value_has_stream_signal(value: Any) -> bool:
    if value is None:
        return False
...
def _is_effectively_empty_stream_accumulation(
    ...
    if state.refusal_content_index_and_output:
        refusal = (
            getattr(state.refusal_content_index_and_output[1], "refusal", None) or ""
        ).strip()
...
class _NullSessionRecorder:
    """No-op recorder used when the model runs outside a CLI session."""
    ...
    @staticmethod
    def log_user_message(*_args: Any, **_kwargs: Any) -> None:
        return None

    @staticmethod
    def log_assistant_message(*_args: Any, **_kwargs: Any) -> None:
        return None

    @staticmethod
    def rec_training_data(*_args: Any, **_kwargs: Any) -> None:
        return None
...
class OpenAIChatCompletionsModel(Model):
    ...
    def __init__(
        ...
        session_recorder = get_session_recorder()
        self._has_session_recorder = session_recorder is not None
        self.logger = session_recorder or _NullSessionRecorder()
    ...
    def _messages_for_token_count_after_history_mutation(
        ...
        *,
        ...
        rebuild from ``input`` alone — that drops history and makes ``PKS_CONTEXT_USAGE`` look
            ...
            if not has_system:
                ...
                for msg in converted_messages:
                    if msg.get("role") == "system":
                        ...
                        break
        try:
            from pks.util import fix_message_list
    ...
    def get_token_info(self) -> dict[str, Any]:
        """Return token/context metadata for CLI and TUI renderers."""
    ...
    async def get_response(
        ...
        with generation_span(
            ...
            os.environ['PKS_CONTEXT_USAGE'] = str(context_usage)
                ...
                if _first:
                    if _is_effectively_empty_assistant_message(_first.message):
                        logger.debug(
                            ...
                            "see PKS_DEBUG=2 if you need the raw object)."
                        )
                    else:
                        logger.debug(
            ...
            if should_display_message:
                ...
                previous_stream_setting = os.environ.get("PKS_STREAM", "false")
                os.environ["PKS_STREAM"] = "false"  # Force non-streaming mode for markdown parsing
                ...
                os.environ["PKS_STREAM"] = previous_stream_setting
        ...
        *,
        ...
        try:
            ...
            with generation_span(
                ...
                try:
                    async for chunk in stream:
                        ...
                        if reasoning_content:
                            ...
                            if should_show_rich_stream and thinking_context is None:
                                thinking_context = create_claude_thinking_context(
                                    self.agent_name,
                        ...
                        if content:
                            ...
                            if thinking_context:
                                from pks.util import finish_claude_thinking_display
                            ...
                            # Update streaming display if enabled - ALWAYS respect PKS_STREAM setting
                        ...
                        if tool_calls:
                            for tc_delta in tool_calls:
                                ...
                                if tool_call_msg not in streamed_tool_calls:
                                    ...
                                    from pks.util.streaming import _compact_suppresses_verbose
                                    if (
                                        not _compact_suppresses_verbose()
                ...
                # Keep only token/context statistics; billing is intentionally absent.
                final_stats = {
    ...
    async def _fetch_response(
        ...
        if os.getenv("PKS_SHOW_CACHE", "").lower() in ("true", "1", "yes"):
        ...
        if system_instructions:
            # The active agent's freshly rendered instructions are authoritative.
            # History can contain a stale system message after resume/agent switch;
            # replace it instead of silently retaining the previous agent's prompt.
            from pks.sdk.agents.simple_agent_manager import AGENT_MANAGER

            shared_context = AGENT_MANAGER.get_shared_context_injection()
            final_system_instructions = system_instructions + shared_context
            system_message = next(
                (msg for msg in converted_messages if msg.get("role") == "system"),
                None,
            )
            if system_message is None:
                ...
                system_message["content"] = final_system_instructions
            ...
            if os.getenv("PKS_DEBUG_SYSTEM_PROMPT", "").strip().lower() in {"1", "true", "yes"}:
                ...
                safe_name = "".join(
                    ...
                    for char in str(self.agent_name or "unknown")
                )
                path = Path.home() / f"pks-debug-sent-{safe_name}.txt"
                path.write_text(text, encoding="utf-8")
                digest = hashlib.sha256(text.encode("utf-8")).hexdigest()[:16]
                print(
                    f"[PKS-PROMPT] stage=sent agent={self.agent_name!r} "
                    f"len={len(text)} sha256={digest} "
                    f"has_pks_ocr={'pks-ocr' in text} path={path}"
                )
        ...
        configured_model = str(agent_model if agent_model else self.model)

        kwargs = {
            "model": api_model_id(configured_model),
            "messages": converted_messages,
        ...
        kwargs["api_base"] = resolve_openai_base_url().rstrip("/")
        kwargs["api_key"] = resolve_openai_api_key()
        kwargs["custom_llm_provider"] = "openai"

        effort = (
            getattr(model_settings, "reasoning_effort", None)
            or os.getenv("PKS_REASONING_EFFORT", "")
        ).strip().lower()
        if effort and effort not in {"off", "none", "false", "0"}:
    ...
    def _get_client(self) -> AsyncOpenAI:
        if self._client is None:
            ...
            if not api_key:
                _c = get_config()
                raise UserError(
                    "Missing OPENAI_API_KEY for the configured model endpoint. "
                    f"(PKS_MODEL={_c.model!r})"
                )
