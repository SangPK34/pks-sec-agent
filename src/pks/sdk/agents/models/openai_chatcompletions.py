...
from pks.sdk.agents.simple_agent_manager import SimpleAgentManager, AGENT_MANAGER
...
from pks.util.llm_api_base import (
...
from pks.util.gateway_rate_limiter import (
...
# One-shot stderr line when PKS_UNRESTRICTED_LOG=1 (confirm client sends steering payload).
...
class OpenAIChatCompletionsModel(Model):
    ...
    async def get_response(
        ...
        with generation_span(
            ...
            if response.usage:
                ...
                try:
                    import pks.util
                    pks.util.COST_TRACKER.interaction_input_tokens = self.interaction_input_tokens
                    pks.util.COST_TRACKER.interaction_output_tokens = self.interaction_output_tokens
                    pks.util.COST_TRACKER.interaction_reasoning_tokens = self.interaction_reasoning_tokens
                    pks.util.COST_TRACKER.cache_read_tokens = self.cache_read_tokens
                    pks.util.COST_TRACKER.cache_creation_tokens = self.cache_creation_tokens
                except Exception:
                    pass
            else:
                self.cache_read_tokens = 0
                ...
                try:
                    async for chunk in stream:
                        ...
                        if content:
                            ...
                            # Skip accurate token counting and cost checking during stream as per user request
                            output_text += content
    ...
    async def _fetch_response(
        ...
        _model_for_base = str(kwargs.get("model") or os.getenv("PKS_MODEL") or "")
        ...
        # Abliteration steering: PKS_UNRESTRICTED=true → steering ON + thinking OFF.
        ...
        # Gate steering payload on Alias-backed models only. PKS_UNRESTRICTED must not
        ...
        if _unrestricted and not _UNRESTRICTED_EXTRA_BODY_LOGGED:
            if os.getenv("PKS_UNRESTRICTED_LOG", "").strip().lower() in (
                "1",
                "true",
                "yes",
                "on",
                ...
                print(
                    "[PKS] Unrestricted: client will send extra_body="
        ...
        _model_for_openai_base = str(kwargs.get("model") or os.getenv("PKS_MODEL") or "")
        openai_api_base = (
        ...
        while retry_count < max_retries:
            try:
                ...
                if (
                    ...
                    try:
                        from pks.util import print_message_history
                ...
                if (
                    ...
                    try:
                        ...
                        if isinstance(inferred_max, int) and inferred_max > 0:
                            os.environ["PKS_MODEL_MAX_INPUT_TOKENS"] = str(inferred_max)
        ...
        if self._client is None:
            ...
            if not api_key:
                ...
                raise UserError(
                    ...
                    "For alias-family models (alias*/pks*/csi*), set ALIAS_API_KEY. "
