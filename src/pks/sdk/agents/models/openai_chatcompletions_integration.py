...
def integrate_openai_chatcompletions_display():
    ...
    # Import the module
    import pks.sdk.agents.models.openai_chatcompletions as openai_module

    # Import the display wrapper
    from pks.tui.display.wrapper import DISPLAY
    
    # Ensure original functions are stored by the display layer if needed
    try:
        from pks.tui.display.original_functions import store_original_functions
    ...
    if "pks.util" in sys.modules:
        util_module = sys.modules["pks.util"]
        ...
        try:
            from pks.tui.display.console_interceptor import TUIConsoleInterceptor
