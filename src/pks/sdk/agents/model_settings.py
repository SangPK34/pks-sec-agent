...
def get_default_temperature() -> float:
    ...
    try:
        return float(os.getenv("PKS_TEMPERATURE", str(DEFAULT_TEMPERATURE)))
...
def get_default_top_p() -> float:
    ...
    try:
        return float(os.getenv("PKS_TOP_P", str(DEFAULT_TOP_P)))
    except (ValueError, TypeError):
...
class ModelSettings:
    ...
    Default values:
    - temperature: 0.7 (can be overridden by PKS_TEMPERATURE env var)
    - top_p: 1.0 (can be overridden by PKS_TOP_P env var)
    """

    temperature: float | None = None
    """The temperature to use when calling the model. Default: 0.7 (from PKS_TEMPERATURE env var)."""

    top_p: float | None = None
    """The top_p to use when calling the model. Default: 1.0 (from PKS_TOP_P env var)."""
    ...
    """Optional provider reasoning level. Omitted unless explicitly configured."""
    ...
    def with_defaults(self) -> ModelSettings:
        ...
        Uses PKS_TEMPERATURE env var (default 0.7) and PKS_TOP_P env var (default 1.0).
        """
        return replace(
            self,
    ...
    def from_env(cls) -> ModelSettings:
        ...
        Reads PKS_TEMPERATURE (default 0.7) and PKS_TOP_P (default 1.0).
        """
        return cls(
            temperature=get_default_temperature(),
            top_p=get_default_top_p(),
            reasoning_effort=os.getenv("PKS_REASONING_EFFORT") or None,
        )
