...
def truncate_output(output: Any, max_length: int | None = None) -> str:
    ...
    Threshold is intentionally modest (``PKS_TOOL_OUTPUT_MAX`` chars, default 4000)
    ...
    try:
        limit = int(os.getenv("PKS_TOOL_OUTPUT_MAX", "4000")) if max_length is None else int(max_length)
        limit = max(800, limit)
    ...
    try:
        ...
        middle = output_str[side:len(output_str) - side]
        ...
        for line in middle.splitlines():
            ...
            if len(s) < 3 or s in seen:
                continue
            if flag_re.search(s):               # flags: rarest + most critical -> always win
                if len(flags) < 15:
                    seen.add(s)
                    flags.append(s[:240])
            elif strong_re.search(s):           # urls / creds / errors
                if len(strong) < 40:
                    seen.add(s)
                    strong.append(s[:240])
            elif len(weak) < 40:                # hex / base64 blobs (skip repeated-char noise)
                m = weak_re.search(s)
        ...
        marker = (
            f"... [truncated: ~{hidden:,} chars / ~{n_lines:,} middle lines hidden"
    ...
    async def execute_function_tool_calls(
        ...
        *,
        ...
        if os.getenv("PKS_TUI_MODE") == "true":
            try:
                with open(f"{_PKS_DEBUG_DIR}/cai_tui_error.log", "a") as f:
                    import datetime
