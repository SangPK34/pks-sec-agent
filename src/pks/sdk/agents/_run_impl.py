...
# on pks.output.OUTPUT, which the CompactCLIHandler consumes to render the
...
def _emit_compact_task_start(
    ...
    try:
        ...
        from pks.repl.ui.task_label import infer_task_label
...
def _emit_compact_task_complete(task_id: str, started_at: float, result: Any) -> None:
    """Emit a UI-bounded :class:`TaskCompleteEvent` for ``task_id``."""
    import time

    try:
        from pks.output import OUTPUT, TaskCompleteEvent
        from pks.util.terminal import _prepare_tool_output_for_display

        OUTPUT.emit(
            TaskCompleteEvent(
                task_id=task_id,
                output=(
                    _prepare_tool_output_for_display("", result)
...
def _emit_compact_task_error(task_id: str, started_at: float, error: BaseException) -> None:
    ...
    try:
        from pks.output import OUTPUT, TaskErrorEvent
...
def _signal_pats():
    """Lazily-compiled (flag, strong, weak) matchers for rescuing lines from a dropped middle."""
    global _SIGNAL_PATS
    if _SIGNAL_PATS is None:
        import re

        flag = re.compile(r"[A-Za-z0-9_]{2,}\{[^{}\n]{1,200}\}")   # flag-like WORD{...}
        strong = re.compile(
            r"https?://|ftp://"                                    # URLs
            r"|password|passwd|secret|token|api[_-]?key|private[_-]?key"
            r"|BEGIN [A-Z ]+PRIVATE|credential|\bflag\b|user(?:name)?|login|ssh-rsa"
            r"|error|exception|denied|failed|traceback",            # errors
            re.IGNORECASE,
        )
        weak = re.compile(r"\b[0-9a-fA-F]{16,}\b|[A-Za-z0-9+/]{24,}={0,2}")  # hex/base64 blobs
        _SIGNAL_PATS = (flag, strong, weak)
    return _SIGNAL_PATS


def truncate_output(output: Any, max_length: int | None = None) -> str:
    """Smartly truncate large tool output before it enters the model's history.

    Keeps a generous head + tail, PLUS high-signal lines rescued from the dropped
    middle (flags ``WORD{...}``, URLs, creds/keywords, long hex/base64, errors) so a
    flag or key sitting in the middle is never lost. A marker records how much was
    hidden so the model can re-run a narrowed ``grep``/``sed`` for the rest.

    Threshold is intentionally modest (``PKS_TOOL_OUTPUT_MAX`` chars, default 20000)
    so history stays lean over a long session; small/medium outputs pass untouched.
    """
    try:
    ...
    try:
        limit = int(os.getenv("PKS_TOOL_OUTPUT_MAX", "20000")) if max_length is None else int(max_length)
        limit = max(800, limit)
    ...
    try:
        side = max(600, limit // 3)
        head = output_str[:side]
        tail = output_str[-side:]
        middle_start = side
        middle_end = len(output_str) - side

        flag_re, strong_re, weak_re = _signal_pats()
        seen: set[str] = set()
        flags: list[str] = []
        strong: list[str] = []
        weak: list[str] = []
        middle_lines = 0
        cursor = middle_start
        while cursor < middle_end:
            newline = output_str.find("\n", cursor, middle_end)
            line_end = middle_end if newline < 0 else newline
            line = output_str[cursor:line_end]
            middle_lines += 1
            s = line.strip()
            if len(s) >= 3 and s not in seen and flag_re.search(s):
                if len(flags) < 15:
                    seen.add(s)
                    flags.append(s[:240])
            elif len(s) >= 3 and s not in seen and strong_re.search(s):
                if len(strong) < 40:
                    seen.add(s)
                    strong.append(s[:240])
            elif len(s) >= 3 and s not in seen and len(weak) < 40:
                m = weak_re.search(s)
                if m and len(set(m.group(0))) >= 3:
                    seen.add(s)
                    weak.append(s[:240])
            if newline < 0:
                break
            cursor = newline + 1
        kept = (flags[:15] + strong[:15] + weak[:5])[:25]

        hidden = len(output_str) - 2 * side
        marker = (
            f"... [truncated: ~{hidden:,} chars / ~{middle_lines:,} middle lines hidden"
            + (f"; {len(kept)} high-signal line(s) rescued below" if kept else "")
            + " — re-run a narrowed grep/sed if you need the rest] ..."
        )
        parts = [head, "", marker]
        if kept:
            parts.append("--- rescued from middle (flags / urls / keys / errors) ---")
            parts.extend(kept)
            parts.append("--- end rescued ---")
        parts += ["", tail]
        llm_memory_output = "\n".join(parts)
        if len(llm_memory_output) <= limit:
            return llm_memory_output

        marker = "\n... [truncated: output exceeded LLM memory limit] ...\n"
        content_budget = limit - len(marker)
        head_size = content_budget // 2
        tail_size = content_budget - head_size
        return output_str[:head_size] + marker + output_str[-tail_size:]
    except Exception:
        marker = "\n... TRUNCATED ...\n"
        content_budget = limit - len(marker)
        head_size = content_budget // 2
    ...
    async def execute_function_tool_calls(
        ...
        *,
        ...
        if os.getenv("PKS_TUI_MODE") == "true":
            try:
                with open(f"{_PKS_DEBUG_DIR}/pks_tui_error.log", "a") as f:
                    import datetime
        ...
        async def run_single_tool(
            ...
            # Bind the actual Runner agent to this child task.  Tool renderers
            # must not guess ownership from a process-global model registry.
            _tool_agent_context_token = None
            try:
                from pks.tools.streaming import push_tool_execution_agent

                _tool_agent_context_token = push_tool_execution_agent(agent)
            except Exception:
                pass
            ...
            try:
                ...
                if _tool_agent_context_token is not None:
                    try:
                        from pks.tools.streaming import pop_tool_execution_agent

                        pop_tool_execution_agent(_tool_agent_context_token)
                    except Exception:
                        pass
            return result
        ...
        try:
            if tool_runs:
                from pks.util.wait_hints import tool_batch_wait_hints
        ...
        # Shared blackboard: auto-log every agent's tool call + result head so ALL agents see
        # what was already tried (survives handoffs/interrupts). Best-effort; never break tools.
        try:
            import json as _pks_json

            from pks.util import blackboard as _pks_bb

            _pks_agent = getattr(agent, "name", "") or ""
            _pks_skip = {
                "note_finding", "set_flag", "check_available_agents",
                "analyze_task_requirements", "get_agent_number",
            }
            for _tr, _res in zip(tool_runs, results):
                _tn = getattr(_tr.function_tool, "name", "") or "tool"
                if _tn in _pks_skip:
                    continue
                _a = getattr(getattr(_tr, "tool_call", None), "arguments", "") or ""
                if _tn == "generic_linux_command":
                    _cmd = _a
                    try:
                        _pd = _pks_json.loads(_a)
                        _cmd = _pd.get("command") or _pd.get("full_command") or _a
                    except Exception:
                        pass
                    _pks_bb.note_command(_pks_agent, str(_cmd), output=str(_res))
                else:
                    _pks_bb.note_command(_pks_agent, f"{_tn} {str(_a)[:120]}", output=str(_res))
        except Exception:
            pass
        ...
        for tool_run, result in zip(tool_runs, results):
            raw_output = result
            llm_memory_output = truncate_output(raw_output)
            function_results.append(
                FunctionToolResult(
                    ...
                    run_item=ToolCallOutputItem(
                        output=raw_output,
                        raw_item=ItemHelpers.tool_call_output_item(
                            tool_run.tool_call, llm_memory_output
                        ),
                        agent=agent,
                    ),
                )
    ...
    async def execute_handoffs(
        ...
        *,
        ...
        with handoff_span(from_agent=agent.name) as span_handoff:
            ...
            try:
                from pks.tui.display.handoff_context import propagate_display_context_to_agent
