...
Usage:
    ...
    pks-replay path/to/file.jsonl 0.5
...
Or alternatively:
    asciinema rec --command="JSONL_FILE_PATH='caiextensions-memory/caiextensions/memory/it/pentestperf/hackableii/hackableII_autonomo.jsonl' REPLAY_DELAY='0.05' pks-replay"
...
os.environ["PKS_DISABLE_SESSION_RECORDING"] = "true"

# Add the parent directory to the path to import pks modules
...
from pks.sdk.agents.run_to_jsonl import get_token_stats, load_history_from_jsonl
...
def replay_conversation(
    ...
    # Model, token totals, and timing.
    file_model = usage[0]
    total_input_tokens = usage[1]
    total_output_tokens = usage[2]
    active_time = usage[3] if len(usage) > 3 else 0
    idle_time = usage[4] if len(usage) > 4 else 0
    ...
    for i, message in enumerate(messages):
        try:
            ...
            if role == "user":
                print(color(f"PKS> ", fg="cyan") + f"{content}")
                turn_counter += 1
...
def main():
    ...
    try:
        ...
        # Get token and timing stats from the JSONL file
        usage = get_token_stats(jsonl_file_path)

        # Display timing information if available (new format)
        if len(usage) > 3:
            print(color(f"Active time: {usage[3]:.2f}s", fg="blue"))
            print(color(f"Idle time: {usage[4]:.2f}s", fg="blue"))
        ...
        if "PKS_DISABLE_SESSION_RECORDING" in os.environ:
            del os.environ["PKS_DISABLE_SESSION_RECORDING"]
