    ...
    def create_daily_logs(self) -> Optional[str]:
        ...
        if "2025-05-30" in daily_counts.index:
            ...
            ax.axvline(
                ...
                color="green",
                linestyle="--",
                label='"PKSv0.4.0" and "gpt-5.6-terra" releases',
            )
...
def parse_logs(file_path, parse_ips=False):
    ...
    # 1. Old: ...-pks_20250405_091537_root_linux_6.10.14-linuxkit_81_38_188_36.jsonl
    old_pattern = re.compile(
        r"pks_(\d{8})_(\d{6})_([^_]+)_([^_]+)_([^_]+)_(\d+)_(\d+)_(\d+)_(\d+)\.jsonl$"
    )
    # 2. New: uuid_pks_uuid_20250426_054313_root_linux_6.12.13-amd64_177_91_253_204.jsonl
    new_pattern = re.compile(
        r"([\w-]+)_pks_([\w-]+)_(\d{8})_(\d{6})_([^_]+)_([^_]+)_([^_]+)_([\d]+)_([\d]+)_([\d]+)_([\d]+)\.jsonl$"
    )
        ...
        for line in file:
            try:
                ...
                if "pks_" in filename:
                    ...
                    if m_new:
                        # uuid_pks_uuid_YYYYMMDD_HHMMSS_user_system_version_ip.jsonl
...
def get_overall_stats():
    """Fetch overall download statistics for pks-framework"""
    url = "https://pypistats.org/api/packages/pks-framework/overall"
...
def get_system_stats():
    """Fetch system-specific download statistics for pks-framework"""
    url = "https://pypistats.org/api/packages/pks-framework/system"
    response = requests.get(url)
    if response.status_code == 200:
        return response.json()
    else:
...
def create_pypi_plot():
    ...
    ax1.set_title("PKS Framework Download Statistics", fontsize=14, pad=20)
