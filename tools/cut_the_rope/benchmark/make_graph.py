...
Usage:
    ...
    python tools/cut_the_rope/benchmark/make_graph.py --json /path/to/pks/tools/cut_the_rope/benchmark/data/mercadolibre/mercadolibre_graph.jsonl --input_log /path/to/pks/tools/cut_the_rope/benchmark/data/mercadolibre/mercadolibre.jsonl
Arguments:
...
The script performs the following steps:
    ...
    2. Computes token statistics from the log.
    3. Loads the graph structure from the specified JSON file.
    4. Computes edge probabilities using tokens and message count.
...
from pks.ctr.experiment import (
    NodeInfo,
    EdgeInfo,
    GraphStructure,
    run_ctr_baseline_analysis,
    preprocess_graph,
    postprocess_graph,
    parse_input_log,
)
from pks.ctr.probability_computation import compute_edge_probabilities_offline, get_log_tokens
...
def main():
    ...
    (model_name_log, total_prompt_tokens, total_completion_tokens,
        last_active_time, last_idle_time) = get_token_stats(args.input_log)
    total_tokens_real = total_prompt_tokens + total_completion_tokens

    # Obtain heuristic tokens.
