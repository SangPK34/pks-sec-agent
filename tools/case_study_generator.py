...
PKS_MODEL="claude-sonnet-4-20250514" PKS_STREAM=True python3 case_study_generator.py --jsonl_file logs/cai_b97af8fc-3d51-45d3-8393-6c3341d33807_20250602_201144_luijait_darwin_24.5.0_81_38_189_27.jsonl --output_php_file alias_web/case_study_test.php

PKS Case Study Generator - Generate PHP case studies from JSONL files.

This script loads context from JSONL files using the same mechanism as PKS's /load command,
...
# Import PKS SDK components
from pks.sdk.agents import Runner
from pks.sdk.agents.models.openai_chatcompletions import message_history, add_to_message_history
...
from src.pks.agents.usecase import use_case_agent
...
async def generate_case_study(jsonl_file: str, output_php_file: str) -> Optional[str]:
    ...
    stream_mode = os.getenv("PKS_STREAM", "true").lower() != "false"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Generate PHP case studies from JSONL files using PKS UseCase agent.",
...
Examples:
  ...
  # Disable streaming
  PKS_STREAM=false python case_study_generator.py --jsonl_file logs/last --output_php_file output.php
        """,
    )
    parser.add_argument(
        "--jsonl_file",
...
async def main():
    ...
    console.print(
        Panel(
            "[bold cyan]PKS Case Study Generator[/bold cyan]\n"
            "Generate professional cybersecurity case studies from JSONL session logs\n\n"
            "[dim]This tool uses the PKS UseCase agent to analyze session context and generate\n"
    ...
    if args.model:
        os.environ["PKS_MODEL"] = args.model
